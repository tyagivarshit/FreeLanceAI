...
export const SettingsPage = () => {
        ...
        try {
            const [profileRes, sessionsRes, extRes, billingRes] = await Promise.all([
                ...
                settingsService
                    ...
                    .catch(() => ({ success: false, extension: null })),
    ...
    const handleDataExport = async () => {
        ...
        try {
            ...
            if (res.success && res.export) {
                const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(res.export, null, 2));
                const downloadAnchor = document.createElement("a");
                downloadAnchor.setAttribute("href", dataStr);
                downloadAnchor.setAttribute("download", `freelanceos-data-export-${new Date().toISOString().slice(0, 10)}.json`);
    ...
    return (<AppLayout currentPath="/settings.html" breadcrumbTitle="Settings">
      <Toast toast={toast}/>
      ...
      <div className="settings-tabs-nav" role="tablist" aria-label="Settings Categories">
        {[
            ...
            return (<button key={tab.key} id={`tab-${tab.key}`} className={`settings-tab-btn ${isActive ? "active" : ""}`} role="tab" aria-selected={isActive} aria-controls={`panel-${tab.key}`} onClick={() => switchTab(tab.key)}>
              {tab.label}
            </button>);
        })}
      </div>

      {loading ? (<div className="settings-skeleton-container" id="settings-skeleton">
          <Skeleton variant="card" className="settings-card-skeleton"/>
          <Skeleton variant="card" className="settings-card-skeleton"/>
        </div>) : error ? (<div className="feed-error-state card-like-error" id="settings-error" role="alert">
          ...
          <button className="btn btn-secondary btn-retry-large" id="settings-retry-btn" type="button" onClick={() => loadSettingsData()}>
            Retry
          </button>
        </div>) : (<div className="settings-content" id="settings-content">
          {/* 1. Profile Tab Panel */}
          {activeTab === "profile" && (<section id="panel-profile" className="settings-panel active" role="tabpanel" aria-labelledby="tab-profile">
                ...
                <div className="settings-card-body">
                  <div className="form-grid">
                    <div className="form-group">
                      ...
                      <input id="profile-email" type="email" className="form-input" readOnly disabled value={profile?.email || ""}/>
                    ...
                    <div className="form-group">
                      ...
                      <input id="profile-user-id" type="text" className="form-input code-font" readOnly disabled value={profile?.userId || ""}/>
                    ...
                    <div className="form-group">
                      ...
                      <div className="status-badge-wrapper">
                        <span id="profile-status-badge" className={`badge ${profile?.status === "active" ? "badge-success" : "badge-warning"}`}>
          ...
          {activeTab === "security" && (<section id="panel-security" className="settings-panel active" role="tabpanel" aria-labelledby="tab-security">
              ...
              <div className="settings-card">
                ...
                <div className="settings-card-body">
                  <form id="form-password-change" className="settings-form" onSubmit={handlePasswordSubmit} noValidate>
                    <div className="form-group">
                      <label htmlFor="current-password" className="form-label">
                        Current Password
                      </label>
                      <input id="current-password" type="password" className="form-input" required autoComplete="current-password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} disabled={passwordLoading}/>
                    </div>
                    ...
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input id="new-password" type="password" className="form-input" minLength={8} required autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} disabled={passwordLoading}/>
                      ...
                      <div className="form-group">
                        ...
                        <input id="confirm-password" type="password" className="form-input" minLength={8} required autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={passwordLoading}/>
                      </div>
                    </div>

                    {passwordMessage && (<div id="password-alert" className={`alert-box ${passwordMessage.isError ? "alert-error" : "alert-success"}`} role="alert">
              ...
              <div className="settings-card">
                <div className="settings-card-header session-header-flex">
                  ...
                  {sessions.length > 1 && (<Button variant="secondary" size="sm" id="btn-revoke-all-sessions" onClick={confirmRevokeAllOthers}>
                ...
                <div className="settings-card-body">
                  <div id="sessions-list" className="sessions-list-grid" role="list">
                    ...
                    return (<div key={session.sessionId} className={`session-item-card ${isCurrent ? "session-item-current" : ""}`} role="listitem">
                          <div className="session-icon-wrap" aria-hidden="true">
                            <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor">
                              <path d="M4 6h16v10H4z" fill="none" stroke="currentColor" strokeWidth="2"/>
                          ...
                          <div className="session-info-wrap">
                            ...
                            `${session.browser || "Web Browser"} on ${session.platform || "Device"}`}
                              ...
                              {isCurrent && (<span className="badge badge-success badge-sm">Current Device</span>)}
                            </div>
                          ...
                          {!isCurrent && (<Button variant="secondary" size="sm" className="session-revoke-btn" onClick={() => confirmRevokeSession(session)}>
                              Revoke
                            </Button>)}
                        </div>);
                })}
          ...
          {activeTab === "data" && (<section id="panel-data" className="settings-panel active" role="tabpanel" aria-labelledby="tab-data">
              <div className="settings-card">
                <div className="settings-card-header">
                ...
                <div className="settings-card-body">
                  <div className="export-box">
                    ...
                    <Button variant="secondary" id="btn-export-data" loading={exportLoading} onClick={handleDataExport}>
                      <svg className="btn-icon" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">
              ...
              <div className="settings-card danger-card">
                ...
                <div className="settings-card-body">
                  <div className="danger-action-row">
                    ...
                    <Button variant="danger-outline" id="btn-danger-reset" onClick={() => {
                    setModalConfig({
                        title: "Reset Workspace Data",
                        message: "This will permanently delete all cached matches and activity logs. Are you sure?",
          ...
          {activeTab === "extension" && (<section id="panel-extension" className="settings-panel active" role="tabpanel" aria-labelledby="tab-extension">
              <div className="settings-card">
                <div className="settings-card-header">
                ...
                <div className="settings-card-body">
                  ...
                  <div className="sync-toggles-section">
                    ...
                    <div className="toggle-row">
                      ...
                      <input id="toggle-auto-import" type="checkbox" className="toggle-input" defaultChecked/>
                    </div>
                    <div className="toggle-row">
                      ...
                      <input id="toggle-bg-sync" type="checkbox" className="toggle-input" defaultChecked/>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "billing" && (<section id="panel-billing" className="settings-panel active" role="tabpanel" aria-labelledby="tab-billing">
              <div className="settings-card">
                ...
                <div className="settings-card-body">
                  <div className="billing-summary-box">
                    ...
                    subscription?.trialDaysRemaining !== undefined && (<div id="billing-trial-info" className="badge badge-trial">
                              {subscription.trialDaysRemaining} days remaining on Pro Trial
                            </div>)}
                      </div>
                      <div className="billing-actions-group">
                        {subscription?.hasCustomer && (<Button variant="secondary" id="btn-settings-portal" loading={portalLoading} onClick={handleOpenPortal}>
                            Manage in Stripe Portal ↗
                          </Button>)}
                        <a id="link-settings-upgrade" href="/billing.html" className="btn btn-primary">
                          View Plans &amp; Upgrade
                        </a>
                      </div>
                    </div>
      ...
      {/* Confirmation Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={modalConfig.title} footer={<>
