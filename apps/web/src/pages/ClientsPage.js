...
export const ClientsPage = () => {
    ...
    const loadClients = useCallback(async (p, s, signal) => {
        setLoading(true);
        setError(null);
        try {
            const params = {
    ...
    return (<AppLayout currentPath="/clients.html" breadcrumbTitle="Clients" topbarActions={<button className="topbar-action-btn" id="clients-refresh-icon" aria-label="Refresh clients" onClick={() => loadClients(page, statusFilter)}>
      ...
      <section className="welcome-section clients-title-row" aria-label="Clients heading">
        ...
        <button className="btn btn-secondary clients-refresh-btn" id="clients-refresh-btn" type="button" onClick={() => loadClients(page, statusFilter)}>
          Refresh
        </button>
      </section>
      ...
      <section className="clients-toolbar" aria-label="Client list controls">
        ...
        <select id="client-status-filter" className="clients-select" aria-label="Filter clients by status" value={statusFilter} onChange={(e) => {
      ...
      <section className="clients-list-shell" aria-label="Client list">
        {loading ? (<div className="clients-skeleton" id="clients-skeleton" aria-live="polite" aria-label="Loading clients">
            <Skeleton variant="row" className="client-row-skeleton"/>
            <Skeleton variant="row" className="client-row-skeleton"/>
            <Skeleton variant="row" className="client-row-skeleton"/>
          </div>) : error ? (<div className="feed-error-state card-like-error" id="clients-error" role="alert">
            ...
            <button className="btn btn-secondary btn-retry-large" id="clients-error-retry" type="button" onClick={() => loadClients(page, statusFilter)}>
              Retry loading clients
            </button>
          </div>) : clients.length === 0 ? (<div className="empty-state" id="clients-empty">
          ...
          </div>) : (<div className="clients-list" id="clients-list" role="list">
            {clients.map((client) => (<article key={client.id} className="client-list-row" role="listitem">
                <a href={`/clients/${encodeURIComponent(client.id)}`} className="client-row-link" aria-label={`Open client ${client.name}`}>
      ...
      <nav className="clients-pagination" aria-label="Client list pagination">
        <button className="btn btn-secondary clients-page-btn" id="clients-prev-page" type="button" disabled={page <= 1 || loading} onClick={() => setPage((prev) => Math.max(1, prev - 1))}>
          Previous
        </button>
        <span className="clients-page-summary" id="clients-page-summary" aria-live="polite">
          Page {page} of {totalPages}
        </span>
        <button className="btn btn-secondary clients-page-btn" id="clients-next-page" type="button" disabled={page >= totalPages || loading} onClick={() => setPage((prev) => prev + 1)}>
          Next
        </button>
      </nav>
    </AppLayout>);
