...
import type { Client, ClientTimeline } from "../types/client.js";
...
export const ClientDetailPage: React.FC = () => {
  ...
  const loadClient = useCallback(async (id: string, signal?: AbortSignal) => {
    if (!id) return;
  ...
  const loadTimeline = useCallback(async (id: string, page: number, signal?: AbortSignal) => {
    if (!id) return;
    setTimelineLoading(true);
    setTimelineError(null);
    try {
      const params: { page: number; pageSize: number; signal?: AbortSignal } = {
        page,
        pageSize: 20,
      };
  ...
  return (
    <AppLayout
      ...
      breadcrumbTitle={client?.name || "Detail"}
      parentBreadcrumb={{ label: "Clients", href: "/clients.html" }}
      topbarActions={
        <button
          className="topbar-action-btn"
          id="client-detail-refresh-icon"
    ...
    >
      <section
        className="welcome-section client-detail-title-row"
        aria-label="Client detail heading"
      >
        <div>
        ...
        <button
          className="btn btn-secondary client-detail-refresh-btn"
          id="client-detail-refresh-btn"
          type="button"
          onClick={() => {
            loadClient(clientId);
            loadTimeline(clientId, timelinePage);
      ...
      <section className="client-detail-shell" aria-label="Client detail">
        {clientLoading ? (
          <div className="client-detail-skeleton" id="client-detail-skeleton" aria-live="polite">
          ...
          <div
            className="empty-state client-detail-not-found"
            id="client-detail-not-found"
            role="status"
          >
          ...
          <div className="feed-error-state card-like-error" id="client-detail-error" role="alert">
          ...
          <article className="client-detail-panel" id="client-detail-panel">
            {/* Hero Header */}
            <div className="client-detail-hero">
              <div
                className="client-avatar client-detail-avatar"
                id="client-detail-avatar"
                aria-hidden="true"
              >
              ...
              <span
                className={`client-status status-${(client.status || "unknown").toLowerCase()}`}
            ...
            <div className="client-detail-grid">
              <section className="client-detail-section" aria-labelledby="client-profile-heading">
                <h3
                  className="client-detail-section-title font-display"
                ...
                <dl className="client-detail-fields">
                  <div className="client-detail-field-row">
                    ...
                    <dd id="client-detail-email">
                      {client.email ? (
                        <a href={`mailto:${client.email}`} className="client-detail-value-link">
                          {client.email}
                        </a>
                      ) : (
                        "Not provided"
                  ...
                  <div className="client-detail-field-row">
                    ...
                    <dd id="client-detail-website">
                      {client.website ? (
                        <a
                          href={client.website}
                          className="client-detail-value-link"
                          target="_blank"
            ...
            <section
              className="client-detail-section client-timeline-section"
            ...
            >
              ...
              {timelineLoading ? (
                <div
                  className="client-timeline-skeleton"
                  id="client-timeline-skeleton"
                  aria-live="polite"
                >
                ...
                <div className="component-error" id="client-timeline-error" role="alert">
                  <span className="error-msg" id="client-timeline-error-msg">
                    {timelineError}
                  </span>
                  <button
                    className="btn-retry"
                    id="client-timeline-retry"
                    type="button"
                    onClick={() => loadTimeline(clientId, timelinePage)}
                  >
                    Retry
                  </button>
                </div>
              ) : !timeline || timeline.entries.length === 0 ? (
                <div className="empty-state timeline-empty" id="client-timeline-empty">
                ...
                <div
                  className="activity-timeline client-timeline-list"
                  id="client-timeline-list"
                  role="list"
                >
                  {timeline.entries.map((entry) => (
                    <article
                      key={entry.id}
                      className="timeline-item client-timeline-item"
                      role="listitem"
                      aria-label={`${entry.category} on ${formatDateTime(entry.timestamp)}`}
                    >
              ...
              {timeline && timeline.total > timeline.pageSize && (
                <nav
                  className="clients-pagination client-timeline-pagination"
                  aria-label="Client timeline pagination"
                >
                  <button
                    className="btn btn-secondary clients-page-btn"
                    id="client-timeline-prev"
                    type="button"
                  ...
                  <span
                    className="clients-page-summary"
                    id="client-timeline-page-summary"
                    aria-live="polite"
                  >
