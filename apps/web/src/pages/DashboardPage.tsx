...
import type { KpiMetric, OpportunityPulse, ActivityItem } from "../types/dashboard.js";
...
export const DashboardPage: React.FC = () => {
  ...
  // KPIs
  const [scannedKpi, setScannedKpi] = useState<KpiState>({
    data: null,
    loading: true,
    error: false,
  });
  const [matchesKpi, setMatchesKpi] = useState<KpiState>({
    data: null,
    loading: true,
    error: false,
  });
  const [proposalsKpi, setProposalsKpi] = useState<KpiState>({
    data: null,
    loading: true,
    error: false,
  });
  ...
  // Usage & Quota limits
  const [entitlements, setEntitlements] = useState<SubscriptionResponse | null>(null);
  ...
  // Load KPI
  const loadScanned = useCallback(async (signal?: AbortSignal) => {
  ...
  return (
    <AppLayout currentPath="/dashboard.html" breadcrumbTitle="Dashboard">
      {/* Welcome Header */}
      ...
      <section className="kpi-grid" aria-label="Key Performance Indicators" id="kpi-grid">
        ...
        <div className="metric-card" id="metric-scanned">
          ...
          <div className="metric-body">
            {scannedKpi.loading ? (
              <Skeleton variant="card" className="metric-skeleton" />
            ) : scannedKpi.error ? (
              <div className="component-error" id="kpi-scanned-error">
                <span className="error-msg">Failed to load</span>
                <button
                  className="btn-retry"
                  aria-label="Retry loading jobs scanned"
                  onClick={() => loadScanned()}
                >
                  Retry
                </button>
        ...
        <div className="metric-card" id="metric-matches">
          ...
          <div className="metric-body">
            {matchesKpi.loading ? (
              ...
              <div className="component-error" id="kpi-matches-error">
                <span className="error-msg">Failed to load</span>
                <button
                  className="btn-retry"
        ...
        <div className="metric-card" id="metric-proposals">
          ...
          <div className="metric-body">
            {proposalsKpi.loading ? (
              <Skeleton variant="card" className="metric-skeleton" />
            ) : proposalsKpi.error ? (
              <div className="component-error" id="kpi-proposals-error">
                <span className="error-msg">Failed to load</span>
                <button
                  className="btn-retry"
        ...
        <div className="pulse-card">
          <div className="pulse-left">
            ...
            <div className="pulse-details">
              ...
              {pulse.loading ? (
                <Skeleton variant="text" className="pulse-skeleton" id="pulse-skeleton" />
              ) : pulse.error ? (
                <div className="component-error" id="pulse-error">
                  <span className="error-msg">API connection missing</span>
                  <button
                    className="btn-retry"
                    aria-label="Retry pulse scan details"
                    onClick={() => loadPulse()}
                  >
                    Retry
                  </button>
                ...
                <p className="pulse-description" id="pulse-desc">
      ...
      <div className="dashboard-grid">
        ...
        <section className="feed-column" aria-label="Top Opportunities">
          ...
          {jobsLoading ? (
            <div className="skeleton-feed" id="skeleton-feed">
              <Skeleton variant="card" className="card-skeleton" />
              <Skeleton variant="card" className="card-skeleton" />
              <Skeleton variant="card" className="card-skeleton" />
            </div>
          ) : jobsError ? (
            <div className="feed-error-state card-like-error" id="opportunities-error">
            ...
            <div className="empty-state" id="opportunities-empty">
            ...
            <div className="opportunities-list" id="opportunities-list">
              {jobs.map((job) => {
                const hasScore = typeof job.score === "number" && job.score !== null;
                const isMatching = matchingJobId === job.id;

                return (
                  <div key={job.id} className="opportunity-card" id={`opp-card-${job.id}`}>
                    <div className="opp-card-header">
                      ...
                      {hasScore && (
                        <div className={`match-score-pill score-${getScoreRangeClass(job.score!)}`}>
                          {job.score}% match
                        </div>
                      )}
                    </div>
        ...
        <div className="widgets-column">
          ...
          <section className="widget-card" aria-label="Subscription and Usage Limits">
            ...
            <div className="widget-body">
              {entitlementsLoading ? (
                <Skeleton variant="card" className="usage-skeleton" id="usage-skeleton" />
              ) : entitlementsError ? (
                <div className="component-error" id="usage-error">
                ...
                <div className="usage-details-wrap" id="usage-details">
                        ...
                        style={{
                          width: `${
                            ...
                            entitlements.limits.aiProposals.value
                              ? Math.min(
                                  100,
                                  ((entitlements.usage?.aiProposals ?? 0) /
                                    entitlements.limits.aiProposals.value) *
                                    100,
                                )
                              : 100
          ...
          <section className="widget-card" aria-label="Recent Activity">
            ...
            <div className="widget-body">
              {activityLoading ? (
                <Skeleton variant="card" className="activity-skeleton" id="activity-skeleton" />
              ) : activityError ? (
                <div className="component-error" id="activity-error">
                  <span className="error-msg">Failed to retrieve activity logs</span>
                  <button
                    className="btn-retry"
                    id="btn-activity-retry"
                ...
                <div className="empty-state timeline-empty" id="activity-empty">
