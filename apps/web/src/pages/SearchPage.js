...
export const SearchPage = () => {
    ...
    useEffect(() => {
        window.__setSearchQuery = (val) => {
            setQuery(val);
            setPage(1);
        };
        return () => {
            delete window.__setSearchQuery;
        };
    }, []);
    useEffect(() => {
    ...
    const performSearch = useCallback(async (q, types, p, signal) => {
    ...
    useEffect(() => {
        ...
        const targetUrl = trimmed || selectedType || page > 1 ? `/search.html?${params.toString()}` : "/search.html";
    ...
    return (<AppLayout currentPath="/search.html" breadcrumbTitle="Search">
      <section className="search-page-container" aria-label="Global search">
        ...
        <div className="search-command-bar">
          <div className="search-input-wrapper">
            ...
            <input ref={inputRef} id="search-input" type="search" className="search-input" placeholder="Search clients, jobs, matches, timeline activity... (Ctrl+K)" value={query} onInput={(e) => {
            setQuery(e.target.value);
            setPage(1);
        }} onChange={(e) => {
            setQuery(e.target.value);
            setPage(1);
        }} onKeyDown={handleResultsKeyDown} autoFocus aria-label="Search across workspace"/>
            {query && (<button className="search-clear-btn" aria-label="Clear search input" onClick={handleClear}>
                ×
              </button>)}
          ...
          {/* Filter Type Pills */}
          <div className="search-filter-pills" role="tablist" aria-label="Search entity type filters">
            {[
            ...
            return (<button key={tab.value} className={`search-filter-pill ${active ? "active" : ""}`} role="tab" aria-selected={active} onClick={() => {
        ...
        <div className="search-results-wrapper">
          {loading ? (<div className="search-skeleton-feed" id="search-skeleton" aria-live="polite">
              <Skeleton variant="card" className="search-card-skeleton"/>
              <Skeleton variant="card" className="search-card-skeleton"/>
              <Skeleton variant="card" className="search-card-skeleton"/>
            </div>) : error ? (<div className="feed-error-state card-like-error" id="search-error" role="alert">
              ...
              <button className="btn btn-secondary btn-retry-large" id="search-retry-btn" type="button" onClick={() => performSearch(query, selectedType, page)}>
                Retry Search
              </button>
            </div>) : !query.trim() ? (<div className="empty-state search-empty-initial" id="search-initial">
            ...
            </div>) : results.length === 0 ? (<div className="empty-state" id="search-no-results">
              <div className="empty-icon-wrap">
                <svg className="icon" viewBox="0 0 24 24" width="40" height="40">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                </svg>
            ...
            </div>) : (<div className="search-results-list" id="search-results-list" role="list">
              {results.map((item, index) => {
                ...
                const scorePct = item.relevance?.score
                    ...
                    : null;
                return (<article key={`${item.resultType}-${item.entityId}-${index}`} className={`search-result-card ${isSelected ? "focused" : ""}`} role="listitem">
                    <a href={targetUrl} className="search-result-link">
                      <div className="search-result-header">
                        ...
                        {scorePct !== null && (<span className="search-relevance-score">{scorePct}% match</span>)}
                      </div>

                      <h4 className="search-result-title font-display">{item.display.title}</h4>

                      {item.display.subtitle && (<p className="search-result-subtitle">{item.display.subtitle}</p>)}

                      {item.display.snippet && (<p className="search-result-snippet">{item.display.snippet}</p>)}

                      {item.relevance?.matchedFields && item.relevance.matchedFields.length > 0 && (<div className="search-matched-fields">
                          <span className="matched-fields-label">Matched in:</span>
                          {item.relevance.matchedFields.map((f, i) => (<span key={i} className="field-tag">
                              {f}
                            </span>))}
                        </div>)}
                    </a>
          ...
          {/* Pagination */}
          {results.length > 0 && total > pageSize && (<nav className="clients-pagination search-pagination" aria-label="Search pagination">
              <button className="btn btn-secondary clients-page-btn" id="search-prev-page" type="button" disabled={page <= 1 || loading} onClick={() => setPage((p) => Math.max(1, p - 1))}>
                Previous
              </button>
              <span className="clients-page-summary" id="search-page-summary" aria-live="polite">
                Page {page} of {totalPages}
              </span>
              <button className="btn btn-secondary clients-page-btn" id="search-next-page" type="button" disabled={page >= totalPages || loading} onClick={() => setPage((p) => p + 1)}>
                Next
              </button>
            </nav>)}
        </div>
