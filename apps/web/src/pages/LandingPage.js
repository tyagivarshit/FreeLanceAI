...
const FAQ_ITEMS = [
    {
        ...
        answer: "FreelanceOS extracts key requirements, tech stacks, and budget constraints from job posts, then computes a multi-factor compatibility score against your skills and experience profile.",
    },
    {
        id: "faq-ans-2",
        question: "Does FreelanceOS work with Upwork and LinkedIn?",
        answer: "Yes. Our Manifest V3 Chrome Extension seamlessly connects your active browser sessions to capture job postings directly from Upwork and LinkedIn in real time.",
    },
    {
        id: "faq-ans-3",
        question: "What is included in the Pro 7-Day Free Trial?",
        answer: "You receive full access to all Pro tier features—including unlimited matches, 50 AI proposals, Client Brain insights, and extension sync—for 7 days without upfront commitment.",
    },
    {
        id: "faq-ans-4",
        question: "What does Client Brain do?",
        answer: "Client Brain synthesizes communication records, past project scopes, and notes to deliver actionable summaries, relationship health scores, and tactical next-step recommendations.",
    },
    {
        id: "faq-ans-5",
        question: "What are the AI proposal generation limits?",
        answer: "The Free Starter tier includes 5 AI proposals per month. The Pro tier provides 50 proposals per month, with custom high-volume allocations available on Enterprise.",
    },
];
export const LandingPage = () => {
    ...
    return (<PublicLayout>
      ...
      <section id="how-it-works" className="landing-section landing-how-it-works" aria-labelledby="how-it-works-title">
        <div className="section-container">
          <div className="section-header">
      ...
      <section id="faq" className="landing-section landing-faq" aria-labelledby="faq-title">
        <div className="section-container">
          ...
          <div className="faq-accordion" role="region" aria-label="FAQ Accordion">
            ...
            return (<div key={item.id} className="faq-item">
                  <button className="faq-question" aria-expanded={isExpanded} aria-controls={item.id} id={`faq-q-${idx + 1}`} onClick={() => toggleFaq(item.id)}>
                    <span>{item.question}</span>
                    <span className="faq-icon" aria-hidden="true">
                      {isExpanded ? "−" : "+"}
                    </span>
                  </button>
                  <div id={item.id} className={`faq-answer ${isExpanded ? "" : "hidden"}`} role="region" aria-labelledby={`faq-q-${idx + 1}`}>
                    <p>{item.answer}</p>
                  </div>
                </div>);
        })}
      ...
      {/* Final CTA Section */}
      <section id="final-cta" className="landing-section landing-final-cta" aria-labelledby="final-cta-title">
        <div className="section-container">
