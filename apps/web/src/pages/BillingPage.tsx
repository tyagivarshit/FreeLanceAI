...
import type { PlanItem, SubscriptionResponse, PriceInfo } from "../types/billing.js";
...
export const BillingPage: React.FC = () => {
  ...
  const getPriceForCurrency = (prices: PriceInfo[], curr: string): string => {
    const p = prices.find((item) => item.currency.toUpperCase() === curr.toUpperCase());
    if (p && p.formatted) return p.formatted;
    if (p) return `${p.currency} ${(p.amountMinor / 100).toFixed(2)}`;
    return "$0.00";
  };
  ...
  return (
    <AppLayout
      currentPath="/billing.html"
      breadcrumbTitle="Billing"
      topbarActions={
        <div className="billing-currency-selector-wrap">
    ...
    >
      ...
      {loading ? (
        <div className="billing-skeleton-container" id="billing-skeleton">
        ...
        <div className="feed-error-state card-like-error" id="billing-error" role="alert">
        ...
        <div className="billing-content-wrapper" id="billing-content">
            ...
            <div className="subscription-card-header">
                ...
                <div className="plan-badge-row">
                  ...
                  {subscription?.trialDaysRemaining !== null &&
                    subscription?.trialDaysRemaining !== undefined && (
                      <span className="badge badge-trial" id="sub-trial-badge">
              ...
              {subscription?.hasCustomer && (
                <Button
                  ...
                  aria-label="Manage billing details in Stripe Customer Portal"
                >
                  Manage Subscription ↗
                </Button>
              )}
            ...
            <div className="billing-plans-grid" id="billing-plans-list" role="list">
              {plans.map((plan) => {
                ...
                return (
                  <article
                    key={plan.planId}
                    className={`pricing-card ${isPopular ? "popular" : ""} ${isCurrent ? "current-tier" : ""}`}
                    role="listitem"
                    id={`plan-card-${plan.planId}`}
                  >
                    {isPopular && (
                      <div className="popular-badge">Recommended • 7-Day Free Trial</div>
                    )}
