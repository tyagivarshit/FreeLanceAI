...
export const MatchingPage = () => {
    ...
    const loadMatches = useCallback(async (p, s, minS, plat, signal) => {
        setLoading(true);
        setError(null);
        try {
            ...
            if (s)
                urlParams.set("status", s);
            if (minS)
                urlParams.set("minScore", minS);
            if (plat)
                urlParams.set("platform", plat);
    ...
    return (<AppLayout currentPath="/matching.html" breadcrumbTitle="Matching" topbarActions={<button className="topbar-action-btn" id="matching-refresh-icon" aria-label="Refresh matches" onClick={() => loadMatches(page, statusFilter, minScoreFilter, platformFilter)}>
      ...
      <section className="welcome-section matching-title-row" aria-label="Matching heading">
        ...
        <button className="btn btn-secondary matching-refresh-btn" id="matching-refresh-btn" type="button" onClick={() => loadMatches(page, statusFilter, minScoreFilter, platformFilter)}>
          Refresh
        </button>
      </section>
      ...
      <section className="matching-toolbar" aria-label="Job match filters">
        <div className="filter-group">
          ...
          <select id="matching-status-filter" className="matching-select" value={statusFilter} onChange={(e) => {
        ...
        <div className="filter-group">
          ...
          <select id="matching-score-filter" className="matching-select" value={minScoreFilter} onChange={(e) => {
        ...
        <div className="filter-group">
          ...
          <select id="matching-platform-filter" className="matching-select" value={platformFilter} onChange={(e) => {
      ...
      <section className="matching-list-shell" aria-label="Matches feed">
        {loading ? (<div className="matching-skeleton-feed" id="matching-skeleton" aria-live="polite">
            <Skeleton variant="card" className="matching-card-skeleton"/>
            <Skeleton variant="card" className="matching-card-skeleton"/>
            <Skeleton variant="card" className="matching-card-skeleton"/>
          </div>) : error ? (<div className="feed-error-state card-like-error" id="matching-error" role="alert">
            ...
            <button className="btn btn-secondary btn-retry-large" id="matching-error-retry" type="button" onClick={() => loadMatches(page, statusFilter, minScoreFilter, platformFilter)}>
              Retry loading matches
            </button>
          </div>) : matches.length === 0 ? (<div className="empty-state" id="matching-empty">
          ...
          </div>) : (<div className="matching-grid" id="matching-list" role="list">
            {matches.map((match) => {
                const scoreClass = getScoreRangeClass(match.score);
                return (<article key={match.id} className={`match-card status-${(match.status || "created").toLowerCase()}`} role="listitem" id={`match-card-${match.id}`}>
                  <div className="match-card-header">
                    <div className="match-card-meta">
                      <span className="platform-tag">{match.platform || "Upwork"}</span>
                      <span className={`status-badge status-${(match.status || "created").toLowerCase()}`}>
                  ...
                  {match.explanation && (<p className="match-card-explanation">{match.explanation}</p>)}

                  {match.strengths && match.strengths.length > 0 && (<div className="match-card-chips">
                      {match.strengths.slice(0, 3).map((s, idx) => (<span key={idx} className="match-chip chip-strength">
                          {s}
                        </span>))}
                    </div>)}

                  <div className="match-card-footer">
                    <Button variant="primary" size="sm" onClick={() => handleOpenDetail(match)} aria-label={`View match breakdown for ${match.jobTitle}`}>
                      View Explanation
                    </Button>
                    {match.canonicalUrl && (<a href={match.canonicalUrl} className="btn btn-secondary btn-sm" target="_blank" rel="noopener noreferrer">
                        Open Job ↗
                      </a>)}
                    <button className="btn-archive-icon" aria-label="Archive match" title="Archive match" disabled={match.status === "ARCHIVED"} onClick={() => handleArchive(match.id)}>
      ...
      <nav className="clients-pagination" aria-label="Matches pagination">
        <button className="btn btn-secondary clients-page-btn" id="matching-prev-page" type="button" disabled={page <= 1 || loading} onClick={() => setPage((p) => Math.max(1, p - 1))}>
          Previous
        </button>
        <span className="clients-page-summary" id="matching-page-summary" aria-live="polite">
          Page {page} of {totalPages}
        </span>
        <button className="btn btn-secondary clients-page-btn" id="matching-next-page" type="button" disabled={page >= totalPages || loading} onClick={() => setPage((p) => p + 1)}>
          Next
        </button>
      </nav>

      {/* Detail Slide-out Drawer */}
      <MatchDetailDrawer match={selectedMatch} isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} onArchive={handleArchive} archiving={archiving}/>
    </AppLayout>);
};
