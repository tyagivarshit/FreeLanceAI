...
export const Sidebar = ({ currentPath, isMobileOpen, onCloseMobile }) => {
    ...
    const navItems = [
        {
            ...
            label: "Dashboard",
            icon: (<svg className="icon" viewBox="0 0 24 24" width="20" height="20">
    ...
    const isCurrent = (aliases) => {
        return (aliases.some((a) => currentPath.startsWith(a)) ||
            (currentPath.startsWith("/clients/") && aliases.includes("/clients.html")));
    };
    return (<>
      <aside id="sidebar" className={`sidebar ${collapsed ? "collapsed" : ""} ${isMobileOpen ? "drawer-open" : ""}`} aria-label="Main Navigation">
        <div className="sidebar-header">
          <div className="logo" aria-hidden="true">
            F
          </div>
          <span className="brand-name font-display">FreelanceOS</span>
          <button id="sidebar-toggle" className="sidebar-toggle-btn" aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} onClick={() => setCollapsed(!collapsed)}>
        ...
        <nav className="sidebar-nav" aria-label="Primary nav">
          {navItems.map((item) => {
            ...
            return (<a key={item.path} href={item.path} className={`nav-item ${active ? "active" : ""}`} aria-current={active ? "page" : undefined} onClick={onCloseMobile}>
                {item.icon}
        ...
        <div className="sidebar-footer">
          <button id="sidebar-logout-btn" className="sidebar-logout-btn" aria-label="Log out" onClick={() => logout(false)}>
      ...
      <div id="drawer-overlay" className={`drawer-overlay ${isMobileOpen ? "visible" : ""}`} aria-hidden={!isMobileOpen} onClick={onCloseMobile}/>
    </>);
};
