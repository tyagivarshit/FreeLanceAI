...
export const MatchDetailDrawer = ({ match, isOpen, onClose, onArchive, archiving = false, }) => {
    useEffect(() => {
    ...
    const semanticPct = typeof breakdown.semantic === "number" ? Math.round(breakdown.semantic * 100) : 0;
    return (<div id="matching-detail-modal" className="matching-detail-modal" role="dialog" aria-modal="true" aria-labelledby="detail-title">
      <div id="matching-detail-backdrop" className="matching-detail-backdrop" aria-hidden="true" onClick={onClose}/>
      <div className="matching-detail-drawer" id="matching-detail-drawer">
        <header className="detail-header">
          <div className="detail-header-top">
            <div className="detail-tags">
              ...
              <span className={`status-badge status-${(match.status || "created").toLowerCase()}`} id="detail-status-badge">
                {match.status || "EVALUATED"}
              </span>
              {match.cacheState && (<span className="cache-badge" id="detail-cache-badge">
                  {match.cacheState === "CACHED" ? "Cached" : "Fresh"}
                </span>)}
            </div>
            <button className="detail-close-btn" id="matching-detail-close-btn" aria-label="Close match details" onClick={onClose}>
            ...
            <div className="breakdown-grid" id="detail-score-breakdown">
              <div className="breakdown-item">
                ...
                <div className="progress-bar-wrap">
                  <div className="progress-bar" id="breakdown-skills-bar" style={{ width: `${skillsPct}%` }}/>
                </div>
              </div>
              ...
              <div className="breakdown-item">
                ...
                <div className="progress-bar-wrap">
                  <div className="progress-bar" id="breakdown-semantic-bar" style={{ width: `${semanticPct}%` }}/>
                </div>
              </div>
              ...
              <div className="breakdown-item">
                <div className="breakdown-label-row">
                  ...
                  <span className={`breakdown-status-tag tag-${(breakdown.experience || "unknown").toLowerCase()}`} id="breakdown-exp-tag">
              ...
              <div className="breakdown-item">
                <div className="breakdown-label-row">
                  ...
                  <span className={`breakdown-status-tag tag-${(breakdown.budget || "unknown").toLowerCase()}`} id="breakdown-budget-tag">
              ...
              <div className="breakdown-item">
                <div className="breakdown-label-row">
                  ...
                  <span className={`breakdown-status-tag tag-${(breakdown.jobType || "unknown").toLowerCase()}`} id="breakdown-jobtype-tag">
              ...
              <div className="breakdown-item">
                <div className="breakdown-label-row">
                  ...
                  <span className={`breakdown-status-tag tag-${(breakdown.location || "unknown").toLowerCase()}`} id="breakdown-location-tag">
          ...
          <section className="detail-section" aria-labelledby="heading-explanation">
            ...
            <div className="factor-group" id="factor-strengths-group">
              ...
              <div className="factor-tags" id="detail-strengths-list">
                {match.strengths && match.strengths.length > 0 ? (match.strengths.map((s, i) => (<span key={i} className="factor-chip factor-chip-strength">
                      {s}
                    </span>))) : (<span className="factor-none">None identified</span>)}
              </div>
            </div>
            ...
            <div className="factor-group" id="factor-gaps-group">
              ...
              <div className="factor-tags" id="detail-gaps-list">
                {match.gaps && match.gaps.length > 0 ? (match.gaps.map((g, i) => (<span key={i} className="factor-chip factor-chip-gap">
                      {g}
                    </span>))) : (<span className="factor-none">None identified</span>)}
              </div>
            </div>
        ...
        <footer className="detail-footer" id="detail-action-bar">
          {match.canonicalUrl && (<a href={match.canonicalUrl} className="btn btn-primary detail-platform-btn" id="detail-platform-link" target="_blank" rel="noopener noreferrer">
              Open on Platform ↗
            </a>)}
          {onArchive && (<Button variant="secondary" id="detail-archive-btn" className="detail-archive-btn" disabled={match.status === "ARCHIVED" || archiving} loading={archiving} onClick={() => onArchive(match.id)}>
