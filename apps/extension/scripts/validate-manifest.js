...
if (manifest.version !== pkg.version) {
    logFail(`Version mismatch! manifest.json version is ${manifest.version} but package.json version is ${pkg.version}`);
}
    ...
    if (envMatch) {
        ...
        if (activeEnv === "production") {
            ...
            if (urlMatch && urlMatch[1]?.includes("localhost")) {
                logFail("Build aborted: production environment config points to local development localhost.");
            }
        }
    }
}
...
const scanFileForSecrets = (filePath) => {
    ...
    for (const pattern of secretPatterns) {
        if (pattern.test(content)) {
            logFail(`Secret leakage warning: suspicious credentials found in ${path.relative(extensionRoot, filePath)}`);
        }
    }
};
