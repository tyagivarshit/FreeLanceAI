...
let fetchMockResponse = () => Promise.resolve({ ok: true, json: () => Promise.resolve([]) });
...
import { SUPPORTED_PROTOCOL_VERSION, validateEnvelope, getObjectDepth, } from "../src/messaging/schema.js";
let DashboardUI;
function dispatchMessage(msg, sender = { contextType: "EXTENSION_UI" }) {
...
describe("Chapter 9H — Final Extension Integration & Certification", () => {
    ...
    beforeEach(() => {
        ...
        fetchMockResponse = () => Promise.resolve({
            ok: true,
    ...
    describe("1. Manifest Certification", () => {
        ...
        test("Verifies CSP policies block remote code execution and unsafe-eval", () => {
            ...
            if (csp) {
                ...
                const scriptSrc = csp.split(";").find((t) => t.trim().startsWith("script-src"));
                if (scriptSrc) {
                    const sources = scriptSrc.trim().split(/\s+/).slice(1);
                    for (const s of sources) {
                        assert.ok(s === "'self'" || s === "'none'" || s === "'wasm-unsafe-eval'", `Script source '${s}' violates MV3 security restrictions`);
                    }
                }
            }
        });
    ...
    describe("3. Platform Adapter Certification", () => {
        test("Platform Registry correctly manages and isolates adapters", () => {
            ...
            registry.register(faultyAdapter);
            // Resolution should skip faulty and proceed normally
            const upworkCtx = createPlatformContext("https://www.upwork.com/jobs/~01abc1234567890def", 1, 0);
            const resolved = registry.resolve(upworkCtx);
            assert.strictEqual(resolved.identity, "UPWORK", "Registry successfully isolated the faulty canHandle check");
        });
    });
    // ===========================================================
    ...
    describe("4 & 5. Upwork & LinkedIn Adapters Integration", () => {
        test("Upwork URL handles and supports standard patterns", () => {
            ...
            assert.ok(adapter.canHandle(createPlatformContext("https://www.upwork.com/jobs/~01abc1234567890def", 1, 0)));
            assert.ok(!adapter.canHandle(createPlatformContext("https://www.upwork.com/nx/find-work/", 1, 0)));
        });
        test("LinkedIn URL handles and supports standard patterns", () => {
            const adapter = new LinkedInAdapter();
            assert.ok(adapter.canHandle(createPlatformContext("https://www.linkedin.com/jobs/view/123456789", 1, 0)));
    ...
    describe("6. Dashboard UI Integration", () => {
        test("Renders dashboard connection badge depending on state message", async () => {
            ...
            assert.ok(textEl.textContent.includes("Offline"), "Dashboard connection badge was correctly set to Offline state");
            const retryBtn = mockElements.get("job-retry-match-btn");
            assert.strictEqual(retryBtn.disabled, true, "Retry evaluation button must be disabled when offline");
        });
    });
    // ===========================================================
    ...
    describe("7. Offline Integration", () => {
        test("Service Worker correctly transitions state and deduplicates", async () => {
            ...
            await storage.saveSnapshot("dashboard-jobs", [{ id: "j1", title: "Cache Entry" }], "api/jobs");
            ...
            fetchMockResponse = () => fetchPromise.then(() => ({
                ok: true,
    ...
    describe("8. Cross-Chapter Message Flow Simulation", () => {
        test("End-to-end routing simulation: UI options page -> Service Worker -> DB Snapshot", async () => {
            ...
            assert.strictEqual(ui.jobs[0].title, "E2E Verified Job", "Cross-chapter UI-to-SW message dispatch flow verified");
        });
    });
    // ===========================================================
    ...
    describe("9. Security Certification", () => {
            ...
            const checkDirectory = (dir) => {
                ...
                for (const file of files) {
                    ...
                    if (fs.statSync(absPath).isDirectory()) {
                        ...
                        assert.ok(!content.includes("eval("), `eval() is strictly forbidden inside extension code: ${absPath}`);
                        assert.ok(!content.includes("new Function("), `new Function() is strictly forbidden: ${absPath}`);
                    }
                }
            };
        ...
        test("Sanitization: sanitizes passwords and access tokens recursively before storage", () => {
            ...
            const sanitized = sanitizePrivateData(payload);
    ...
    describe("10. Service Worker Lifecycle Certification", () => {
        test("Checks request timeout cleanup and locks recovery", async () => {
            ...
            assert.strictEqual(res.code, "REQUEST_TIMEOUT", "SW dispatcher handles and rejects timeouts correctly");
        });
    });
    // ===========================================================
    ...
    describe("12. Data Fabrication Audits", () => {
        test("Verifies that empty database snapshots present honest results rather than dummy fallbacks", async () => {
            ...
            await ui.initialize();
            assert.strictEqual(ui.jobs.length, 0, "UI does not generate fake data when empty response is returned");
        });
    });
    // ===========================================================
    ...
    describe("13. Boundary Certification", () => {
        test("Inspects dependencies to ensure no cross-chapter domain leakage exists", () => {
            ...
            assert.ok(!registryContent.includes("@freelanceos/core"), "Platform adapter must not leak into core domain models");
            assert.ok(!registryContent.includes("postgres"), "Extension must remain decoupled from production repositories");
        });
    });
});
