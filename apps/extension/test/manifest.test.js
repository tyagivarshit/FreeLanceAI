...
describe("Chapter 9A — Extension Manifest V3 & Security Foundation", () => {
    ...
    test("Required metadata name, description, and version must be present", () => {
        ...
        assert.ok(manifest.description && manifest.description.trim() !== "", "Description is required");
    ...
    test("Service worker must be declared and the referenced file must exist", () => {
        const raw = fs.readFileSync(manifestPath, "utf-8");
        const manifest = JSON.parse(raw);
        assert.ok(manifest.background?.service_worker, "Service worker background script must be declared");
        ...
        assert.ok(fs.existsSync(swPath) || fs.existsSync(tsSwPath), `Service worker file not found at: ${manifest.background.service_worker}`);
    });
    ...
    test("Content scripts must be declared and files must exist in source or dist", () => {
        ...
        assert.ok(manifest.content_scripts.length > 0, "At least one content script should be declared");
        for (const cs of manifest.content_scripts) {
            assert.ok(cs.matches && cs.matches.length > 0, "Content script matches rule must be declared");
            if (cs.js) {
                for (const jsFile of cs.js) {
                    const jsPath = path.join(extensionRoot, jsFile);
                    const tsPath = path.join(extensionRoot, jsFile.replace("dist/", "").replace(/\.js$/, ".ts"));
                    assert.ok(fs.existsSync(jsPath) || fs.existsSync(tsPath), `Content script file not found: ${jsFile}`);
                }
            }
        }
    });
            ...
            for (const token of tokens) {
                if (token.startsWith("script-src")) {
                    ...
                    for (const src of sources) {
                        ...
                        assert.ok(src === "'self'" || src === "'none'" || src === "'wasm-unsafe-eval'", `Script source '${src}' violates remote code isolation policies.`);
                    }
                }
            }
        }
    ...
    test("Production config must not target local development api urls", () => {
        ...
        if (envMatch && envMatch[1] === "production") {
            ...
            assert.ok(!productionUrl.includes("localhost"), "Production apiUrl must not target localhost");
        }
    });
    ...
    test("Manifest version matches workspace package version", () => {
        ...
        assert.strictEqual(manifest.version, pkg.version, "Manifest and Package versions must be aligned");
    });
    ...
    test("No secrets or credentials should be present in configuration or source code", () => {
        ...
        for (const kw of secretsKeyworkds) {
            ...
            if (pattern.test(content)) {
                ...
                assert.fail(`Possible secret leakage: credential keyword '${kw}' matches suspicious value signature in config.ts`);
            }
        }
    });
});
