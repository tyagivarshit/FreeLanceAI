...
import { getObjectDepth, SUPPORTED_PROTOCOL_VERSION, validateEnvelope, } from "../src/messaging/schema.js";
describe("Chapter 9B — Extension Messaging Protocol & Security", () => {
    ...
    describe("1. Envelope Validation", () => {
        test("Accepts a valid envelope format", async () => {
            ...
            const res = (await dispatcher.dispatch(msg, { contextType: "CONTENT_SCRIPT" }));
    ...
    describe("3. Sender & Receiver Validation", () => {
        ...
        test("Rejects Chrome Sender URLs outside whitelist", async () => {
            ...
            const res = (await dispatcher.dispatch(msg, maliciousSender));
            assert.strictEqual(res.code, "HANDLER_ERROR");
    ...
    describe("4. Payload Validation & Limits", () => {
        ...
        test("Enforces specific schema rules for JOB_DETECTED type", async () => {
            ...
            const res = (await dispatcher.dispatch(badMsg, { contextType: "CONTENT_SCRIPT" }));
            assert.strictEqual(res.code, "HANDLER_ERROR");
    ...
    describe("5. Client, Timeout & Correlation", () => {
        ...
        test("Dispatcher enforces handler execution timeouts", async () => {
            ...
            const res = (await tempDispatcher.dispatch(msg, { contextType: "CONTENT_SCRIPT" }));
    ...
    describe("6. Security & Sanitization", () => {
        test("Dispatcher converts errors and sanitizes credentials and local filesystem paths", async () => {
            ...
            tempDispatcher.registerHandler("PING", async () => {
                throw new Error("Cannot connect to server. Credentials: password=SuperSecretToken123. Path: D:\\secrets\\key.pem");
            });
            const msg = {
                protocolVersion: SUPPORTED_PROTOCOL_VERSION,
    ...
    describe("7. Duplicate & Replay safety", () => {
        test("Dispatcher rejects duplicate request messageId", async () => {
            ...
            // First run succeeds
            const first = (await dispatcher.dispatch(msg, { contextType: "CONTENT_SCRIPT" }));
            assert.strictEqual(first.type, "PING_RESPONSE");
            // Second run with same messageId fails as duplicate
            const second = (await dispatcher.dispatch(msg, { contextType: "CONTENT_SCRIPT" }));
    ...
    describe("AUTHORIZE_EXTENSION Envelope & Schema Validations", () => {
        test("AUTHORIZE_EXTENSION envelope is accepted", async () => {
            let receivedPayload = null;
            dispatcher.registerHandler("AUTHORIZE_EXTENSION", async (payload) => {
            ...
            const msg = {
                ...
                correlationId: "msg-auth-1",
                type: "AUTHORIZE_EXTENSION",
                timestamp: Date.now(),
                payload: {}
            };
            const res = await dispatcher.dispatch(msg, { contextType: "EXTENSION_UI" });
            assert.strictEqual(res.type, "AUTHORIZE_EXTENSION_RESPONSE");
            assert.strictEqual(res.payload.success, true);
        });

        test("malformed/missing envelope is still rejected", async () => {
            const res = await dispatcher.dispatch({ type: "AUTHORIZE_EXTENSION" }, { contextType: "EXTENSION_UI" });
            assert.strictEqual(res.code, "HANDLER_ERROR");
            assert.ok(res.message.includes("Missing protocolVersion"));
        });
        
        test("unknown payload fields/types are still rejected according to existing conventions", async () => {
            const msg = {
                protocolVersion: SUPPORTED_PROTOCOL_VERSION,
                messageId: "msg-auth-fail",
                correlationId: "msg-auth-fail",
                type: "AUTHORIZE_EXTENSION",
                timestamp: Date.now(),
                payload: { unexpected: "data" } // Empty payload required
            };
            const res = await dispatcher.dispatch(msg, { contextType: "EXTENSION_UI" });
            assert.strictEqual(res.code, "HANDLER_ERROR");
            assert.ok(res.message.includes("AUTHORIZE_EXTENSION expects an empty payload"));
        });
    });
});
