...
describe("Phase 12D — Chrome Web Store Readiness Test Suite", () => {
    ...
    describe("1. Manifest V3 & Store Metadata", () => {
        ...
        test("Manifest contains valid name, description, and semantic version", () => {
            ...
            assert.ok(manifest.description && typeof manifest.description === "string", "description is required");
    ...
    describe("2. Permission Least-Privilege & Boundary Enforcement", () => {
        test("Permissions are strictly limited to the minimal approved list (storage only)", () => {
            ...
            for (const perm of requestedPermissions) {
                assert.ok(approvedPermissions.includes(perm), `Unauthorized permission requested in manifest: ${perm}`);
            }
        });
        test("Prohibited high-risk browser permissions are completely excluded", () => {
            ...
            for (const prohibited of prohibitedPermissions) {
                assert.strictEqual(requestedPermissions.includes(prohibited), false, `High-risk permission '${prohibited}' must not be declared`);
            }
        });
        test("Host permissions are strictly restricted to Upwork, LinkedIn, and backend API", () => {
            const requestedHosts = manifest.host_permissions || [];
            assert.ok(requestedHosts.length > 0, "Host permissions must be declared for platform adapters");
            for (const host of requestedHosts) {
                ...
                const isAllowedDomain = host.includes("upwork.com") ||
    ...
    describe("3. CSP, Remote Code & Code Safety", () => {
        test("CSP enforces script-src 'self' and object-src 'self' without unsafe-eval", () => {
            ...
            assert.strictEqual(csp.includes("unsafe-inline"), false, "CSP must not allow 'unsafe-inline'");
            const tokens = csp.split(";").map((t) => t.trim());
            for (const token of tokens) {
                if (token.startsWith("script-src")) {
                    const sources = token.split(/\s+/).slice(1);
                    for (const src of sources) {
                        assert.ok(src === "'self'" || src === "'none'" || src === "'wasm-unsafe-eval'", `Disallowed script-src token: ${src}`);
                    }
                }
            }
        });
        test("Source code contains zero eval, new Function, or remote script injection", () => {
            ...
            for (const file of files) {
                ...
                if (fs.statSync(fullPath).isFile() && fullPath.endsWith(".ts")) {
                    ...
                    assert.strictEqual(/\beval\s*\(/.test(content), false, `Prohibited eval() call found in ${file}`);
                    assert.strictEqual(/\bnew\s+Function\s*\(/.test(content), false, `Prohibited new Function() found in ${file}`);
                    assert.strictEqual(/document\.write\s*\(/.test(content), false, `Prohibited document.write() found in ${file}`);
                    assert.strictEqual(/https?:\/\/.*\.js/.test(content), false, `Prohibited remote JS reference found in ${file}`);
                }
            }
        });
        test("Source code contains zero secret keys or credentials", () => {
            ...
            for (const file of files) {
                ...
                if (fs.statSync(fullPath).isFile() && fullPath.endsWith(".ts")) {
                    ...
                    assert.strictEqual(/password\s*=\s*["'][^"']{5,}["']/i.test(content), false, `Hardcoded password in ${file}`);
                    assert.strictEqual(/jwt_secret\s*=\s*["'][^"']{5,}["']/i.test(content), false, `JWT secret in ${file}`);
                }
            }
        });
    });
    ...
    describe("4. Privacy Boundary & Credential Sanitization", () => {
        test("sanitizePrivateData strips tokens, passwords, cookies, and auth headers", () => {
            ...
            const sanitized = sanitizePrivateData(rawPayload);
            ...
            assert.strictEqual(sanitized.nested.AccessToken, undefined);
            assert.strictEqual(sanitized.nested.Cookie, undefined);
        });
    });
    // =========================================================================
    ...
    describe("5. Store Documentation & Policy Files", () => {
        ...
        test("docs/chrome-store-permission-justification.md exists and contains reviewer justifications", () => {
            ...
            assert.ok(fs.existsSync(justPath), "docs/chrome-store-permission-justification.md must exist");
    ...
    describe("6. Build & Packaging Integrity", () => {
        ...
        test("All manifest referenced files exist in repository", () => {
            ...
            if (manifest.background?.service_worker) {
                ...
                const tsSwPath = path.join(extensionRoot, manifest.background.service_worker.replace("dist/", "").replace(/\.js$/, ".ts"));
