...
export class UpworkAdapter {
    ...
    canHandle(context) {
        ...
        const isSupportedPath = pathname.includes("/jobs/") ||
    ...
    async identify(context) {
        if (!this.canHandle(context)) {
            throw new Error(`UNSUPPORTED_PLATFORM: URL ${context.url} is not supported by UpworkAdapter.`);
        }
     ...
     */
    async delegateToContentScript(context, jobId, signal) {
        return new Promise((resolve) => {
            ...
            if (typeof chrome !== "undefined" && chrome.tabs?.sendMessage) {
                chrome.tabs.sendMessage(context.tabId, { type: "UPWORK_EXTRACT_DOM", context }, (response) => {
                    if (signal) {
     ...
     */
    async scrapeDOM(context, jobId, signal) {
        ...
        if (typeof window !== "undefined" && window.location?.href) {
            ...
            try {
                const currentId = (await this.identify({
                    ...context,
                    url: currentUrl,
     ...
     */
    async waitForDOMReady(signal, timeoutMs = 2000) {
