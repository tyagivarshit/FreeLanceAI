...
export class LinkedInAdapter {
    ...
    canHandle(context) {
        ...
        const isUnsupportedPage = pathname.startsWith("/feed") ||
        ...
        const isSupportedPath = isJobView ||
            hasCurrentJobId ||
    ...
    async identify(context) {
        if (!this.canHandle(context)) {
            throw new Error(`UNSUPPORTED_PLATFORM: URL ${context.url} is not supported by LinkedInAdapter.`);
        }
    ...
    async delegateToContentScript(context, jobId, signal) {
        return new Promise((resolve) => {
            ...
            if (typeof chrome !== "undefined" && chrome.tabs?.sendMessage) {
                chrome.tabs.sendMessage(context.tabId, { type: "LINKEDIN_EXTRACT_DOM", context }, (response) => {
                    if (signal) {
