...
export class MessageDispatcher {
    ...
    maxLoggedMessageIds = 5000;
    constructor(receiverType) {
    ...
    async dispatch(msg, chromeSender) {
        ...
        try {
            ...
            if (!definition.allowedSenders.includes(senderContext)) {
                throw new Error(`Security violation: unauthorized sender context '${senderContext}' for message type '${messageType}'.`);
            }
            if (!definition.allowedReceivers.includes(this.receiverType)) {
                throw new Error(`Security violation: unauthorized receiver context '${this.receiverType}' for message type '${messageType}'.`);
            }
            ...
            if (definition.category === "REQUEST") {
                ...
                responsePayload = await this.executeWithTimeout(handler(envelope.payload, chromeSender), timeoutMs, messageId);
            }
            else {
                // Event category
    ...
    determineSenderContext(chromeSender) {
        if (!chromeSender) {
            // Stub/Test fallback
        ...
        if (chromeSender.id && chromeSender.url) {
            ...
            if (chromeSender.tab) {
                ...
                const url = chromeSender.url || chromeSender.tab.url || "";
    ...
    async executeWithTimeout(promise, timeoutMs, msgId) {
        let timer;
    ...
    sanitizeError(raw) {
        ...
        clean = clean.replace(/(?:password|token|key|secret|pass|cred)(?:\s*[:=]\s*["']?)([A-Za-z0-9_-]{4,})/gi, "[REDACTED]");
        // Redact file paths
    ...
    logObservability(log) {
        // Redact sensitive details, output structured observability log
        console.log(`[Observability] Event: ${log.eventName} | Type: ${log.messageType} | ID: ${log.messageId} | Outcome: ${log.outcome} | Code: ${log.errorCode || "NONE"}`);
    }
    generateUuid() {
