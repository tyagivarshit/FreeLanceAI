 ...
 */
function calculateReconnectDelay(times) {
...
redis.on("error", (error) => {
    console.error("[Redis Error] Socket client error encountered:", error instanceof Error ? error.message : error);
});
redis.on("close", () => {
...
export async function verifyRedisConnection() {
    try {
        ...
        throw new Error(`[Redis Init Error] Redis connection could not be established: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
}
/**
...
export async function closeRedisConnection() {
    ...
    try {
        ...
        console.error("[Redis Error] Error occurred during client closure:", error instanceof Error ? error.message : error);
