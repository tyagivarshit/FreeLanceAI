...
export class UpworkPlatformAdapter extends BasePlatformAdapter {
  ...
  private parseSemanticFallback(): IngestedJobPayload {
    ...
    // Upwork descriptions are usually in a specific section, but we use a broad semantic selector
...
`;
fs.writeFileSync('apps/extension/src/platform/upwork/parser.ts', ts);
