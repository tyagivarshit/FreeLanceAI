...
export class RedisMemoryCacheService {
   ...
   */
  public async addMessage(sessionId: string, messagePayload: string, maxMessages: number): Promise<void> {
   ...
   */
  public async getMessages(sessionId: string): Promise<string[]> {
    const key = `memory:session:${sessionId}`;
