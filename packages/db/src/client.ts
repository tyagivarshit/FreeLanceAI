...
export async function verifyConnection(): Promise<void> {
  ...
  while (attempts > 0) {
    try {
      ...
      attempts--;
      console.warn(
        `[Database] Connection attempt failed. Remaining attempts: ${attempts}. Error:`,
        error instanceof Error ? error.message : error,
      );
      if (attempts === 0) {
        throw new Error(
          `[Database Init Error] Database connection could not be established: ${
...
export async function closeDatabaseConnection(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[Database Error] Error occurred during database pool closure:",
      error instanceof Error ? error.message : error,
    );
    throw error;
  }
