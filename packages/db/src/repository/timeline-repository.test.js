...
describe("PostgresTimelineRepository Unit Tests", () => {
    ...
    beforeEach(() => {
        ...
        db.select = function (fields) {
            ...
            const builder = {
                ...
                where: () => builder,
                orderBy: () => builder,
                limit: (_limitVal) => builder,
                offset: (_offsetVal) => builder,
                then: (onfulfilled) => {
                    if (isCount) {
    ...
    test("1. Saves client timeline and its entries correctly inside a transaction", async () => {
        ...
        const timeline = ClientTimeline.create("9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6e", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6f", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6f");
    ...
    test("2. Retrieves ClientTimeline correctly with chronological entries", async () => {
        ...
        const result = await repo.findById("9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6f");
        assert.ok(result);
