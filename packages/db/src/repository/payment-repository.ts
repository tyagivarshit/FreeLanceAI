...
import { Payment, Money, PaymentAggregateStore, PaymentPersistenceContract } from "@freelanceos/core";

export class PostgresPaymentRepository implements PaymentAggregateStore, PaymentPersistenceContract {
  ...
  public async findById(paymentId: string, tenantId: string): Promise<Payment | null> {
    const rows = await db
      .select()
      .from(payments)
  ...
  public async findByReference(paymentReference: string, tenantId: string): Promise<Payment | null> {
    const rows = await db
      .select()
      .from(payments)
  ...
  public async checkUniqueIntent(_ownerId: string, _paymentReference: string, _paymentId?: string): Promise<boolean> {
