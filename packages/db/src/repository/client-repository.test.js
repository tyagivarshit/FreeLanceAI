...
import { deleteTestUsers, ensureMigrationsApplied, isPostgresAvailable, listMigrationFiles, truncateClientDomainTables, } from "./postgres-integration-helper.js";
...
describe("PostgresClientRepository Unit Tests", () => {
    ...
    test("1. Saves and serializes Client aggregate correctly", async () => {
        ...
        const client = Client.create("9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d", {
            name: "Acme Corp",
    ...
    test("2. Retrieves and maps Client aggregate in findById", async () => {
        ...
        const result = await repo.findById("9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d", "8b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d");
        assert.ok(result);
...
describe("PostgresClientRepository PostgreSQL Integration Tests", () => {
    ...
    test("1. applies client migration and exposes the clients table", async (t) => {
        ...
        const tableResult = await pool.query("SELECT to_regclass('public.clients')");
    ...
    test("4. bounds pagination with deterministic ordering", async (t) => {
        ...
        assert.deepStrictEqual(page.items.map((item) => item.profile.name), ["Gamma Client", "Beta Client"]);
    ...
    test("5. enforces concurrent duplicate protection at the database layer", async (t) => {
        ...
        const concurrentA = Client.create("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", tenantA, tenantA, { name: "Concurrent One" }, undefined, { firstName: "Same", lastName: "Email", email: "race@acme.com" });
        const concurrentB = Client.create("bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", tenantA, tenantA, { name: "Concurrent Two" }, undefined, { firstName: "Same", lastName: "Email", email: "RACE@ACME.COM" });
    ...
    test("6. verifies job, match, and timeline relationships", async (t) => {
        ...
        const repo = new PostgresClientRepository();
        await repo.create(Client.create(clientA, tenantA, tenantA, { name: "Relationship Client" }, undefined, {
            firstName: "Rhea",
            lastName: "Link",
      ...
      `, [matchId, tenantA]);
        ...
        await assert.rejects(() => pool.query(`
          ...
          `, [tenantB, clientA]), /foreign key|violates/);
    ...
    test("7. rejects malformed client data and excludes secret columns", async (t) => {
        ...
        assert.throws(() => Client.create(clientA, tenantA, tenantA, { name: "A" }, undefined, {
            firstName: "Bad",
            lastName: "Email",
            email: "not-an-email",
    ...
    test("8. performs bounded client search with deterministic ordering and tenant isolation", async (t) => {
        ...
        const repo = new PostgresClientRepository();
        const client1 = Client.create("12345678-1234-4234-8234-123456789001", tenantA, tenantA, { name: "Searchable Alpha Inc", website: "https://alpha.com" }, undefined, { firstName: "Alice", lastName: "Smith", email: "alice@alpha.com" });
        const client2 = Client.create("12345678-1234-4234-8234-123456789002", tenantA, tenantA, { name: "Alpha Technologies LLC", website: "https://alphatech.com" }, undefined, { firstName: "Aaron", lastName: "Adams", email: "aaron@alphatech.com" });
        const foreignClient = Client.create("12345678-1234-4234-8234-123456789003", tenantB, tenantB, { name: "Foreign Alpha Corp", website: "https://foreign-alpha.com" }, undefined, { firstName: "Arthur", lastName: "Foreign", email: "arthur@foreign.com" });
