...
import { Project, ProjectMetadata, ProjectVisibility, ProjectAggregateStore, ProjectPersistenceContract } from "@freelanceos/core";

export class PostgresProjectRepository implements ProjectAggregateStore, ProjectPersistenceContract {
  ...
  public async findById(projectId: string, tenantId: string): Promise<Project | null> {
    const rows = await db
      .select()
      .from(projects)
  ...
  public async findByReference(projectReference: string, tenantId: string): Promise<Project | null> {
    const rows = await db
      .select()
      .from(projects)
  ...
  public async checkUniqueReference(_tenantId: string, _projectReference: string, _projectId?: string): Promise<boolean> {
