...
import { JobImport, JobSource, JobExternalIdentity, JobImportProvenance, JobRawPayload, JobImportFingerprint, JobImportSnapshot, JobSearchEngine, DEFAULT_SEARCH_PAGE, DEFAULT_SEARCH_PAGE_SIZE, MAX_SEARCH_PAGE_SIZE, } from "@freelanceos/core";
export class PostgresJobsRepository {
    async save(jobImport) {
        const values = {
            id: jobImport.id,
    ...
    async findByExternalIdentity(tenantId, source, externalJobId) {
        const rows = await db
            .select()
            .from(jobImports)
            .where(and(eq(jobImports.tenantId, tenantId), eq(jobImports.source, source), eq(jobImports.externalJobId, externalJobId)))
            .limit(1);
        if (rows.length === 0) {
            return null;
        }
    ...
    async findByTenant(tenantId, options) {
        ...
        if (options.status) {
            conditions.push(eq(jobImports.status, options.status));
        }
    ...
    async searchJobs(queryText, scope, page = DEFAULT_SEARCH_PAGE, pageSize = DEFAULT_SEARCH_PAGE_SIZE) {
        ...
        const scopeCondition = and(eq(jobImports.ownerId, scope.ownerId), eq(jobImports.tenantId, scope.tenantId));
        const searchCondition = or(sql `lower(${jobImports.rawPayload}->>'title') LIKE ${searchPattern}`, sql `lower(${jobImports.rawPayload}->>'description') LIKE ${searchPattern}`, sql `lower(${jobImports.source}) LIKE ${searchPattern}`, sql `lower(${jobImports.externalJobId}) LIKE ${searchPattern}`, sql `lower(cast(${jobImports.rawPayload}->'skills' as text)) LIKE ${searchPattern}`, sql `lower(${jobImports.rawPayload}->>'category') LIKE ${searchPattern}`, sql `lower(cast(${jobImports.status} as text)) LIKE ${searchPattern}`);
    ...
    mapToAggregate(row) {
        ...
        const rawPayload = new JobRawPayload(row.rawPayload);
