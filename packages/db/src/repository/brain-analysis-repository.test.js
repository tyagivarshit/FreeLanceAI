...
import { isPostgresAvailable, ensureMigrationsApplied, truncateClientDomainTables, deleteTestUsers, } from "./postgres-integration-helper.js";
import { BrainScope, BrainAnalysisAggregate, BrainAnalysisRequest, BrainContext, BrainRequestMetadata, BrainResult, BrainConfidence, BrainEvidence, BrainFailure, } from "@freelanceos/core";
describe("PostgresBrainAnalysisRepository Integration Tests", () => {
    ...
    before(async () => {
        ...
        await pool.query(`INSERT INTO users (id, email, normalized_email, status, created_at, updated_at)
       VALUES 
       ...
       ON CONFLICT (id) DO NOTHING`, [tenantAId, tenantBId]);
    ...
    function createSampleResult(analysisId, scope) {
    ...
    test("5. strict cross-tenant & cross-owner isolation", async (t) => {
        ...
        assert.strictEqual(crossTenantClaim, null, "Tenant B must not be able to claim Tenant A analysis");
    ...
    test("6. idempotency unique index enforces single active record per idempotency key", async (t) => {
        ...
        await assert.rejects(async () => repo.create(BrainAnalysisAggregate.create(req2)), /Concurrent duplicate analysis request detected/);
    });
    test("7. stale orphan recovery transitions expired RUNNING records to TIMEOUT", async (t) => {
        ...
        await pool.query(`UPDATE brain_analyses SET status = 'RUNNING', claimed_at = $1 WHERE id = $2`, [twoMinutesAgo, req.metadata.requestId]);
    ...
    test("8. concurrent claim safety under parallel execution", async (t) => {
        ...
        const claimPromises = Array.from({ length: 5 }, () => repo.claimExecution(req.metadata.requestId, scopeA));
        const claimResults = await Promise.all(claimPromises);
        const successfulClaims = claimResults.filter((c) => c !== null);
        assert.strictEqual(successfulClaims.length, 1, "Exactly one worker must win the execution claim");
    });
});
