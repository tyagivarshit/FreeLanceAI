import { pgTable, pgEnum, uuid, timestamp, jsonb, uniqueIndex, index, unique, } from "drizzle-orm/pg-core";
...
export const clients = pgTable("clients", {
    ...
    return {
        ...
        tenantCreatedAtIdx: index("clients_tenant_created_at_idx").on(table.tenantId, table.createdAt),
        ...
        tenantEmailUniqueIdx: uniqueIndex("clients_tenant_primary_email_unique_idx")
            ...
            .where(sql `${table.primaryContact} IS NOT NULL AND trim(${table.primaryContact}->>'email') <> ''`),
        tenantTaxIdUniqueIdx: uniqueIndex("clients_tenant_tax_id_unique_idx")
            ...
            .where(sql `${table.billingDetails} IS NOT NULL AND trim(${table.billingDetails}->>'taxRegistrationId') <> ''`),
    };
});
