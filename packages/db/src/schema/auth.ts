...
export const users = pgTable(
  ...
  (table) => {
    return {
      normalizedEmailIdx: uniqueIndex("users_normalized_email_unique_idx").on(
...
export const userPasswordHashes = pgTable(
...
export const sessions = pgTable(
  ...
  (table) => {
    return {
      refreshTokenHashIdx: uniqueIndex("sessions_refresh_token_hash_unique_idx").on(
        table.refreshTokenHash,
      ),
      userIdIdx: index("sessions_user_id_idx").on(table.userId),
      userIdRevokedExpiresIdx: index("sessions_user_id_revoked_at_expires_at_idx").on(
        table.userId,
        table.revokedAt,
        table.expiresAt,
      ),
