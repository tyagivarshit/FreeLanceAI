import { pgTable, pgEnum, uuid, varchar, jsonb, index, uniqueIndex, } from "drizzle-orm/pg-core";
...
export const promptContexts = pgTable("prompt_contexts", {
    ...
    return {
        ...
        tenantReferenceUniqueIdx: uniqueIndex("prompt_contexts_tenant_ref_idx").on(table.tenantId, table.reference),
        // Strict GIN Index on JSONB for dynamic search without sequential scans
        sourceReferencesGinIdx: index("prompt_contexts_source_ref_gin_idx").using("gin", table.sourceReferences),
    };
});
