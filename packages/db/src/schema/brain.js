import { pgTable, pgEnum, uuid, varchar, text, jsonb, integer, timestamp, uniqueIndex, index, unique, } from "drizzle-orm/pg-core";
...
export const brainAnalyses = pgTable("brain_analyses", {
    ...
    return {
        ...
        tenantCreatedAtIdx: index("brain_analyses_tenant_created_at_idx").on(table.tenantId, table.createdAt),
        ...
        ownerIdempotencyUniqueIdx: uniqueIndex("brain_analyses_owner_idempotency_unique_idx")
            ...
            .where(sql `${table.idempotencyKey} IS NOT NULL AND ${table.status} IN ('REQUESTED', 'RUNNING', 'COMPLETED')`),
    };
});
