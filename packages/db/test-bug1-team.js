...
test("Multi-user team workspaces (Bug 1 & 2)", async () => {
    ...
    const user3 = uuidv4();
    await db.execute('DELETE FROM users WHERE email IN (\'t1@example.com\', \'u1@example.com\', \'u2@example.com\', \'t2@example.com\', \'u3@example.com\')');
    ...
    const c1 = Client.create(clientId, tenant1, user1, { name: "Team Client" }, { currency: "USD", taxRegistrationId: "TAX1", billingAddress: { street: "1", city: "NY", state: "NY", postalCode: "1", country: "US" } }, { firstName: "John", lastName: "Doe", email: clientEmail });
    ...
    const c2 = Client.create(uuidv4(), tenant1, user2, { name: "Duplicate Client" }, undefined, { firstName: "Jane", lastName: "Doe", email: clientEmail });
    try {
