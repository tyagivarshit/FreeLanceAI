...
describe("Email Verification Use Case Tests", () => {
  let updateCalls: Array<{ table: unknown; setValues: unknown; whereClause: unknown }> = [];
  ...
  test("6. Successfully verifies token and transitions user status to active", async () => {
    ...
    (db as any).transaction = async function (callback: any) {
      const mockTx = {
    ...
    assert.ok((updateCalls[0]?.setValues as any)?.consumedAt instanceof Date);

    // 2nd update: users.status and emailVerifiedAt
    assert.strictEqual(updateCalls[1]?.table, users);
    assert.strictEqual((updateCalls[1]?.setValues as any)?.status, "active");
    assert.ok((updateCalls[1]?.setValues as any)?.emailVerifiedAt instanceof Date);
  });
});
