 ...
 */
export async function verifySessionContextFingerprint(sessionId, userId, expectedIp, expectedUserAgent, currentIp, currentUserAgent) {
...
async function triggerHijackProtocol(sessionId, userId, reason, context) {
    logger.warn({
 ...
 */
export async function enforceMfaRateLimit(userId) {
 ...
 */
export async function recordFailedMfaAttempt(userId) {
    const cache = new RedisCacheStore();
 ...
 */
export async function clearMfaAttempts(userId) {
    const cache = new RedisCacheStore();
