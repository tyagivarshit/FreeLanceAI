...
import { createVerificationEmail, ConsoleFallbackEmailService, NodemailerEmailService, ResendEmailService, } from "./email-service.js";
...
describe("Email Service & Verification Template Tests", () => {
    test("1. createVerificationEmail generates valid URLs, subject, and bodies", () => {
        ...
        assert.strictEqual(verificationUrl, `https://app.freelanceos.com/api/auth/verify-email?token=${token}`);
    ...
    test("3. NodemailerEmailService invokes sendMail on transporter", async () => {
        ...
        const service = new NodemailerEmailService(mockTransporter);
        const result = await service.sendEmail({
