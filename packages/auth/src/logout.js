...
export async function logoutUser(input) {
    const { accessToken, refreshToken, sessionId: inputSessionId, userId: inputUserId, global = false, ipAddress = "unknown", } = input;
    logger.info({
    ...
    if (!sessionId) {
        logger.warn({
            message: "[Logout Use Case] Single logout requested but session identity could not be resolved",
        });
        return { success: true, clearCredentialDirective: true };
    }
