...
export function decodeBase32(input) {
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (val === -1)
            throw new Error("Invalid base32 character in input");
        value = (value << 5) | val;
        bits += 5;
        if (bits >= 8) {
 ...
 */
export function verifyTotpCode(code, secretBase32, window = 1) {
    if (!/^\d{6}$/.test(code))
        return false;
    try {
...
function generateTotpCode(secretBytes, counter) {
