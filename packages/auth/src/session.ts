...
export async function rotateSession(
  rawRefreshToken: string,
  metadata: SessionMetadata,
): Promise<{ newRawRefreshToken: string; newSignedAccessToken: string }> {
  ...
  return await db.transaction(async (tx) => {
    ...
    if (!isCurrentToken) {
      ...
      const isWithinGracePeriod =
        timeSinceLastUpdateMs <= runtimeConfig.ROTATION_GRACE_PERIOD_SEC * 1000;
      ...
      if (isWithinGracePeriod) {
        ...
        try {
          cachedToken = await Promise.race([
            ...
            new Promise<null>((_, reject) => setTimeout(() => reject(new Error("Redis GET timeout")), 200))
          ]) as string | null;
        } catch (err) {
          logger.warn({
