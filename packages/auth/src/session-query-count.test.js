...
describe("Session Validate Query Count (Bug 1F Regression Test)", () => {
    ...
    test("validateSession should execute exactly 1 database query (single JOIN optimization)", async () => {
        ...
        try {
            ...
            assert.strictEqual(queryCount, 1, `validateSession should issue exactly 1 query, but issued ${queryCount}`);
        }
        finally {
            // Restore prototype
    ...
    test("Structural check: Prepared statement path must be intact for production", async () => {
        ...
        if (fs.existsSync(sessionFilePath)) {
            ...
            assert.ok(hasPreparedStatementDefinition && hasConditionalExecution, "The prepared statement path for validateSession must remain in the code for production performance.");
        }
    });
});
