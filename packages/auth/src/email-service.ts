  ...
  constructor(customTransporter?: Transporter) {
    if (customTransporter) {
      ...
      this.transporter = nodemailer.createTransport({
        ...
        secure,
        auth:
          runtimeConfig.SMTP_USER && runtimeConfig.SMTP_PASS
            ? {
 ...
 */
export class ConsoleFallbackEmailService implements EmailService {
  async sendEmail(payload: EmailPayload): Promise<EmailSendResult> {
    logger.warn({
      message:
        "[Email Service] No real SMTP/Resend credentials configured. Email logged to console.",
      to: payload.to,
      subject: payload.subject,
