...
export async function signupUser(input: SignupInput): Promise<RegistrationResult> {
  ...
  let sessionTokens: { signedAccessToken: string; refreshToken: string } | undefined;
  if (!runtimeConfig.CONFIG_REQUIRE_VERIFICATION_FOR_SESSION) {
    if (!sessionMetadata) {
      throw new SignupError(
        "Session metadata is required to initialize a session when auto-login is active.",
        "MISSING_METADATA",
      );
    }
