...
describe("Session Use Case Integration Tests", () => {
    test("should handle refresh token rotation race condition idempotently", async () => {
        ...
        try {
            ...
            assert.strictEqual(resA.newRawRefreshToken, resB.newRawRefreshToken, "Idempotency failed: Concurrent requests received different refresh tokens!");
            await new Promise(r => setTimeout(r, 1200));
            await assert.rejects(rotateSession(oldToken, { userAgent: "Req C", ipAddress: "127.0.0.1" }), (err) => {
