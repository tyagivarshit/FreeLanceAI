...
describe("Signup Use Case Validation & Mock Flow Tests", () => {
  ...
  let insertedRecords: { table: unknown; values: unknown }[] = [];

  interface MockInsertValues {
    email?: string;
    status?: string;
  }
    ...
    db.transaction = async function (callback) {
      ...
      const mockTx = {
        insert: (table: unknown) => ({
          values: (values: MockInsertValues) => {
  ...
  test("should throw ValidationError if password is too short", async () => {
    ...
    await assert.rejects(
      ...
      (err: unknown) => {
        ...
        assert.ok(
          (err as ValidationError).errors.some((e: string) => e.includes("at least 12 characters")),
        );
        return true;
      },
    );
