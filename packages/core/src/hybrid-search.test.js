...
import { SearchScope, SearchFilter, SearchLimit, SearchCandidate, SimilarityScore, } from "./vector-search.js";
import { FusionStrategy, FusionWeight, FusionScore, LexicalCandidate, HybridSearchCandidate, HybridSearchResult, HybridSearchRequest, } from "./hybrid-search.js";
describe("Hybrid Search Domain and Boundary Tests", () => {
    ...
    test("Fusion strategy value validation and case-insensitivity", () => {
        ...
        assert.throws(() => {
            strat.value = "WeightedScoreFusion";
        }, TypeError);
    });
    ...
    test("Weighted score fusion behavior: vector-only, lexical-only, and merged candidates", () => {
        const result = HybridSearchResult.fuse([vecCandidate1, vecCandidate2], [lexCandidate2, lexCandidate3], defaultStrategy, weightVector, weightLexical, defaultLimit);
    ...
    test("RRF score fusion behavior with rank-based calculation", () => {
        ...
        const result = HybridSearchResult.fuse([vecCandidate1, vecCandidate2], [lexCandidate2, lexCandidate3], rrfStrategy, weightVector, weightLexical, defaultLimit);
        // Candidates in RRF:
    ...
    test("Limit truncation in candidate list", () => {
        ...
        const result = HybridSearchResult.fuse([vecCandidate1, vecCandidate2], [lexCandidate2, lexCandidate3], defaultStrategy, weightVector, weightLexical, strictLimit);
    ...
    test("Mock interfaces contract compliance check", async () => {
        ...
        const mockService = {
            fuseSearch: async (req) => {
                assert.ok(req);
                return new HybridSearchResult([]);
            },
        };
        const mockLexical = {
            searchLexical: async (queryText, scope, limit) => {
                assert.ok(queryText);
                if (scope) {
                    assert.ok(scope);
                }
    ...
    test("Hybrid search candidate properties immutability check", () => {
        ...
        assert.throws(() => {
            cand.fusionScore = new FusionScore(0.9);
        }, TypeError);
    });
});
