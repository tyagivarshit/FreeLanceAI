...
import { JobSource, JobExternalIdentity, JobImportProvenance, JobRawPayload, JobImportFingerprint, JobImport, JOB_IMPORT_RECEIVED, JOB_IMPORTED, JOB_IMPORT_ARCHIVED, } from "./job-import.js";
describe("Chapter 8A — Job Import Pipeline Domain Tests", () => {
    ...
    describe("JobExternalIdentity", () => {
        ...
        test("empty source rejected", () => {
            assert.throws(() => {
                new JobExternalIdentity(null, "ext-1");
    ...
    describe("Raw payload protection", () => {
        test("valid payload and nested object/array immutability", () => {
            ...
            assert.throws(() => {
                returnedData.skills.push("javascript");
            });
            assert.throws(() => {
                returnedData.client.country = "CA";
            });
        });
    });
    ...
    describe("JobImport Aggregate creation", () => {
        test("Draft creation and RECEIVED status, snapshot version 1, event emitted", () => {
            ...
            const aggregate = JobImport.create(id, "tenant-1", "owner-1", defaultExternalIdentity, defaultProvenance, defaultPayload, defaultFingerprint);
    ...
    describe("Ownership validation", () => {
        test("authorized markImported succeeds, unauthorized rejected", () => {
            const aggregate = JobImport.create("job-import-1", "tenant-1", "owner-1", defaultExternalIdentity, defaultProvenance, defaultPayload, defaultFingerprint);
    ...
    describe("Lifecycle Transitions", () => {
        test("RECEIVED -> IMPORTED", () => {
            const agg = JobImport.create("job-1", "tenant-1", "owner-1", defaultExternalIdentity, defaultProvenance, defaultPayload, defaultFingerprint);
    ...
    describe("Persistence signatures check", () => {
        test("contracts verify technology neutrality", () => {
            const dummyStore = {
                async save(_jobImport) { },
                async findById(_id, _tenantId) {
                    return null;
                },
                async findByExternalIdentity(_tenantId, _source, _externalJobId) {
                    return null;
                },
            };
            const dummyContract = {
