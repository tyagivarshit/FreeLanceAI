...
import { SearchQuery, AuthorizedSearchScope, SearchDomainError, assertSearchJsonSafe, ClientSearchEngine, InMemoryClientSearchRepository, mapClientToSearchResult, } from "./index.js";
describe("Phase 11D-2: Client Search Domain & Provider Tests", () => {
    ...
    test("9. pageSize maximum bound of 100 enforced", async () => {
        ...
        assert.throws(() => new SearchQuery({ query: "Acme", pageSize: 101 }), (err) => {
    ...
    test("18. malformed search queries fail safely with domain errors", async () => {
        ...
        assert.throws(() => new SearchQuery({ query: "valid", page: -1 }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_PAGINATION");
            return true;
        });
    ...
    test("20. repository interface enforces query-level pagination without full-dataset loading", async () => {
        ...
        const mockRepo = {
            searchClients: async (_q, _scope, page, pageSize) => {
                capturedPage = page;
