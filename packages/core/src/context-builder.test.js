...
import { Context, ContextBlueprint, ContextMetadata, ContextAssemblyRule, ContextSourceReference, CONTEXT_REGISTERED, CONTEXT_VALIDATED, CONTEXT_PUBLISHED, CONTEXT_ARCHIVED, } from "./context-builder.js";
describe("Context Builder Domain Aggregate Tests", () => {
    ...
    test("Context creation success: status Draft, rules/refs copied, registered event emitted", () => {
        const context = Context.create("ctx-1", "client.profile.summary", "owner-999", defaultBlueprint, defaultMetadata, defaultAssemblyRules, defaultSourceRefs);
    ...
    test("Context reference format validation rejects invalid keys", () => {
        assert.throws(() => {
            Context.create("ctx-1", "client..summary", "owner-999", defaultBlueprint, defaultMetadata, defaultAssemblyRules, defaultSourceRefs);
    ...
    test("Ownership validation blocks execution under invalid actor ID", () => {
        const context = Context.create("ctx-1", "client.profile.summary", "owner-999", defaultBlueprint, defaultMetadata, defaultAssemblyRules, defaultSourceRefs);
        assert.throws(() => {
            context.updateDraft("unauthorized-owner", alternativeBlueprint, defaultMetadata, defaultAssemblyRules, defaultSourceRefs);
    ...
    test("Updating draft replaces value objects and keeps historical arrays immutable", () => {
        const context = Context.create("ctx-1", "client.profile.summary", "owner-999", defaultBlueprint, defaultMetadata, defaultAssemblyRules, defaultSourceRefs);
        context.updateDraft("owner-999", alternativeBlueprint, alternativeMetadata, alternativeAssemblyRules, alternativeSourceRefs);
    ...
    test("Lifecycle flow: Draft -> Validated -> Published -> Archived", () => {
        ...
        assert.strictEqual(context.domainEvents.length, 1);
        assert.strictEqual(context.domainEvents[0].eventType, CONTEXT_VALIDATED);
        // Cannot edit in Validated status
        assert.throws(() => {
            context.updateDraft("owner-999", alternativeBlueprint, defaultMetadata, defaultAssemblyRules, defaultSourceRefs);
        ...
        assert.strictEqual(context.domainEvents.length, 1);
        assert.strictEqual(context.domainEvents[0].eventType, CONTEXT_PUBLISHED);
        ...
        assert.strictEqual(context.domainEvents.length, 1);
        assert.strictEqual(context.domainEvents[0].eventType, CONTEXT_ARCHIVED);
    ...
    test("Mock interfaces contract compliance check", async () => {
        ...
        const mockPersistence = {
            checkUniqueReference: async (ownerId, reference, excludeContextId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludeContextId) {
        ...
        const isUnique = await mockPersistence.checkUniqueReference("owner-999", "client.profile.summary");
