...
import { db, users, clients, clientEmbeddings } from "@freelanceos/db";
...
describe("Phase 5: Search & Retrieval Integration Tests", () => {
  ...
  let embeddingEngine: EmbeddingEngineService;
  let vectorEngine: VectorSearchEngineService;
  let hybridEngine: HybridSearchEngineService;
  let rerankEngine: ReRankingEngineService;
  ...
  beforeAll(async () => {
    ...
    // Initialize Services
    aiGateway = new AiGatewayService({} as any, {} as any, {});
  ...
  describe("5D: Secure Re-Ranking Engine Tests", () => {
    ...
    it("should immediately throw a SECURITY EXCEPTION if foreign payload chunks bleed into context", async () => {
      ...
      await expect(
        rerankEngine.reRankPayload(TEST_TENANT_ID, corruptedPayload, "test query")
      ).rejects.toThrow(/SECURITY EXCEPTION - RE-RANKER/);
    });
