...
import { ToneProfile, ToneRequest, ToneResult, ReplyToneAdjustment, REPLY_TONE_DRAFTED, REPLY_TONE_REQUESTED, REPLY_TONE_ADJUSTED, REPLY_TONE_ARCHIVED, } from "./reply-tone.js";
describe("Chapter 7C — Reply Studio / Tone Adjustment Domain Tests", () => {
    ...
    describe("Ownership validation", () => {
        test("ownership checks on command operations", () => {
            const aggregate = ReplyToneAdjustment.create("tone-1", "reply.1", "owner-1", 1, defaultRequest);
            // Unauthorized checks
            assert.throws(() => {
    ...
    describe("Persistence Interfaces Compile Checks", () => {
        test("signatures matching check", () => {
            ...
            const dummyContract = {
                async checkUniqueReference(_ownerId, _reference, _excludeAdjustmentId) {
                    return true;
                },
            };
