...
import { Policy, PolicyReference, PolicyDefinition, PolicyMetadata, PolicyRuleSet, PolicyEvaluationResult, DecisionFingerprint, PolicySnapshot, POLICY_REGISTERED, POLICY_VALIDATED, POLICY_PUBLISHED, POLICY_ARCHIVED, POLICY_EVALUATED, } from "./policy.js";
describe("Policy Engine Domain Aggregate Root & Value Objects Tests", () => {
    const defaultDefinition = new PolicyDefinition("Allow access to resource when role matches Admin");
    ...
    test("Policy creation success: status Draft, snapshot initialized, domain event emitted", () => {
        const policy = Policy.create("pol-1", "auth.admin.access", "owner-123", defaultDefinition, defaultMetadata, defaultRuleSet, "snap-1");
        ...
        assert.strictEqual(policy.snapshots[0].definitionSnapshot.governanceSpecification, "Allow access to resource when role matches Admin");
        assert.strictEqual(policy.snapshots[0].metadataSnapshot.displayName, "Admin Authorization Policy");
        ...
        const event = policy.domainEvents[0];
    ...
    test("Policy reference format validation rejects invalid keys", () => {
        assert.throws(() => {
            Policy.create("pol-1", "auth..admin", "owner-123", defaultDefinition, defaultMetadata, defaultRuleSet, "snap-1");
    ...
    test("Missing owner throws validation error", () => {
        assert.throws(() => {
            Policy.create("pol-1", "auth.admin.access", "", defaultDefinition, defaultMetadata, defaultRuleSet, "snap-1");
    ...
    test("Lifecycle flow and evaluation behavior updates snapshot history", () => {
        ...
        assert.strictEqual(policy.snapshots[1].decisionFingerprintSnapshot.decisionIdentifier, "dec-999");
        assert.strictEqual(policy.domainEvents.length, 1);
    ...
    test("Append-only snapshot history is structurally frozen", () => {
        ...
        assert.throws(() => {
            snapshots.push(new PolicySnapshot({
