...
import { Plan, PlanCatalog, calculateCalendarMonthPeriod } from "./plan.js";
...
import { InMemoryStripeSubscriptionRepository, InMemoryWebhookEventStore, StripeWebhookProcessor, } from "./webhook.js";
import { EntitlementResolver, EntitlementEnforcer, InMemoryUsageRepository, } from "./entitlements.js";
import { MemoryCacheStore } from "./job-match-cache.js";
// Helper to set up test environment
function setupTestContext(cacheStore) {
...
describe("CHAPTER 10D - ENTITLEMENTS & USAGE ENFORCEMENT", () => {
    describe("PLAN ACCESS (Tests 1-10)", () => {
        ...
        test("5. Priority scoring denied for Starter", async () => {
            const { resolver } = setupTestContext();
            const decision = await resolver.resolveEntitlement("tenant_1", "user_1", "PRIORITY_WEIGHT_SCORING");
            assert.strictEqual(decision.allowed, false);
            assert.strictEqual(decision.reason, "FEATURE_NOT_INCLUDED");
        });
        test("6. Priority scoring allowed for Pro", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_pro", "user_pro", "PRIORITY_WEIGHT_SCORING");
        ...
        test("7. Priority AI denied for Pro", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_pro", "user_pro", "PRIORITY_AI_GENERATION");
            assert.strictEqual(decision.allowed, false);
        });
        test("8. Priority AI allowed for Power Bidder", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_pb", "user_pb", "PRIORITY_AI_GENERATION");
            assert.strictEqual(decision.allowed, true);
        });
        test("9. Multi-workspace denied for Pro", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_pro", "user_pro", "MULTI_WORKSPACE");
            assert.strictEqual(decision.allowed, false);
        });
    ...
    describe("TRIAL (Tests 11-20)", () => {
        test("11. Active trial receives Pro features", async () => {
            ...
            await trialPersistence.save(new TrialGrant({
                grantId: "trial_1",
                userId: "user_trial",
                planId: "PRO",
                status: "ACTIVE",
            ...
            const decision = await resolver.resolveEntitlement("tenant_trial", "user_trial", "ADVANCED_MATCHING", new Date(start.getTime() + 1000));
            assert.strictEqual(decision.allowed, true);
            assert.strictEqual(decision.plan, "PRO");
        ...
        test("12. Active trial receives Pro limits", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_trial", "user_trial", "AI_PROPOSAL", new Date(start.getTime() + 1000));
        ...
        test("13. Exact trial expiration denies Pro features", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_trial", "user_trial", "ADVANCED_MATCHING", end);
        ...
        test("14. Expired trial falls back correctly", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_trial", "user_trial", "ADVANCED_MATCHING", checkTime);
            assert.strictEqual(decision.allowed, false);
            const decisionProposal = await resolver.resolveEntitlement("tenant_trial", "user_trial", "AI_PROPOSAL", checkTime);
            assert.strictEqual(decisionProposal.limit.type, "LIMITED");
            if (decisionProposal.limit.type === "LIMITED") {
                assert.strictEqual(decisionProposal.limit.value, 3, "Should fall back to Starter limits (3 proposals)");
            }
        });
        ...
        test("16. Converted trial uses paid billing state", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_trial", "user_trial", "PRIORITY_AI_GENERATION", new Date(start.getTime() + 1000));
            assert.strictEqual(decision.allowed, true);
            assert.strictEqual(decision.plan, "POWER_BIDDER");
            assert.strictEqual(decision.source, "SUBSCRIPTION");
        });
        ...
        test("18. Logout/login cannot restart trial", async () => {
            ...
            const decisionBefore = await resolver.resolveEntitlement("tenant_trial", "user_trial", "ADVANCED_MATCHING", new Date(start.getTime() + 1000));
            assert.strictEqual(decisionBefore.allowed, true);
            // Simulating check after trial duration has passed
            const decisionAfter = await resolver.resolveEntitlement("tenant_trial", "user_trial", "ADVANCED_MATCHING", new Date(end.getTime() + 1000));
        ...
        test("19. Multiple devices share authoritative trial state", async () => {
            ...
            const dev1 = await resolver.resolveEntitlement("tenant_device_1", "user_trial", "ADVANCED_MATCHING", new Date(start.getTime() + 1000));
            const dev2 = await resolver.resolveEntitlement("tenant_device_2", "user_trial", "ADVANCED_MATCHING", new Date(start.getTime() + 1000));
        ...
        test("20. Timezone does not change trial duration", async () => {
            ...
            assert.strictEqual(grant.trialEndsAt.getTime() - grant.trialStartedAt.getTime(), 7 * 24 * 60 * 60 * 1000);
        });
    });
    describe("USAGE (Tests 21-34)", () => {
        ...
        test("29. Pro matching is unlimited", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_pro", "user_pro", "ADVANCED_MATCHING");
        ...
        test("33. Usage cannot be reset by logout/login", async () => {
            const { resolver } = setupTestContext();
            await resolver.consumeUsage("tenant_starter", "user_starter", "AI_PROPOSAL");
            const decision1 = await resolver.resolveEntitlement("tenant_starter", "user_starter", "AI_PROPOSAL");
            assert.strictEqual(decision1.remaining, 2);
            // Simulating a second device or session (new login does not affect server state)
            const decision2 = await resolver.resolveEntitlement("tenant_starter", "user_starter", "AI_PROPOSAL");
        ...
        test("34. Usage cannot be reset by changing email", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_starter", "user_starter", "AI_PROPOSAL");
    ...
    describe("PERIOD (Tests 41-44)", () => {
        test("41. Current usage period", async () => {
            const { resolver } = setupTestContext();
            const decision = await resolver.resolveEntitlement("tenant_starter", "user_starter", "JOB_SCAN");
        ...
        test("42. New usage period", async () => {
            ...
            const d1 = await resolver.resolveEntitlement("tenant_starter", "user_starter", "JOB_SCAN", t1);
            const d2 = await resolver.resolveEntitlement("tenant_starter", "user_starter", "JOB_SCAN", t2);
        ...
        test("43. Old period does not block new period", async () => {
            ...
            for (let i = 0; i < 5; i++) {
                const res = await resolver.consumeUsage("tenant_starter", "user_starter", "JOB_SCAN", 1, t1);
                assert.strictEqual(res.success, true);
            }
            const resBlocked = await resolver.consumeUsage("tenant_starter", "user_starter", "JOB_SCAN", 1, t1);
            assert.strictEqual(resBlocked.success, false);
            // Verify that next period (September) starts fresh and allows scanning
            const resNew = await resolver.consumeUsage("tenant_starter", "user_starter", "JOB_SCAN", 1, t2);
    ...
    describe("CACHE (Tests 45-54)", () => {
        ...
        test("47. Cache miss", async () => {
            ...
            assert.strictEqual(planInfo.plan.planId, "STARTER", "Should Miss and resolve database default");
        });
        ...
        test("50. Trial expiration invalidation", async () => {
            ...
            const p1 = await resolver.resolveEffectivePlan("tenant_exp", "user_exp", new Date(start.getTime() + 1000));
            ...
            const p2 = await resolver.resolveEffectivePlan("tenant_exp", "user_exp", new Date(end.getTime() + 1000));
    ...
    describe("SECURITY (Tests 55-62)", () => {
        test("55. Forged plan rejected", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_1", "user_1", "ADVANCED_MATCHING");
            assert.strictEqual(decision.allowed, false, "Should resolve to Starter and block Pro feature");
        });
        test("56. Forged feature rejected", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_1", "user_1", "PRIORITY_AI_GENERATION");
            assert.strictEqual(decision.allowed, false);
        });
        ...
        test("59. Forged trial rejected", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_starter", "user_starter", "ADVANCED_MATCHING");
            assert.strictEqual(decision.allowed, false);
            assert.strictEqual(decision.plan, "STARTER");
        });
        test("60. Cross-tenant entitlement rejected", async () => {
            ...
            assert.strictEqual(decision.allowed, false, "tenant_b has no access to tenant_a subscription");
        });
        test("61. Cross-tenant usage rejected", async () => {
            ...
            assert.strictEqual(decisionB.remaining, 3, "tenant_b should still have 3 remaining proposals");
        });
    ...
    describe("BILLING STATE (Tests 63-70)", () => {
        ...
        test("67. Payment-failure state according to existing semantics", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_1", "user_1", "ADVANCED_MATCHING");
            assert.strictEqual(decision.allowed, false, "past_due subscription should deny Pro features (fail closed)");
        });
        test("68. Unknown billing entity", async () => {
            const { resolver } = setupTestContext();
            const decision = await resolver.resolveEntitlement("unknown_tenant", "unknown_user", "JOB_SCAN");
    ...
    describe("FAIL-CLOSED (Tests 71-74)", () => {
        ...
        test("73. Cache contains stale state", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_stale", "user_stale", "ADVANCED_MATCHING");
            assert.strictEqual(decision.allowed, true, "Authoritative DB state should win over stale cache");
        });
        test("74. Invalid billing state", async () => {
            ...
            const decision = await resolver.resolveEntitlement("tenant_invalid", "user_invalid", "ADVANCED_MATCHING");
            assert.strictEqual(decision.allowed, false, "Invalid billing status fails closed and denies paid feature");
        });
    });
    describe("PRODUCTION HARDENING AUDIT REMEDIATIONS (R1, R2, R3)", () => {
        describe("R1 - Paid Precedence", () => {
            test("R1-1. Active paid PRO overrides active trial", async () => {
                ...
                const planInfo = await resolver.resolveEffectivePlan("tenant_r1", "user_r1", new Date(start.getTime() + 1000));
                assert.strictEqual(planInfo.plan.planId, "PRO");
                assert.strictEqual(planInfo.source, "SUBSCRIPTION", "Subscription must override active trial");
            });
            test("R1-2. Active POWER_BIDDER overrides active PRO trial", async () => {
                ...
                const planInfo = await resolver.resolveEffectivePlan("tenant_r1_2", "user_r1_2", new Date(start.getTime() + 1000));
                assert.strictEqual(planInfo.plan.planId, "POWER_BIDDER");
                assert.strictEqual(planInfo.source, "SUBSCRIPTION", "POWER_BIDDER subscription must override PRO trial");
            });
            ...
            test("R1-4. Active trial + no subscription", async () => {
                ...
                const planInfo = await resolver.resolveEffectivePlan("tenant_r1_4", "user_r1_4", new Date(start.getTime() + 1000));
                assert.strictEqual(planInfo.plan.planId, "PRO");
            ...
            test("R1-6. Inactive subscription + active trial", async () => {
                ...
                const planInfo = await resolver.resolveEffectivePlan("tenant_r1_6", "user_r1_6", new Date(start.getTime() + 1000));
                assert.strictEqual(planInfo.plan.planId, "PRO");
                assert.strictEqual(planInfo.source, "TRIAL", "Inactive subscription must not override active trial");
            });
        ...
        describe("R2 - Trial Conversion", () => {
            function setupProcessor(trialPersistence, subscriptionRepo, customerMappingRepo) {
            ...
            test("R2-2. PRO trial -> POWER_BIDDER paid conversion", async () => {
                ...
                assert.ok(updatedGrants[0]);
                assert.strictEqual(updatedGrants[0].status, "CONVERTED", "PRO trial should be CONVERTED on POWER_BIDDER paid plan purchase");
            });
