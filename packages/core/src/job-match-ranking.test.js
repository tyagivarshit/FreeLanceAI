...
import { JobMatchRankingPolicy, RankedMatch, RankingFingerprint, buildCandidateSetIdentity, JobMatchRankingSnapshot, JOB_MATCH_RANKING_CREATED, JOB_MATCH_RANKING_COMPLETED, JOB_MATCH_RANKING_ARCHIVED, JobMatchRanking, } from "./job-match-ranking.js";
describe("Chapter 8F — Job Match Ranking Domain Tests", () => {
    const defaultPolicy = new JobMatchRankingPolicy("v1", "score-desc", "tiebreaker-asc-match-asc", "ordinal");
    const defaultScoredMatches = [
        {
            matchId: "match-1",
            scoreId: "score-1",
            tenantId: "tenant-1",
    ...
    describe("1. Ranking Identity", () => {
        test("valid construction and logical properties", () => {
            const ranking = JobMatchRanking.create("rank-123", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", defaultCandidateIds);
            ...
            assert.strictEqual(ranking.candidateSetIdentity, buildCandidateSetIdentity(defaultCandidateIds));
        });
        test("identity fields are immutable", () => {
            const ranking = JobMatchRanking.create("rank-123", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", defaultCandidateIds);
            assert.throws(() => {
                ranking.rankingSubjectId = "mutated";
            }, TypeError);
            assert.throws(() => {
                ranking.rankingVersion = "v2";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("2. Ranking Policy", () => {
        test("valid construction, immutable", () => {
            const policy = new JobMatchRankingPolicy("v1", "score-desc", "tiebreaker-asc-match-asc", "ordinal");
            ...
            assert.throws(() => {
                policy.rankingPolicyVersion = "v2";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("4. Tie-Breaking", () => {
        test("equal scores deterministic fallback: tieBreakerKey ASC, matchId ASC", () => {
            ...
            const ranking = JobMatchRanking.create("rank-123", "tenant-1", "owner-1", "subject-job-123", matchingVersion, scoringVersion, "v1", "v1", ids);
    ...
    describe("7. Duplicate Candidates", () => {
        test("duplicate matchId rejected in create factory", () => {
            assert.throws(() => {
                JobMatchRanking.create("rank-123", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", ["match-1", "match-1"]);
    ...
    describe("8. Score-Version Compatibility", () => {
        test("rejects mismatched matchingVersion or scoringVersion candidates", () => {
            const ranking = JobMatchRanking.create("rank-123", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", // expected scoringVersion v1
            "v1", "v1", defaultCandidateIds);
    ...
    describe("9. Empty Ranking", () => {
        test("empty sets are valid ranking outputs", () => {
            const ranking = JobMatchRanking.create("rank-123", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", []);
    ...
    describe("14. Archive Operation", () => {
        test("archive transitions CREATED and RANKED to ARCHIVED correctly", () => {
            const ranking1 = JobMatchRanking.create("rank-1", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", defaultCandidateIds);
            ...
            const ranking2 = JobMatchRanking.create("rank-2", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", defaultCandidateIds);
    ...
    describe("15. Invalid Lifecycle", () => {
        test("rejects backwards states from RANKED and terminal states", () => {
            const ranking = JobMatchRanking.create("rank-1", "tenant-1", "owner-1", "subject-job-123", "v1", "v1", "v1", "v1", defaultCandidateIds);
            ranking.rank("owner-1", defaultScoredMatches, defaultPolicy);
            assert.throws(() => {
                ranking.transitionTo("CREATED");
            }, /Invalid lifecycle transition from RANKED to CREATED/);
            ranking.archive("owner-1");
            assert.strictEqual(ranking.status, "ARCHIVED");
            assert.throws(() => ranking.rank("owner-1", defaultScoredMatches, defaultPolicy), /Invalid lifecycle transition from ARCHIVED to RANKED/);
    ...
    describe("22. Persistence contracts", () => {
        test("neutral contracts compile and run successfully", () => {
            const dummyStore = {
                ...
                async findByRankingIdentity(_tenantId, _rankingSubjectId, _candidateSetIdentity, _matchingVersion, _scoringVersion, _rankingVersion, _rankingPolicyVersion) {
                    return null;
                },
            };
            const dummyContract = {
