...
import { SearchQuery, AuthorizedSearchScope, SearchDomainError, assertSearchJsonSafe, JobSearchEngine, InMemoryJobSearchRepository, mapJobToSearchResult, } from "./index.js";
describe("Phase 11D-3: Job Search Domain & Provider Tests", () => {
    ...
    const jobRecords = [
        ...
        },
        {
            ...
            source: "linkedin",
            status: "RECEIVED",
            description: "Design accessible and high performance UI components with Tailwind and Next.js.",
        ...
        },
        {
            ...
            source: "indeed",
            status: "ARCHIVED",
            description: "Manage multi-cloud Kubernetes clusters, Terraform modules, and Prometheus alerting.",
    ...
    test("10. maximum pageSize = 100 enforced", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "engineer", pageSize: 101 }), (err) => {
    ...
    test("18. safe SearchResult mapping formats canonical DTOs properly", () => {
        ...
        assert.strictEqual(searchResult.display.snippet, "Lead zero-trust migration and enterprise threat models.");
    ...
    test("20. malformed search queries fail safely with domain errors", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "valid", pageSize: 0 }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_PAGINATION");
            return true;
        });
    ...
    test("22. repository interface enforces single bounded execution without N+1 queries", async () => {
        let callCount = 0;
        const singleCallRepo = {
            searchJobs: async (_q, _scope, page, pageSize) => {
                callCount++;
                return {
                    items: [
                        {
