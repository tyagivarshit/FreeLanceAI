...
import { Prompt, PromptDefinition, PromptMetadata, LogicalVisibilityClassification, PROMPT_REGISTERED, PROMPT_UPDATED, PROMPT_PUBLISHED, PROMPT_DEPRECATED, PROMPT_ARCHIVED, } from "./prompt-registry.js";
describe("Prompt Registry Domain Aggregate Tests", () => {
    ...
    test("Prompt creation success: initial status Draft, version 1 draft snapshot, and event emitted", () => {
        const prompt = Prompt.create("prompt-1", "client.onboarding.welcome", "owner-123", defaultDefinition, defaultMetadata, defaultVisibility);
        ...
        assert.strictEqual(prompt.definition.promptTextSpecification, "Hello {{name}}, welcome to FreelanceOS!");
        ...
        assert.strictEqual(ver.definitionSnapshot.promptTextSpecification, defaultDefinition.promptTextSpecification);
    ...
    test("Prompt reference format validation rejects invalid strings", () => {
        assert.throws(() => {
            Prompt.create("prompt-1", "client..onboarding.welcome", // double dots
            "owner-123", defaultDefinition, defaultMetadata, defaultVisibility);
        }, /Invalid prompt reference format/);
        assert.throws(() => {
            Prompt.create("prompt-1", "Client.Onboarding", // Upper-case
            "owner-123", defaultDefinition, defaultMetadata, defaultVisibility);
        }, /Invalid prompt reference format/);
        assert.throws(() => {
            Prompt.create("prompt-1", "client-onboarding", // hyphens instead of dots
            "owner-123", defaultDefinition, defaultMetadata, defaultVisibility);
        }, /Invalid prompt reference format/);
    });
    test("Prompt owner validation blocks operations with mismatched owner ID", () => {
        const prompt = Prompt.create("prompt-1", "client.onboarding.welcome", "owner-123", defaultDefinition, defaultMetadata, defaultVisibility);
        assert.throws(() => {
            prompt.updateDraft("unauthorized-owner", alternativeDefinition, defaultMetadata, defaultVisibility);
    ...
    test("Updating a draft successfully replaces metadata value object and updates existing version snapshot", () => {
        ...
        prompt.updateDraft("owner-123", alternativeDefinition, alternativeMetadata, alternativeVisibility);
        ...
        assert.strictEqual(prompt.versions.length, 1);
        const ver = prompt.versions[0];
        assert.strictEqual(ver.definitionSnapshot.promptTextSpecification, alternativeDefinition.promptTextSpecification);
    ...
    test("Creating a new draft from Published increments version number and leaves published version frozen", () => {
        ...
        prompt.publish("owner-123");
        prompt.clearDomainEvents();
        prompt.createNewDraft("owner-123", alternativeDefinition, alternativeMetadata, alternativeVisibility);
        assert.strictEqual(prompt.status, "Draft");
        ...
        assert.strictEqual(v1.definitionSnapshot.promptTextSpecification, defaultDefinition.promptTextSpecification);
        ...
        assert.strictEqual(v2.definitionSnapshot.promptTextSpecification, alternativeDefinition.promptTextSpecification);
    ...
    test("Mock persistence unique reference check interface compliance", async () => {
        const mockPersist = {
            checkUniqueReference: async (ownerId, reference, excludePromptId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludePromptId) {
