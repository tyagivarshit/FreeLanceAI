 ...
 */
export function mapClientToSearchResult(item, queryText) {
    const matchedFields = [];
...
export class ClientSearchEngine {
    ...
    constructor(repository) {
        if (!repository || typeof repository.searchClients !== "function") {
            throw new SearchDomainError("INVALID_SEARCH_REQUEST", "ClientSearchRepository implementation is required.");
        }
    ...
    async search(query, scope) {
        ...
        try {
            const searchResultList = await this._repository.searchClients(query.query, scope, query.page, query.pageSize);
            const results = searchResultList.items.map((item) => mapClientToSearchResult(item, query.query));
            return new SearchResultSet({
                results,
...
export class InMemoryClientSearchRepository {
    _records = [];
    ...
    async searchClients(queryText, scope, page = DEFAULT_SEARCH_PAGE, pageSize = DEFAULT_SEARCH_PAGE_SIZE) {
        const q = queryText.toLowerCase().trim();
        ...
        const matched = scoped.filter((r) => {
            ...
            return (nameMatch || emailMatch || websiteMatch || firstNameMatch || lastNameMatch || phoneMatch);
        });
