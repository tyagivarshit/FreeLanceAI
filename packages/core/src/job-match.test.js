...
import { calculateCosineSimilarity, matchExperience, matchBudget, matchJobType, matchLocation, freezeMatchSignals, JobMatchSnapshot, JOB_MATCH_CREATED, JOB_MATCH_EVALUATED, JOB_MATCH_ARCHIVED, JobMatch, } from "./job-match.js";
describe("Chapter 8D — Job Matching Engine Domain Tests", () => {
    ...
    const defaultJobEmbedding = {
        id: "emb-123",
        tenantId: "tenant-1",
        vector: [0.1, 0.2, 0.3],
        dimensions: 3,
    };
    const defaultMatchingInputs = {
    ...
    describe("1. Match Identity", () => {
        test("valid construction and logical properties", () => {
            const match = JobMatch.create("match-1", "tenant-1", "owner-1", "free-123", "job-123", "norm-123", "v1", "v1", "emb-123", "v1");
        ...
        test("identity fields are immutable", () => {
            const match = JobMatch.create("match-1", "tenant-1", "owner-1", "free-123", "job-123", "norm-123", "v1", "v1");
            assert.throws(() => {
                match.freelancerId = "mutated";
            }, TypeError);
            assert.throws(() => {
                match.matchingVersion = "v2";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("2. Tenant Isolation", () => {
        ...
        test("cross-tenant inputs are rejected", () => {
            ...
            const crossTenantFreelancerInputs = {
                freelancerProfile: {
    ...
    describe("3. Semantic Matching", () => {
        ...
        test("verifies that no overallScore is calculated inside similarity signal", () => {
            ...
            assert.ok(!("overallScore" in match.matchSignals));
            assert.ok(!("weightedScore" in match.matchSignals));
        });
    });
    // ==========================================
    // 4. SKILL MATCHING
    ...
    describe("4. Skill Matching", () => {
        ...
        test("skill signal arrays are immutable", () => {
            ...
            const signals = match.matchSignals;
            assert.throws(() => {
                signals.matchedSkills.push("docker");
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("9. MatchSignals", () => {
        test("deep immutability on MatchSignals fields, zero score metrics", () => {
            ...
            assert.throws(() => {
                signals.skillCoverage = 0.9;
            }, TypeError);
            assert.throws(() => {
                signals.matchedSkills.push("javascript");
            }, TypeError);
    ...
    describe("12. Archive Operation", () => {
        test("transitions CREATED -> ARCHIVED and EVALUATED -> ARCHIVED correctly", () => {
            const match1 = JobMatch.create("match-1", "tenant-1", "owner-1", "free-123", "job-123", "norm-123", "v1", "v1", "emb-123", "v1");
            match1.archive("owner-1");
            assert.strictEqual(match1.status, "ARCHIVED");
            const match2 = JobMatch.create("match-2", "tenant-1", "owner-1", "free-123", "job-123", "norm-123", "v1", "v1", "emb-123", "v1");
    ...
    describe("13. Invalid Lifecycle", () => {
        test("rejects backward and terminal state lifecycle updates", () => {
            ...
            assert.throws(() => match.evaluate("owner-1", defaultMatchingInputs), /Invalid lifecycle transition from ARCHIVED to EVALUATED/);
    ...
    describe("18. Persistence contracts", () => {
        test("neutral contracts compile and run successfully", () => {
            const dummyStore = {
                ...
                async findByMatchingIdentity(_tenantId, _freelancerId, _jobId, _matchingVersion) {
                    return null;
                },
            };
            const dummyContract = {
