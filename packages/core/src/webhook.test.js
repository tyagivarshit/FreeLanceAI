...
import { StripeWebhookProcessor, InMemoryStripeSubscriptionRepository, InMemoryWebhookEventStore, } from "./webhook.js";
...
class InMemoryPaymentAggregateStore {
    _payments = new Map();
    async save(payment) {
    ...
    async findById(paymentId, ownerId) {
        return this._payments.get(`${paymentId}_${ownerId}`) ?? null;
    }
    async findByReference(paymentReference, ownerId) {
...
class MockStripeClient {
    webhooks = {
        constructEvent: (payload, header, _secret, _tolerance) => {
...
describe("10C — Webhook Processing Unit Tests", () => {
    ...
    describe("SIGNATURE VALIDATION TESTS (1-8)", () => {
        test("1. valid signature", async () => {
            ...
            }, (err) => err.code === "MAPPING_NOT_FOUND");
        });
    ...
    describe("DEDUPLICATION & IDEMPOTENCY TESTS (16-25)", () => {
        ...
        test("18. duplicate after process restart (simulated by event store persistence)", async () => {
            const { customerMappingRepo, subscriptionRepo, paymentStore, trialPersistence, eventStore } = await setupTestContext();
            await customerMappingRepo.save({
                tenantId: "tenant_1",
                ownerId: "owner_1",
                stripeCustomerId: "cust_1",
    ...
    describe("FAILURE & RETRY TESTS (57-60)", () => {
        ...
        test("58. retry succeeds after transient failure", async () => {
            ...
            const customerMappingRepo = {
                ...
                findByStripeCustomerId: async (_id) => {
                    if (shouldThrow) {
                        shouldThrow = false;
