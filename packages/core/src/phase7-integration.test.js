...
test("Phase 7: Reply Studio Integration Suite", async (t) => {
    ...
    await t.test("1. Cross-tenant asset extraction attempts safely throw triple-lock security exceptions", async () => {
        ...
        await assert.rejects(generationEngine.enqueueProposalGeneration({
            tenantId: "tenant_B", // Mismatch
            clientId: "client_7a",
            userPrompt: "Generate a proposal"
        }), /Security Violation|Tenant Isolation Violation/, "Generation Engine must block cross-tenant client access");
        ...
        await assert.rejects(rewriteEngine.enqueueTextRewrite({
            tenantId: "tenant_B",
            clientId: "client_7b",
            documentId: "doc_7b", // Mismatched doc
            originalText: "Hello",
            rewriteInstructions: "Fix it"
        }), /Tenant Isolation Violation/, "Rewrite Engine must block cross-tenant document access");
        ...
        await assert.rejects(toneEngine.enqueueToneAdjustment({
            tenantId: "tenant_C",
            clientId: "client_7c",
            profileId: "prof_7c", // Mismatched profile
            originalText: "Hello",
            targetTone: "professional"
        }), /Tenant Isolation Violation/, "Tone Engine must block cross-tenant profile IP extraction");
    });
