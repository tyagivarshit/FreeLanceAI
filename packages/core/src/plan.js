 ...
 */
export function calculateCalendarMonthPeriod(from) {
 ...
 */
export function calculateTrialPeriod(startedAt, endsAt) {
    return {
        type: "TRIAL_DURATION",
 ...
 */
export function validateCurrencyRegionCompatibility(currency, region) {
    ...
    if (region === "NORTH_AMERICA" && canonical !== "USD") {
        throw new Error(`Currency ${canonical} is incompatible with region NORTH_AMERICA. NORTH_AMERICA requires USD.`);
    }
    ...
    if (region === "EUROPE" && canonical !== "EUR") {
        throw new Error(`Currency ${canonical} is incompatible with region EUROPE. EUROPE requires EUR.`);
    }
    if (region === "GLOBAL" && canonical !== "USD") {
        throw new Error(`Currency ${canonical} is incompatible with region GLOBAL. GLOBAL requires USD.`);
    }
}
/**
 * Validates a plan price model.
 */
export function validatePlanPrice(price) {
    ...
    if (price.interval !== "MONTHLY") {
        throw new Error(`Unsupported billing interval: ${price.interval}. Only MONTHLY is supported in 10A.`);
    }
...
export class PricingRegionResolver {
     ...
     */
    static resolveRegionAndCurrency(countryCode) {
...
export class Plan {
    ...
    constructor(properties) {
        ...
        this._prices = properties.prices.map((p) => {
            ...
            if (priceKeys.has(key)) {
                throw new Error(`Duplicate price version rejected: multiple prices for region ${p.region}, interval ${p.interval}, and version ${p.version}.`);
            }
            priceKeys.add(key);
            const frozenPrice = { ...p };
     ...
     */
    getPriceByVersion(priceId, version) {
     ...
     */
    getLatestPrice(region, interval) {
        const regionalPrices = this._prices.filter((p) => p.region === region && p.interval === interval);
    ...
    transitionTo(targetState) {
        ...
        if (!allowed.includes(targetState)) {
            throw new Error(`Invalid plan lifecycle transition from ${this._lifecycleState} to ${targetState}.`);
        }
    ...
    static createPro(version = 1, amountUSDMinor = 1499, amountINRMinor = 79900) {
        return new Plan({
            planId: "PRO",
    ...
    static createPowerBidder(version = 1, amountUSDMinor = 3999, amountINRMinor = 299900) {
        return new Plan({
            planId: "POWER_BIDDER",
...
export class PlanCatalog {
     ...
     */
    validateForNewSelection(planId) {
        const plan = this._plans.get(planId);
        if (!plan) {
            throw new Error(`Plan with ID ${planId} not found in catalog.`);
        }
        if (plan.lifecycleState !== "ACTIVE") {
            throw new Error(`Plan ${planId} is in ${plan.lifecycleState} state and cannot be selected for a new subscription.`);
        }
    }
    /**
     * Validates plan eligibility for an existing subscription.
     */
    validateForExistingSubscription(planId) {
        const plan = this._plans.get(planId);
        if (!plan) {
            throw new Error(`Plan with ID ${planId} not found in catalog.`);
        }
        if (plan.lifecycleState === "RETIRED") {
            throw new Error(`Plan ${planId} is RETIRED and cannot remain active for existing subscriptions.`);
        }
     ...
     */
    resolvePrice(planId, clientCountry, clientCurrency) {
        const plan = this._plans.get(planId);
        if (!plan) {
            throw new Error(`Plan with ID ${planId} not found in catalog.`);
        }
        ...
        if (!regionalPrice) {
            ...
            if (!regionalPrice) {
                throw new Error(`Critical: Plan ${planId} does not have a GLOBAL fallback price definition.`);
            }
        }
        ...
        if (clientCountry) {
            ...
            const clientResolved = PricingRegionResolver.resolveRegionAndCurrency(normalizedClientCountry);
