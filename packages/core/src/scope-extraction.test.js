...
import { ScopeExtractionLifecycle, ScopeFactType, ScopeFactValue, ScopeSourceReference, ScopeEvidence, ScopeFact, ScopeExtractionSnapshot, ScopeExtraction, ScopeExtractionDraftedEvent, } from "./scope-extraction.js";
describe("Scope Extraction Domain and Boundary Tests", () => {
    ...
    test("Scope fact properties validation and immutability", () => {
        ...
        assert.throws(() => {
            fact.metadata.sourceConfidence = 1.0;
        }, TypeError);
    });
    ...
    test("Aggregate lifecycle transitions and snapshots history", () => {
        ...
        assert.throws(() => {
            aggregate.snapshots.push({});
        }, TypeError);
    });
    test("Domain events validation and payload purity", () => {
        const aggregate = ScopeExtraction.draft("ext-001", "client-abc");
        aggregate.completeExtraction();
        const completedEvent = aggregate.domainEvents.find((ev) => ev.eventName === "SCOPE_EXTRACTION_COMPLETED");
