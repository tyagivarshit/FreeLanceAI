...
import { ScoreWeightProfile, ScoringConfiguration, SignalContribution, ScoreBreakdown, ScoreFingerprint, roundToPrecision, JobMatchScoreSnapshot, JOB_MATCH_SCORE_CREATED, JOB_MATCH_SCORE_CALCULATED, JOB_MATCH_SCORE_ARCHIVED, JobMatchScore, } from "./job-match-score.js";
describe("Chapter 8E — Job Weighted Scoring Domain Tests", () => {
    ...
    describe("1. Score Identity", () => {
        test("valid construction and logical properties", () => {
            const score = JobMatchScore.create("score-123", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
        ...
        test("identity fields are immutable", () => {
            const score = JobMatchScore.create("score-123", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            assert.throws(() => {
                score.jobMatchId = "mutated";
            }, TypeError);
            assert.throws(() => {
                score.scoringVersion = "v2";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("2. Weight Profile", () => {
        test("valid weight profile setup, immutable", () => {
            ...
            assert.throws(() => {
                profile.weightProfileVersion = "v2";
            }, TypeError);
            assert.throws(() => {
                profile.weights.skillCoverage = 0.9;
            }, TypeError);
        });
        ...
        test("optional weights sum validation invariant", () => {
            ...
            const customValidateSum = (profile) => {
    ...
    describe("3. Numeric Signals", () => {
        test("semanticSimilarity range check and shifts-to-positive normalization", () => {
            ...
            score.calculate("owner-1", {
                ...defaultMatchSignals,
                semanticSimilarity: 0.0,
            }, config);
            assert.strictEqual(score.finalScore, 0.5);
            // Rejects out of bounds similarity
            const scoreErr = JobMatchScore.create("score-123", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            assert.throws(() => {
                scoreErr.calculate("owner-1", {
                    ...defaultMatchSignals,
    ...
    describe("4. Compatibility Mapping", () => {
        test("controlled states mapping evaluated deterministically with strict UNKNOWN handling", () => {
            ...
            const expContrib = breakdown.contributions.find((c) => c.signalName === "experienceCompatibility");
            const budgetContrib = breakdown.contributions.find((c) => c.signalName === "budgetCompatibility");
    ...
    describe("5. Missing-Signal Policy", () => {
        test("available-weight policy sum isolation", () => {
            ...
            const semanticContrib = score.breakdown.contributions.find((c) => c.signalName === "semanticSimilarity");
    ...
    describe("7. Final Score", () => {
        test("version-aware and scales to 0-1, 0-100, 0-1000 scales with precision", () => {
            ...
            const score100 = JobMatchScore.create("score-123", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            score100.calculate("owner-1", {
                ...defaultMatchSignals,
            ...
            const score01 = JobMatchScore.create("score-123", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            score01.calculate("owner-1", {
                ...defaultMatchSignals,
                semanticSimilarity: 0.5,
                skillCoverage: 1.0,
            }, config01);
            ...
            const score1000 = JobMatchScore.create("score-123", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            score1000.calculate("owner-1", {
                ...defaultMatchSignals,
                semanticSimilarity: 0.5,
                skillCoverage: 1.0,
            }, config1000);
    ...
    describe("8. ScoreBreakdown", () => {
        test("ScoreBreakdown stores all signal details immutably", () => {
            ...
            assert.throws(() => {
                breakdown.contributions.push(contrib);
            }, TypeError);
            assert.throws(() => {
                breakdown.contributions[0].signalName = "mutated";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("12. Archive Operation", () => {
        test("transitions CREATED -> ARCHIVED and CALCULATED -> ARCHIVED correctly", () => {
            const score1 = JobMatchScore.create("score-1", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            score1.archive("owner-1");
            assert.strictEqual(score1.status, "ARCHIVED");
            const score2 = JobMatchScore.create("score-2", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
    ...
    describe("13. Invalid Lifecycle", () => {
        test("rejects backwards and invalid state updates", () => {
            const score = JobMatchScore.create("score-1", "tenant-1", "owner-1", "match-123", "v1", "v1", "v1");
            score.calculate("owner-1", defaultMatchSignals, defaultScoringConfiguration);
            // Rejects CALCULATED -> CREATED
            assert.throws(() => {
                score.transitionTo("CREATED");
            ...
            assert.throws(() => score.calculate("owner-1", defaultMatchSignals, defaultScoringConfiguration), /Invalid lifecycle transition from ARCHIVED to CALCULATED/);
    ...
    describe("20. Persistence contracts", () => {
        test("neutral contracts compile and run successfully", () => {
            const dummyStore = {
                ...
                async findByScoringIdentity(_tenantId, _jobMatchId, _matchingVersion, _scoringVersion, _weightProfileVersion) {
                    return null;
                },
            };
            const dummyContract = {
