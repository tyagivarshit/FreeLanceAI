...
import { ModelReference, JobVectorFingerprint, JobEmbeddingSnapshot, JOB_EMBEDDING_CREATED, JOB_EMBEDDING_GENERATED, JOB_EMBEDDING_ARCHIVED, JobEmbedding, } from "./job-embedding.js";
describe("Chapter 8C — Job Embedding Domain Tests", () => {
    ...
    describe("1. Source Normalization Reference", () => {
        test("valid jobNormalizationId and normalizationVersion", () => {
            const agg = JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
        ...
        test("tenant isolation validation", () => {
            ...
            const aggA = JobEmbedding.create("emb-a", "tenant-A", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
            const aggB = JobEmbedding.create("emb-b", "tenant-B", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
        ...
        test("reference immutability", () => {
            const agg = JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
            assert.throws(() => {
                agg.jobNormalizationId = "mutated";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("2. Embedding Version", () => {
        test("v1 accepted and rejects invalid formats", () => {
            ...
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "", defaultModelRef, defaultVector, defaultDimensions, "input-fp", defaultVectorFingerprint);
            }, /Embedding Version is required/);
            // Invalid string format
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "version1", defaultModelRef, defaultVector, defaultDimensions, "input-fp", defaultVectorFingerprint);
            }, /Invalid embedding version format/);
        });
        test("embeddingVersion is immutable", () => {
            const agg = JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
            assert.throws(() => {
                agg.embeddingVersion = "v2";
            }, TypeError);
        });
        test("participates in identity and reprocessing boundary", () => {
            // If version differs (v1 vs v2), they are distinct aggregates
            const aggV1 = JobEmbedding.create("emb-v1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
            const aggV2 = JobEmbedding.create("emb-v2", "tenant-1", "owner-1", "norm-123", "v1", "v2", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
    ...
    describe("4. Vector Validation", () => {
        test("valid numeric vector accepted", () => {
            const agg = JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [0.0, 1.25, -0.77], 3, "input-fp-123", defaultVectorFingerprint);
        ...
        test("invalid vector configurations rejected", () => {
            // Empty vector
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [], 0, "input-fp", defaultVectorFingerprint);
            }, /Vector must be non-empty/);
            // NaN elements
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [0.5, NaN], 2, "input-fp", defaultVectorFingerprint);
            }, /Every vector element must be a finite number/);
            // Infinity elements
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [0.5, Infinity], 2, "input-fp", defaultVectorFingerprint);
            }, /Every vector element must be a finite number/);
            // -Infinity elements
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [0.5, -Infinity], 2, "input-fp", defaultVectorFingerprint);
            ...
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [0.5, "0.2"], 2, "input-fp", defaultVectorFingerprint);
            }, /Every vector element must be a number/);
            // Dimension mismatch
            assert.throws(() => {
                JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, [0.5, 0.2], 3, "input-fp", defaultVectorFingerprint);
    ...
    describe("5. Vector Immutability", () => {
        test("constructor defensive copies & getter immutability prevent modifications", () => {
            ...
            const agg = JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, inputVector, 3, "input-fp-123", defaultVectorFingerprint);
            ...
            assert.throws(() => {
                vectorFromGetter[0] = 9.9;
            }, TypeError);
            // Attempt array mutations
            assert.throws(() => {
                vectorFromGetter.push(0.4);
            }, TypeError);
            assert.throws(() => {
                vectorFromGetter.pop();
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("8. Aggregate Creation", () => {
        test("initializes state as CREATED with snapshot version 1 and domain event", () => {
            const agg = JobEmbedding.create("emb-123", "tenant-abc", "owner-xyz", "norm-456", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-abc", defaultVectorFingerprint);
    ...
    describe("10. Archive Operation", () => {
        test("transitions CREATED -> ARCHIVED and GENERATED -> ARCHIVED correctly", () => {
            const agg1 = JobEmbedding.create("emb-1", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
            agg1.archive("owner-1");
            assert.strictEqual(agg1.status, "ARCHIVED");
            assert.strictEqual(agg1.snapshots.length, 2);
            const agg2 = JobEmbedding.create("emb-2", "tenant-1", "owner-1", "norm-123", "v1", "v1", defaultModelRef, defaultVector, defaultDimensions, "input-fp-123", defaultVectorFingerprint);
    ...
    describe("16. Persistence Contracts compilation", () => {
        test("signatures match technological neutrality rules", () => {
            const dummyStore = {
                async save(_embedding) { },
                async findById(_id, _tenantId) {
                    return null;
                },
                async findByNormalizationReference(_tenantId, _jobNormalizationId, _normalizationVersion, _embeddingVersion, _modelReference) {
                    return null;
                },
            };
            const dummyContract = {
