...
import { Project, ProjectMetadata, ProjectVisibility, PROJECT_CREATED, PROJECT_UPDATED, PROJECT_STARTED, PROJECT_PAUSED, PROJECT_COMPLETED, PROJECT_CANCELLED, PROJECT_ARCHIVED, } from "./project.js";
describe("Project Domain Aggregate Tests", () => {
    ...
    test("Project creation success: status Draft and PROJECT_CREATED event emitted", () => {
        const project = Project.create("project-1", "tenant-1", "client-1", "owner-1", "ref-proj-1", validMetadata, validVisibility);
    ...
    test("Lifecycle transitions: Active/Paused -> Cancelled -> Archived", () => {
        const p1 = Project.create("p1", "tenant-1", "client-1", "owner-1", "ref-1", validMetadata, validVisibility);
        ...
        const p2 = Project.create("p2", "tenant-1", "client-1", "owner-1", "ref-2", validMetadata, validVisibility);
    ...
    test("Invalid lifecycle status transitions throw error", () => {
        const project = Project.create("project-1", "tenant-1", "client-1", "owner-1", "ref-1", validMetadata, validVisibility);
