...
import { ExplanationPolicy, ExplanationFact, ExplanationModel, ExplanationFingerprint, JobMatchExplanationSnapshot, JOB_MATCH_EXPLANATION_CREATED, JOB_MATCH_EXPLANATION_GENERATED, JOB_MATCH_EXPLANATION_ARCHIVED, JobMatchExplanation, } from "./job-match-explanation.js";
describe("Chapter 8G — Match Explanation Domain Tests", () => {
    ...
    describe("1. Explanation Identity", () => {
        test("valid construction and logical properties", () => {
            const explanation = JobMatchExplanation.create("exp-123", "tenant-1", "owner-1", "match-123", "score-123", "rank-123", "v1", "v1", "v1", "v1", "v1");
        ...
        test("identity fields are immutable", () => {
            const explanation = JobMatchExplanation.create("exp-123", "tenant-1", "owner-1", "match-123", "score-123", "rank-123", "v1", "v1", "v1", "v1", "v1");
            assert.throws(() => {
                explanation.jobMatchId = "mutated";
            }, TypeError);
            assert.throws(() => {
                explanation.explanationVersion = "v2";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("3. Evidence Consumption", () => {
        test("MatchSignals, ScoreBreakdown and Ranking consumed as-is", () => {
            ...
            assert.strictEqual(explanation.explanationModel.rankingContext, "Ranked #2 out of 15 candidates.");
        });
    });
    // ==========================================
    // 4. EXPLANATION FACTS
    ...
    describe("4. Explanation Facts", () => {
        test("matched skill, missing skill, compatibility state and rank facts are generated", () => {
            ...
            const missingFact = facts.find((f) => f.factType === "MISSING_SKILL" && f.rawValue === "python");
            const expFact = facts.find((f) => f.factType === "EXPERIENCE_COMPATIBILITY");
            const rankFact = facts.find((f) => f.factType === "RANKING_CONTEXT" && f.signalName === "rank");
            assert.ok(skillFact);
    ...
    describe("5. Unknown Handling", () => {
        test("UNKNOWN is preserved and does not become a gap mismatch claim", () => {
            ...
            assert.ok(!budgetGap);
            assert.strictEqual(explanation.explanationModel.compatibility.budgetCompatibility, "UNKNOWN");
        });
    });
    // ==========================================
    ...
    describe("9. Duplicate Facts", () => {
        test("deduplicates redundant evidence", () => {
            ...
            const nodeFacts = explanation.facts.filter((f) => f.factType === "MATCHED_SKILL" && f.rawValue === "node");
    ...
    describe("15. Archive Operation", () => {
        test("archive transitions aggregates correctly", () => {
            const exp1 = JobMatchExplanation.create("exp-1", "tenant-1", "owner-1", "match-123", "score-123", "rank-123", "v1", "v1", "v1", "v1", "v1");
            ...
            const exp2 = JobMatchExplanation.create("exp-2", "tenant-1", "owner-1", "match-123", "score-123", "rank-123", "v1", "v1", "v1", "v1", "v1");
    ...
    describe("16. Invalid Lifecycle", () => {
        test("rejects backwards states from GENERATED and terminal states", () => {
            const exp = JobMatchExplanation.create("exp-1", "tenant-1", "owner-1", "match-123", "score-123", "rank-123", "v1", "v1", "v1", "v1", "v1");
            exp.generate("owner-1", defaultEvidence, defaultPolicy);
            assert.throws(() => {
                exp.transitionTo("CREATED");
            }, /Invalid lifecycle transition from GENERATED to CREATED/);
            exp.archive("owner-1");
            assert.strictEqual(exp.status, "ARCHIVED");
            assert.throws(() => exp.generate("owner-1", defaultEvidence, defaultPolicy), /Invalid lifecycle transition from ARCHIVED to GENERATED/);
    ...
    describe("19. Version Compatibility", () => {
        test("rejects mismatches on matchingVersion, scoringVersion or rankingVersion", () => {
            ...
            const exp = JobMatchExplanation.create("exp-1", "tenant-1", "owner-1", "match-123", "score-123", "rank-123", "v1", // matchingVersion
            "v1", // scoringVersion
            "v1", // rankingVersion
            "v1", "v1");
    ...
    describe("23. ExplanationModel Immutability", () => {
        test("model arrays and structures are frozen", () => {
            ...
            assert.throws(() => {
                model.strengths.push("strength-2");
            }, TypeError);
            assert.throws(() => {
                model.compatibility.key = "mutated";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("26. Persistence contracts", () => {
        test("neutral contracts compile and run successfully", () => {
            const dummyStore = {
                async save(_explanation) { },
                async findById(_id, _tenantId) {
                    return null;
                },
                async findByExplanationIdentity(_tenantId, _jobMatchId, _matchingVersion, _scoringVersion, _rankingVersion, _explanationVersion, _explanationPolicyVersion) {
                    return null;
                },
            };
            const dummyContract = {
