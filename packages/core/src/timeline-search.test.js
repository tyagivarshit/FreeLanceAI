...
import { SearchQuery, AuthorizedSearchScope, SearchDomainError, assertSearchJsonSafe, TimelineSearchEngine, InMemoryTimelineSearchRepository, mapTimelineEntryToSearchResult, } from "./index.js";
describe("Phase 11D-6: Timeline Search Domain & Provider Tests", () => {
    ...
    test("5. actorRef matching finds all entries generated by a specific actor", async () => {
        ...
        assert.deepStrictEqual(res.results.map((r) => r.entityId), ["entry-003", "entry-001"]);
    });
    // --------------------------------------------------------------------------
    ...
    test("6. visibility matching filters entries by visibility classification", async () => {
        ...
        const res = await engine.search(query, scopeA);
        assert.strictEqual(res.count, 2);
        assert.deepStrictEqual(res.results.map((r) => r.entityId), ["entry-004", "entry-001"]);
    });
    // --------------------------------------------------------------------------
    ...
    test("9. maximum pageSize = 100 enforced", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "event", pageSize: 101 }), (err) => {
    ...
    test("10. deterministic ordering sorts strictly by timestamp DESC, then id DESC", async () => {
        ...
        assert.deepStrictEqual(res1.results.map((r) => r.entityId), res2.results.map((r) => r.entityId));
    ...
    test("14. forged ownerId and tenantId in search query payload are strictly rejected", () => {
        assert.throws(() => SearchQuery.fromRaw({ query: "event", ownerId: ownerB }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            assert.match(err.publicMessage, /Unknown search query parameter: ownerId/);
            return true;
        });
        assert.throws(() => SearchQuery.fromRaw({ query: "event", tenantId: tenantB }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
    ...
    test("15. safe SearchResult mapping formats canonical DTOs properly", () => {
        ...
        assert.strictEqual(searchResult.display.subtitle, "Public • Actor: agent-main • Ref: meeting-quarterly");
    ...
    test("17. malformed search queries fail safely with domain errors", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "event", page: -1 }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_PAGINATION");
            return true;
        });
    ...
    test("19. no duplicate results returned across paginated windows", async () => {
        const { engine, scopeA } = createTestEngine();
        const p1 = await engine.search(new SearchQuery({ query: "Event", page: 1, pageSize: 3 }), scopeA);
        const p2 = await engine.search(new SearchQuery({ query: "Event", page: 2, pageSize: 3 }), scopeA);
    ...
    test("20. provider-independent execution decouples domain from persistence engines", async () => {
        const customRepo = {
            searchTimeline: async (_q, _scope, page, pageSize) => ({
                items: [
                    {
                        id: "custom-entry-1",
