...
import { GenerationReference, GenerationConstraint, GenerationMetadata, GenerationContent, GenerationRequest, GenerationResult, ReplyGeneration, REPLY_GENERATION_DRAFTED, } from "./reply-generation.js";
describe("Chapter 7A — Reply Studio / Generation Domain Tests", () => {
    ...
    describe("GenerationConstraint", () => {
        ...
        test("invalid type rejected", () => {
            assert.throws(() => new GenerationConstraint("unsupported", "value"), /Invalid constraint type/);
        });
        test("invalid value rejected", () => {
            assert.throws(() => new GenerationConstraint("length", "xlarge"), /Invalid length constraint value/);
            assert.throws(() => new GenerationConstraint("format", "html"), /Invalid format constraint value/);
        });
        test("immutability", () => {
    ...
    describe("GenerationMetadata", () => {
        ...
        test("missing displayName rejected", () => {
            assert.throws(() => new GenerationMetadata({ displayName: "", description: "Desc" }), /Display Name/);
        });
        test("missing description rejected", () => {
            assert.throws(() => new GenerationMetadata({ displayName: "Name", description: "" }), /Description/);
        });
        test("immutability", () => {
            const meta = new GenerationMetadata({ displayName: "Name", description: "Desc" });
            assert.throws(() => {
                meta.displayName = "New";
            }, TypeError);
        });
    });
    // D. GenerationContent
    ...
    describe("GenerationRequest", () => {
        ...
        test("constraints and request immutability", () => {
            ...
            assert.throws(() => {
                req.constraints.push(new GenerationConstraint("format", "markdown"));
            }, TypeError);
        });
    });
    // F. GenerationResult
    ...
    describe("ReplyGeneration creation", () => {
        test("Draft state, snapshot, and event mapping", () => {
            const id = "gen-1";
            const aggregate = ReplyGeneration.create(id, defaultRef, "owner-1", "client-1", "conv-1", defaultRequest);
    ...
    describe("Ownership validation", () => {
        test("Command operation ownership enforcement", () => {
            const aggregate = ReplyGeneration.create("gen-1", defaultRef, "owner-1", "client-1", "conv-1", defaultRequest);
    ...
    describe("Lifecycle Transitions", () => {
        ...
        test("Draft -> Archived, Requested -> Archived, Generated -> Archived", () => {
            const agg1 = ReplyGeneration.create("gen-1", defaultRef, "owner-1", "client-1", "conv-1", defaultRequest);
            agg1.archive("owner-1");
            assert.strictEqual(agg1.status, "Archived");
            const agg2 = ReplyGeneration.create("gen-2", defaultRef, "owner-1", "client-1", "conv-1", defaultRequest);
            agg2.requestGeneration("owner-1");
            agg2.archive("owner-1");
            assert.strictEqual(agg2.status, "Archived");
            const agg3 = ReplyGeneration.create("gen-3", defaultRef, "owner-1", "client-1", "conv-1", defaultRequest);
        ...
        test("Archived is terminal, transitions out are blocked", () => {
            const agg = ReplyGeneration.create("gen-1", defaultRef, "owner-1", "client-1", "conv-1", defaultRequest);
            agg.archive("owner-1");
            assert.throws(() => {
    ...
    describe("Persistence Interfaces Compile Checks", () => {
        test("contracts signatures check", () => {
            const dummyStore = {
                async save(_gen) { },
                async findById(_id, _ownerId) {
                    return null;
                },
                async findByReference(_ref, _ownerId) {
                    return null;
                },
            };
            const dummyContract = {
                async checkUniqueReference(_ownerId, _reference, _excludeGenerationId) {
                    return true;
                },
            };
