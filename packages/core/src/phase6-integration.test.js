...
describe("Phase 6: Scope Intelligence Integration Tests", () => {
    ...
    const gatewayService = new MockAiGatewayService();
    ...
    test("6A: Scope Extraction enforces Zod whitelisting and Drizzle cross-tenant unique barriers", async () => {
        ...
        assert.deepStrictEqual(savedScope.scopeData.technicalRequirements, ["Node.js", "Redis"]);
        // Assert Hardware Constraint: Attempting to insert duplicate client scope should throw Drizzle constraint error
        await assert.rejects(extractionEngine.processExtractionResult(tenantId, clientId, rawAiResponse), (err) => err.code === "23505", // Postgres unique_violation error code
    ...
    test("6B: Scope Rules Engine runs async and securely logs DB isolated rule violations", async () => {
        ...
        const logs = await db.select().from(scopeRuleViolationsLog).where(and(eq(scopeRuleViolationsLog.tenantId, tenantId), eq(scopeRuleViolationsLog.clientId, clientId)));
        assert.strictEqual(logs.length, 1, "Should log exactly one violation");
        assert.strictEqual(logs[0].ruleKey, "NO_OUTSOURCING");
        assert.strictEqual(logs[0].violationContext.reason, "Mentions hiring external agencies.");
    });
    ...
    test("6D: Pricing Intelligence calculates enterprise markup cleanly with Numeric(12,2) bounds", async () => {
        ...
        await assert.rejects(pricingEngine.processPricingResult(tenantId, clientId, rawPricingResponse), (err) => err.code === "23505", "Should safely throw Drizzle constraint exception blocking cross-tenant pricing overrides");
    });
});
