...
import { MemoryUpdateReference, TargetMemoryReference, MemoryUpdateSpecification, MemoryUpdateClassification, MemoryUpdateSourceReference, MemoryUpdatePriority, MemoryUpdateFingerprint, ClientMemoryUpdateSnapshot, ClientMemoryUpdate, CLIENT_MEMORY_UPDATE_PROPOSED, CLIENT_MEMORY_UPDATE_VALIDATED, CLIENT_MEMORY_UPDATE_APPROVED, CLIENT_MEMORY_UPDATE_APPLIED, CLIENT_MEMORY_UPDATE_REJECTED, CLIENT_MEMORY_UPDATE_ARCHIVED, } from "./client-memory-update.js";
describe("Client Memory Update Aggregate & Invariants Tests", () => {
    ...
    const createDefaultSpec = () => new MemoryUpdateSpecification({
        operation: "Create",
    ...
    const createAlternativeSpec = () => new MemoryUpdateSpecification({
        operation: "Replace",
        target: "weekly-meeting-frequency",
    ...
    test("Aggregate creation: status Draft, snapshot version 1, properties matched", () => {
        ...
        const update = ClientMemoryUpdate.create("update-123", defaultUpdateRef, "client-456", "owner-789", defaultTargetRef, spec, defaultClassification, defaultSourceReference, defaultPriority, defaultFingerprint);
    ...
    test("Value Object Invariants: operation types and classifications", () => {
        ...
        assert.strictEqual(new MemoryUpdateSpecification({
            operation: "create",
            target: "t",
            proposedValue: "v",
            reason: "r",
    ...
    test("Snapshot history append-only and immutability", () => {
        const update = ClientMemoryUpdate.create("update-123", defaultUpdateRef, "client-456", "owner-789", defaultTargetRef, createDefaultSpec(), defaultClassification, defaultSourceReference, defaultPriority, defaultFingerprint);
        update.update("owner-789", alternativeTargetRef, createAlternativeSpec(), alternativeClassification, alternativeSourceReference, alternativePriority, alternativeFingerprint);
    ...
    test("Mock store interface compliance check", async () => {
        ...
        const updateObj = ClientMemoryUpdate.create("update-123", defaultUpdateRef, "client-456", "owner-789", defaultTargetRef, createDefaultSpec(), defaultClassification, defaultSourceReference, defaultPriority, defaultFingerprint);
        await mockStore.save(updateObj);
        const mockPersist = {
            checkUniqueReference: async (ownerId, reference, excludeUpdateId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludeUpdateId) {
    ...
    test("ClientMemoryUpdate properties immutability checks", () => {
        ...
        assert.throws(() => {
            update.id = "mutated-id";
        }, TypeError);
        assert.throws(() => {
            update.updateReference = alternativeUpdateRef;
        }, TypeError);
        assert.throws(() => {
            update.clientId = "client-mutated";
        }, TypeError);
        assert.throws(() => {
            update.ownerId = "owner-mutated";
        }, TypeError);
        assert.throws(() => {
            update.targetMemoryReference = alternativeTargetRef;
        }, TypeError);
        assert.throws(() => {
            update.specification = createAlternativeSpec();
        }, TypeError);
        assert.throws(() => {
            update.classification = alternativeClassification;
        }, TypeError);
        assert.throws(() => {
            update.sourceReference = alternativeSourceReference;
        }, TypeError);
        assert.throws(() => {
            update.priority = alternativePriority;
        }, TypeError);
        assert.throws(() => {
            update.fingerprint = alternativeFingerprint;
        }, TypeError);
    });
