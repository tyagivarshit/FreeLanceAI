...
import { EmbeddingReference, EmbeddingSourceReference, EmbeddingVector, EmbeddingSpace, EmbeddingFingerprint, EmbeddingSnapshot, Embedding, EMBEDDING_REGISTERED, EMBEDDING_VALIDATED, EMBEDDING_AVAILABLE, EMBEDDING_ARCHIVED, } from "./embedding.js";
describe("Embedding Aggregate & Invariants Domain Tests", () => {
    ...
    test("Aggregate creation: status Draft, snapshot version 1, properties matched", () => {
        const embedding = Embedding.create("emb-123", defaultRef, defaultSourceRef, defaultVector, defaultSpace, defaultFingerprint, "snap-1");
    ...
    test("Mock store interface compliance check", async () => {
        ...
        const embObj = Embedding.create("emb-123", defaultRef, defaultSourceRef, defaultVector, defaultSpace, defaultFingerprint, "snap-1");
        await mockStore.save(embObj);
        const mockPersist = {
            checkUniqueReference: async (reference, excludeEmbeddingId) => {
                assert.ok(reference);
    ...
    test("Embedding aggregate properties immutability checks", () => {
        ...
        assert.throws(() => {
            embedding.id = "mutated-id";
        }, TypeError);
        assert.throws(() => {
            embedding.embeddingReference = alternativeRef;
        }, TypeError);
        assert.throws(() => {
            embedding.sourceReference = alternativeSourceRef;
        }, TypeError);
        assert.throws(() => {
            embedding.vector = alternativeVector;
        }, TypeError);
        assert.throws(() => {
            embedding.space = alternativeSpace;
        }, TypeError);
        assert.throws(() => {
            embedding.fingerprint = alternativeFingerprint;
        }, TypeError);
    });
});
