...
describe("Conversation Import Domain Aggregate & Value Objects Tests", () => {
    ...
    const createAlternativeMetadata = () => new ImportMetadata({
    ...
    test("Aggregate creation success: initial status Draft, version 1 snapshot", () => {
        ...
        const importObj = ConversationImport.create("import-1", defaultImportRef, defaultConvRef, "client-123", "owner-456", defaultSource, metadata, defaultScope, defaultFingerprint);
    ...
    test("ImportFingerprint provider neutrality, format flexibility, and immutability", () => {
        ...
        const fpHash = new ImportFingerprint("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
        assert.strictEqual(fpHash.value, "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
    ...
    test("Append-only Snapshot History and immutability", () => {
        const importObj = ConversationImport.create("import-1", defaultImportRef, defaultConvRef, "client-123", "owner-456", defaultSource, createDefaultMetadata(), defaultScope, defaultFingerprint);
    ...
    test("Mock persistence unique reference check compliance", async () => {
        const mockPersist = {
            checkUniqueReference: async (ownerId, reference, excludeImportId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludeImportId) {
    ...
    test("ConversationImport mandatory fields immutability checks", () => {
        ...
        assert.throws(() => {
            importObj.id = "mutated-id";
        }, TypeError);
        // Import Reference immutability
        assert.throws(() => {
            importObj.importReference = new ImportReference("import.reference.new");
        }, TypeError);
        // Client Reference immutability
        assert.throws(() => {
            importObj.clientId = "mutated-client";
        }, TypeError);
        // Owner Reference immutability
        assert.throws(() => {
            importObj.ownerId = "mutated-owner";
        }, TypeError);
        // Conversation Reference immutability
        assert.throws(() => {
            importObj.conversationReference =
                new ConversationReference("conversation.mutated");
        }, TypeError);
        // Source Classification immutability
        assert.throws(() => {
            importObj.sourceClassification =
                new SourceClassification("WhatsApp");
        }, TypeError);
        // Import Metadata immutability
        assert.throws(() => {
            importObj.metadata = createAlternativeMetadata();
        }, TypeError);
        // Import Scope immutability
        assert.throws(() => {
            importObj.scope = alternativeScope;
        }, TypeError);
        // Import Fingerprint immutability
        assert.throws(() => {
            importObj.fingerprint = alternativeFingerprint;
        }, TypeError);
    });
});
