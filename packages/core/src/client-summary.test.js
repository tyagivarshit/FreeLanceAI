...
import { SummaryContent, SummaryScope, SummaryMetadata, SummaryClassification, SummaryFingerprint, SummaryReference, SummarySnapshot, ClientSummary, CLIENT_SUMMARY_REGISTERED, CLIENT_SUMMARY_GENERATED, CLIENT_SUMMARY_VALIDATED, CLIENT_SUMMARY_PUBLISHED, CLIENT_SUMMARY_ARCHIVED, } from "./client-summary.js";
describe("Client Summary Domain Aggregate & Value Objects Tests", () => {
    const createDefaultContent = () => new SummaryContent({
    ...
    const createAlternativeContent = () => new SummaryContent({
    ...
    const createDefaultMetadata = () => new SummaryMetadata({
        displayName: "Standard Onboarding Summary",
        description: "Overview of onboarding notes and objectives.",
        purpose: "Alignment",
        scope: defaultScope,
    });
    const createAlternativeMetadata = () => new SummaryMetadata({
    ...
    test("Aggregate creation success: initial status Draft, version 1 snapshot, registered event emitted", () => {
        ...
        const summary = ClientSummary.create("sum-1", defaultReference, "client-123", "owner-456", content, metadata, defaultClassification, defaultScope, defaultFingerprint);
    ...
    test("Immutable Summary content value object", () => {
        const content = createDefaultContent();
        assert.throws(() => {
            content.businessSummary = "Mutated value";
        }, TypeError);
        assert.throws(() => {
            content.knownGoals[0] = "Mutated goal";
        }, TypeError);
    });
    test("Metadata immutability", () => {
        const metadata = createDefaultMetadata();
        assert.throws(() => {
            metadata.displayName = "New Display Name";
        }, TypeError);
    });
    ...
    test("Append-only Snapshot History and completeness", () => {
        const summary = ClientSummary.create("sum-1", defaultReference, "client-123", "owner-456", createDefaultContent(), createDefaultMetadata(), defaultClassification, defaultScope, defaultFingerprint);
        // First update: should append snapshot version 2
        const alternativeContent = createAlternativeContent();
        summary.update("owner-456", alternativeContent, createAlternativeMetadata(), alternativeClassification, alternativeScope, alternativeFingerprint);
    ...
    test("Ownership validation works correctly", () => {
        ...
        assert.throws(() => {
            summary.update("wrong-owner", createAlternativeContent(), createAlternativeMetadata(), alternativeClassification, alternativeScope, alternativeFingerprint);
    ...
    test("Mock persistence unique reference check compliance", async () => {
        const mockPersist = {
            checkUniqueReference: async (ownerId, reference, excludeSummaryId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludeSummaryId) {
