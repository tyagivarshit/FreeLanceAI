...
export class JobMatchRankingPolicy {
    _rankingPolicyVersion;
    _primaryOrdering;
    _tieBreakPolicy;
    _rankNumberingConvention;
    constructor(rankingPolicyVersion, primaryOrdering, tieBreakPolicy, rankNumberingConvention) {
...
export class RankingFingerprint {
    ...
    constructor(properties) {
        ...
        if (properties.candidateScoreIdentities === undefined ||
...
export function buildCandidateSetIdentity(matchIds) {
...
export class JobMatchRankingSnapshot {
    ...
    constructor(properties) {
        ...
        this._rankedEntries = properties.rankedEntries
            ? [...properties.rankedEntries].map((r) => Object.freeze(r))
            : undefined;
...
export class JobMatchRanking {
    ...
    static create(id, tenantId, ownerId, rankingSubjectId, matchingVersion, scoringVersion, rankingVersion, rankingPolicyVersion, candidateMatchIds) {
        const now = new Date();
    ...
    transitionTo(nextStatus) {
        ...
        if (nextStatus === "RANKED") {
            if (this._status !== "CREATED") {
                throw new Error(`Invalid lifecycle transition from ${this._status.toUpperCase()} to RANKED`);
            }
        }
    ...
    rank(actorOwnerId, scoredMatches, rankingPolicy) {
        ...
        for (const match of scoredMatches) {
            ...
            if (typeof match.finalScore !== "number" ||
