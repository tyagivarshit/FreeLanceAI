...
export class ReplyGenerationEngineService {
   ...
   */
  private async validateTenantAccess(tenantId: string, clientId: string): Promise<boolean> {
   ...
   */
  async enqueueProposalGeneration(request: ProposalGenerationRequest): Promise<string> {
   ...
   */
  public async processGenerationStream(jobId: string, request: ProposalGenerationRequest) {
    ...
    try {
      ...
      try {
        ...
        const scopes = await db.select().from(clientScopes).where(
          and(eq(clientScopes.tenantId, request.tenantId), eq(clientScopes.clientId, request.clientId))
        );
