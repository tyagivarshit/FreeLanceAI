...
class ThrottledQueueManager {
    activeCount = 0;
    queue = [];
    CONCURRENCY_LIMIT = 5; // Safe threshold for AI Gateway
    async enqueue(task) {
...
export class InsightEngineService {
     ...
     */
    async extractAndSaveInsight(context) {
     ...
     */
    async persistInsight(tenantId, clientId, insight) {
...
export class InsightRepository {
     ...
     */
    async getActiveInsights(tenantId, clientId) {
        return await db
            .select()
            .from(clientInsights)
            .where(and(eq(clientInsights.tenantId, tenantId), eq(clientInsights.clientId, clientId), eq(clientInsights.status, "ACTIVE")));
    }
}
