...
import { AiGatewayService } from "./ai-gateway-service.js"; // In case dynamic summarization is needed during eviction
...
export class ClientMemoryEngineService {
  ...
  private readonly MAX_BUFFER_CHARS = 6000;
  private readonly MEMORY_DELIMITER = "\n---\n";
   ...
   */
  public async updateMemoryState(context: MemoryUpdateContext, maxRetries = 3): Promise<void> {
    let attempt = 0;
   ...
   */
  private async tryUpdateMemory(context: MemoryUpdateContext): Promise<void> {
    ...
    const existingRows = await db
      ...
      .where(
        and(
          eq(clientMemories.tenantId, tenantId),
          eq(clientMemories.clientId, clientId),
          eq(clientMemories.memoryKey, memoryKey)
        )
      )
      .limit(1);
    ...
    if (!existing) {
      ...
      try {
        ...
        if (err.code === "23505") throw new Error("OPTIMISTIC_LOCK_FAILED");
        throw err;
      }
    } else {
      ...
      const updateResult = await db
        ...
        .where(
          and(
            eq(clientMemories.id, existing.id),
            eq(clientMemories.version, currentVersion) // <--- The Core Optimistic Lock
          )
        );
   ...
   */
  public async getMemoryBuffer(tenantId: string, clientId: string, memoryKey: string): Promise<string | null> {
    const rows = await db
