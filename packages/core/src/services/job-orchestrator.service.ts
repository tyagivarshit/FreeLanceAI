...
import { JobNormalizationEngineService } from "./job-normalization-engine.service.js";
import { JobEmbeddingEngineService } from "./job-embedding-engine.service.js";
import { JobMatchingEngineService } from "./job-matching-engine.service.js";
import { JobMatchCacheService } from "./job-match-cache.service.js";
...
export interface OrchestratorEnvelope {
  ...
  ownerId: string;
  jobImportId: string;
  jobId?: string; // Resolved internally after normalization commits
  stage: "NORMALIZE" | "EMBED" | "MATCH" | "CACHE_INVALIDATE";
  attempt: number;
}
...
export class JobMatchingOrchestratorService {
  ...
  constructor(
    private readonly normalizationEngine: JobNormalizationEngineService,
    private readonly embeddingEngine: JobEmbeddingEngineService,
    private readonly matchingEngine: JobMatchingEngineService,
    private readonly cacheService: JobMatchCacheService,
   ...
   */
  public async startPipeline(tenantId: string, ownerId: string, jobImportId: string): Promise<string> {
   ...
   */
  public async dispatchStage(envelope: OrchestratorEnvelope): Promise<void> {
  ...
  public async startWorker(signal?: AbortSignal): Promise<void> {
    ...
    while (!signal?.aborted) {
      try {
        ...
        const envelope: OrchestratorEnvelope = JSON.parse(payloadString);
  ...
  private async processStage(envelope: OrchestratorEnvelope): Promise<void> {
    ...
    try {
      switch (envelope.stage) {
        ...
        case "EMBED":
          if (!envelope.jobId) throw new Error("Missing jobId context for EMBED stage.");
        ...
        case "MATCH":
          if (!envelope.jobId) throw new Error("Missing jobId context for MATCH stage.");
        ...
        case "CACHE_INVALIDATE":
          if (!envelope.jobId) throw new Error("Missing jobId context for CACHE_INVALIDATE stage.");
