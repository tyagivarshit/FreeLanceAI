...
import { GatewayRequestOptions } from "./ai-gateway-service.js";
...
export class AiQueueService {
  ...
  private workerRedis: Redis; // Dedicated connection for blocking BRPOP
   ...
   */
  async enqueueRequest(options: GatewayRequestOptions): Promise<string> {
  ...
  async startWorker(): Promise<void> {
    ...
    while (true) {
      try {
        ...
        const result = await this.workerRedis.brpop(
          "ai:queue:pending",
          "reply:queue:pending",
          "rewrite:queue:pending",
          "tone:queue:pending",
          "grammar:queue:pending",
          0
        );
        if (!result) continue;
   ...
   */
  private async processJobStream(jobId: string, options: GatewayRequestOptions) {
    ...
    try {
      ...
      if (options.contextReference) {
        if (options.contextReference.startsWith("scope_extraction_")) {
          ...
          const scopeExtractor = new ScopeExtractionEngineService(this, null as any);
          ...
          const rulesEngine = new ScopeRulesEngineService(this, null as any);
          ...
          const confEngine = new ConfidenceScoringEngineService(this, null as any);
          ...
          const priceEngine = new PricingIntelligenceEngineService(this, null as any);
  ...
  private async executeStreamingLlm(jobId: string, channel: string, options: GatewayRequestOptions): Promise<string> {
