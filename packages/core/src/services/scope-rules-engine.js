...
export class ScopeRulesEngineService {
     ...
     */
    async validateScopeCompliance(tenantId, clientId, extractedDataLogs) {
        ...
        const activeRules = await db.select()
            ...
            .where(and(eq(scopeComplianceRules.tenantId, tenantId), eq(scopeComplianceRules.isActive, true)));
     ...
     */
    async processValidationResult(tenantId, clientId, rawAiResponse) {
        try {
