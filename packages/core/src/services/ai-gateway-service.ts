...
import { UsageRepository } from "../entitlements.js";

export interface GatewayRequestOptions {
  tenantId: string;
  ownerId: string;
  contextReference: string; // e.g. "feature.generate_proposal"
  systemPrompt: string;
  userPrompt: string;
  temperature?: number;
}
...
export interface GatewayRepositoryContract {
...
export class AiGatewayService {
  ...
  private repo: GatewayRepositoryContract;
  private usageRepo: UsageRepository;
  
  // Example: 1 Million Tokens per day per tenant limit
  private readonly DAILY_TOKEN_LIMIT = 1000000; 

  constructor(repo: GatewayRepositoryContract, usageRepo: UsageRepository, env: { DEEPSEEK_API_KEY?: string, QWEN_API_KEY?: string }) {
    this.repo = repo;
  ...
  async generate(options: GatewayRequestOptions): Promise<GatewayResponse> {
  ...
  private async callDeepSeek(options: GatewayRequestOptions): Promise<GatewayResponse> {
    if (!this.deepseekApiKey) return { success: false, content: "", modelUsed: "deepseek", usage: {promptTokens:0,completionTokens:0,totalTokens:0}, error: "DeepSeek API key missing" };
  ...
  private async callQwen(options: GatewayRequestOptions): Promise<GatewayResponse> {
    if (!this.qwenApiKey) return { success: false, content: "", modelUsed: "qwen", usage: {promptTokens:0,completionTokens:0,totalTokens:0}, error: "Qwen API key missing" };
  ...
  private async logState(id: string, options: GatewayRequestOptions, status: string, metadataExtra: any = {}) {
    try {
