...
export class ReRankingEngineService {
   ...
   */
  public async reRankPayload<T extends ReRankCandidate>(
    tenantId: string,
    candidates: T[],
    queryText: string
  ): Promise<T[]> {
    ...
    for (const doc of boundedCandidates) {
      ...
      if (payloadTenantId && payloadTenantId !== tenantId) {
        throw new Error(
          `[SECURITY EXCEPTION - RE-RANKER] Multi-tenant context bleed blocked! Foreign payload detected. Expected Tenant: ${tenantId}, Found: ${payloadTenantId}`
        );
      }
    }
   ...
   */
  private async executeSemanticScoring<T extends ReRankCandidate>(
    candidates: T[],
    queryText: string
  ): Promise<T[]> {
