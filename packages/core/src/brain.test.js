...
import { BrainAnalysisRequest, BrainConfidence, BrainContext, BrainDomainError, BrainEvidence, BrainExecutionService, BrainFailure, BrainRequestMetadata, BrainResult, BrainScope, BrainAnalysisAggregate, InMemoryBrainAnalysisRepository, SUPPORTED_BRAIN_ANALYSIS_TYPES, mapBrainError, parseBrainAnalysisType, OpportunityReviewEngine, FollowUpPrioritizationEngine, HeuristicBrainEngine, BrainContextOrchestrator, BrainDecisionDeriver, } from "./brain.js";
const scope = () => new BrainScope({ tenantId: "tenant-1", ownerId: "owner-1", actorId: "actor-1" });
...
function entitlement(allowed = true, unavailable = false) {
    return {
        canUseBrain: async () => ({
            allowed,
            feature: "AI_PROPOSAL",
...
describe("Brain domain foundation", () => {
    ...
    test("2. invalid context", () => {
        assert.throws(() => new BrainScope({ tenantId: "", ownerId: "owner-1", actorId: "actor-1" }), /Tenant ID is required/);
    });
    test("3. missing required context", () => {
        assert.throws(() => new BrainAnalysisRequest({
            analysisType: "CLIENT_HEALTH",
    ...
    test("9. invalid request", () => {
        assert.throws(() => request({ constraints: { maxRecommendations: 99 } }), /Maximum recommendations/);
    });
    test("10. valid result", () => {
        assert.strictEqual(result().status, "COMPLETED");
    });
    test("11. invalid confidence", () => {
        assert.throws(() => new BrainConfidence({ score: Number.NaN, level: "LOW", supportingSignalCount: 0 }), /between 0 and 1/);
        assert.throws(() => new BrainConfidence({ score: Infinity, level: "LOW", supportingSignalCount: 0 }), /between 0 and 1/);
        assert.throws(() => new BrainConfidence({ score: -0.1, level: "LOW", supportingSignalCount: 0 }), /between 0 and 1/);
        assert.throws(() => new BrainConfidence({ score: 1.1, level: "HIGH", supportingSignalCount: 0 }), /between 0 and 1/);
    });
    test("12. invalid evidence", () => {
        assert.throws(() => new BrainEvidence({ sourceType: "CLIENT_SIGNAL", sourceId: "../secret", label: "Bad" }), /invalid reference format/);
    });
    test("13. malformed result", () => {
        assert.throws(() => result({ insights: [], recommendations: [] }), /require insight or recommendation/);
    });
    test("14. insufficient context", () => {
        assert.throws(() => request({ analysisType: "OPPORTUNITY_REVIEW", context: context({ jobs: [] }) }), /requires job signals/);
    });
...
describe("Brain provider boundary", () => {
    ...
    test("19. malformed provider result", async () => {
        ...
        const service = new BrainExecutionService({
            engine: new StaticEngine(malformed),
            entitlementGateway: entitlement(),
        });
        const output = await service.analyze(request());
...
describe("Brain security and serialization", () => {
    ...
    test("30. circular reference protection", () => {
        ...
        assert.throws(() => result({ metadata: circular }), /circular references/);
    });
    test("31. unknown fields behavior", () => {
        ...
        assert.strictEqual(output.providerConfig, undefined);
    });
    ...
    test("33. persisted analysis is strictly scoped to authenticated ownership", async () => {
        ...
        await assert.rejects(async () => repository.claimExecution("analysis-A", crossOwnerScope), /Cannot claim analysis across tenant\/owner boundary/);
    });
...
describe("Brain Analysis Lifecycle & State Machine", () => {
    ...
    test("37. valid failure state transitions: RUNNING -> FAILED / TIMEOUT / INSUFFICIENT_CONTEXT", () => {
        ...
        agg1.claim("actor-1");
        agg1.fail(new BrainFailure({ code: "PROVIDER_UNAVAILABLE", message: "Provider is down" }), "FAILED");
        assert.strictEqual(agg1.status, "FAILED");
        const agg2 = BrainAnalysisAggregate.create(request());
        agg2.claim("actor-1");
        agg2.fail(new BrainFailure({ code: "PROVIDER_TIMEOUT", message: "Provider timed out" }), "TIMEOUT");
        assert.strictEqual(agg2.status, "TIMEOUT");
        const agg3 = BrainAnalysisAggregate.create(request());
        agg3.claim("actor-1");
        agg3.fail(new BrainFailure({ code: "INSUFFICIENT_CONTEXT", message: "Context missing" }), "INSUFFICIENT_CONTEXT");
    ...
    test("38. invalid lifecycle transitions throw explicit domain errors", () => {
        ...
        assert.throws(() => agg.complete(result()), /Invalid lifecycle transition from REQUESTED to COMPLETED/);
        agg.claim("actor-1");
        agg.complete(result());
        // Transitioning from COMPLETED
        assert.throws(() => agg.claim("actor-1"), /Invalid lifecycle transition from COMPLETED to RUNNING/);
        assert.throws(() => agg.complete(result()), /Invalid lifecycle transition from COMPLETED to COMPLETED/);
    });
...
describe("Brain Execution Service with Persistence & Idempotency", () => {
    ...
    test("41. idempotency key returns authoritative cached completed result without re-execution", async () => {
        ...
        assert.strictEqual(executionCalls, 1, "Engine must only be called once for same idempotency key");
    });
...
describe("Opportunity Review & Heuristic Brain Engines", () => {
    ...
    test("49. OpportunityReviewEngine rejects unsupported analysis types", async () => {
        ...
        await assert.rejects(async () => engine.analyze(req), /OpportunityReviewEngine does not support CLIENT_HEALTH/);
    });
    test("50. HeuristicBrainEngine dispatches OPPORTUNITY_REVIEW, CLIENT_HEALTH, and FOLLOW_UP_PRIORITIZATION", async () => {
        ...
        const oppRes = await engine.analyze(new BrainAnalysisRequest({
            analysisType: "OPPORTUNITY_REVIEW",
            context: oppCtx,
            metadata: new BrainRequestMetadata({
                requestId: "req-h-1",
        ...
        const clientRes = await engine.analyze(new BrainAnalysisRequest({
            analysisType: "CLIENT_HEALTH",
            context: clientCtx,
            metadata: new BrainRequestMetadata({
                requestId: "req-h-2",
        ...
        const timelineRes = await engine.analyze(new BrainAnalysisRequest({
    ...
    test("51. FollowUpPrioritizationEngine prioritizes inactive timelines with evidence provenance", async () => {
        ...
        const res = await engine.analyze(new BrainAnalysisRequest({
            analysisType: "FOLLOW_UP_PRIORITIZATION",
            context: ctx,
            metadata: new BrainRequestMetadata({
                requestId: "req-fup-1",
    ...
    test("52. FollowUpPrioritizationEngine rejects non-FOLLOW_UP_PRIORITIZATION requests", async () => {
        ...
        await assert.rejects(async () => engine.analyze(req), /FollowUpPrioritizationEngine does not support OPPORTUNITY_REVIEW/);
    });
    ...
    test("54. BrainContextOrchestrator rejects missing or unauthorized foreign resources", async () => {
        ...
        await assert.rejects(async () => orchestrator.buildContext({
            analysisType: "CLIENT_HEALTH",
            scope: sc,
    ...
    test("59. BrainDecisionDeriver rejects non-COMPLETED analyses and analysis type mismatches", () => {
        ...
        assert.throws(() => deriver.deriveClientHealthDecision(failedResult), /Cannot derive decision from analysis with status FAILED/);
    });
