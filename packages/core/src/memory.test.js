...
import { Memory, MemoryMetadata, MemoryRetentionRule, MemoryEntry, MemorySnapshot, MEMORY_REGISTERED, MEMORY_VALIDATED, MEMORY_PUBLISHED, MEMORY_ARCHIVED, } from "./memory.js";
describe("Memory Aggregate Root and Value Objects Domain Tests", () => {
    ...
    test("Memory creation success: state Draft, snapshots initialized, registered event emitted", () => {
        const memory = Memory.create("mem-1", "client.profile.memory", "owner-123", defaultMetadata, defaultRetentionRules, "snap-1", defaultEntry);
        ...
        assert.strictEqual(memory.snapshots[0].entry.content, "Initial onboarding questionnaire data contents.");
        ...
        const event = memory.domainEvents[0];
    ...
    test("Memory reference format validation rejects invalid keys", () => {
        assert.throws(() => {
            Memory.create("mem-1", "client..memory", "owner-123", defaultMetadata, defaultRetentionRules, "snap-1", defaultEntry);
    ...
    test("Append snapshot to history: status unchanged, snapshot collection has append-only behavior", () => {
        ...
        assert.throws(() => {
            outsideSnapshots.push(new MemorySnapshot({
    ...
    test("Lifecycle flow: Draft -> Validated -> Published -> Archived", () => {
        ...
        assert.strictEqual(memory.domainEvents.length, 1);
        const validatedEvent = memory.domainEvents[0];
        ...
        assert.strictEqual(memory.domainEvents.length, 1);
        const publishedEvent = memory.domainEvents[0];
        ...
        assert.strictEqual(memory.domainEvents.length, 1);
        const archivedEvent = memory.domainEvents[0];
    ...
    test("Mock interfaces contract compliance check", async () => {
        ...
        const mockPersistence = {
            checkUniqueReference: async (ownerId, reference, excludeMemoryId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludeMemoryId) {
        ...
        const isUnique = await mockPersistence.checkUniqueReference("owner-123", "client.profile.memory");
