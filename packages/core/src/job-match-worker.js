...
export class JobMatchWorker {
    ...
    _activeClaimCount = 0;
    _loopTimeout;
    constructor(config, queue, store, cacheManager, resolver, observability) {
        this._config = config;
        this._queue = queue;
        this._store = store;
        ...
        try {
            ...
            if (claimed) {
                ...
                this.executeJob(claimed)
                    .catch((err) => {
                    console.error(`[JobMatchWorker] Unexpected runner failure on workItemId: ${claimed.workItemId}`, err);
                })
                    .finally(() => {
                    this._activeClaimCount--;
                    setImmediate(() => this.poll());
    ...
    async executeJob(workItem) {
        ...
        try {
            ...
            const freelancer = await this._resolver.getFreelancerProfile(workItem.jobMatchContext.freelancerId, tenantId);
            if (!freelancer || freelancer.tenantId !== tenantId) {
                throw new TenantError("Freelancer tenant isolation check failed.");
            }
            const jobNorm = await this._resolver.getJobNormalization(workItem.jobMatchContext.jobNormalizationId, tenantId);
            ...
            const jobEmb = await this._resolver.getJobEmbedding(workItem.jobMatchContext.jobEmbeddingId, tenantId);
            ...
            if (workItem.explanationVersion &&
            ...
            // 8D Matching Engine
            const jobMatch = JobMatch.create(matchId, tenantId, workItem.ownerId, freelancer.freelancerId, jobNorm.id, jobNorm.id, jobNorm.normalizationVersion, workItem.matchingVersion, jobEmb.id, jobEmb.vector && jobEmb.vector.length > 0 ? "v1" : undefined);
            ...
            this.checkCancellation(cancelController.signal);
            // 8E Scoring
            const matchScore = JobMatchScore.create(scoreId, tenantId, workItem.ownerId, matchId, workItem.matchingVersion, workItem.scoringVersion, scoringConfig.weightProfile.weightProfileVersion);
            matchScore.calculate(workItem.ownerId, jobMatch.matchSignals, scoringConfig);
            this.checkCancellation(cancelController.signal);
            // 8F Ranking
            const matchRanking = JobMatchRanking.create(rankingId, tenantId, workItem.ownerId, matchId, workItem.matchingVersion, workItem.scoringVersion, workItem.rankingVersion, rankingPolicy.rankingPolicyVersion, [matchId]);
            const scoredMatch = {
                matchId,
                scoreId,
                tenantId,
            ...
            this.checkCancellation(cancelController.signal);
            // 8G Explanation
            const matchExplanation = JobMatchExplanation.create(explanationId, tenantId, workItem.ownerId, matchId, scoreId, rankingId, workItem.matchingVersion, workItem.scoringVersion, workItem.rankingVersion, workItem.explanationVersion || "v1", explanationPolicy.explanationPolicyVersion);
            const evidence = {
                tenantId,
            ...
            const payload = {
                ...
                rankedMatches: matchRanking.snapshots.length > 0
                    ? matchRanking.snapshots[matchRanking.snapshots.length - 1]?.rankedEntries || []
                    : [],
                explanationFacts: matchExplanation.snapshots.length > 0
            ...
            try {
                ...
                console.warn(`[JobMatchWorker] Non-blocking L2 cache population failed for ${workItemId}:`, cacheErr);
            }
            ...
            if (freshItem.workerId !== this._config.workerId ||
                ...
                throw new PermanentInfrastructureError("Lease ownership was lost. Aborting transaction commit.");
            }
            if (freshItem.status !== "RUNNING" && freshItem.status !== "CANCELLING") {
                throw new PermanentInfrastructureError(`Invalid state sequence detected on commit: ${freshItem.status}`);
            }
            ...
            await this._store.complete(workItemId, tenantId, this._config.workerId, workItem.leaseId, workItem.fencingToken, resultReferences);
            await this._queue.acknowledge(workItemId, tenantId, this._config.workerId, workItem.leaseId, workItem.fencingToken);
    ...
    handleExecutionFailure(workItem, error) {
        const workItemId = workItem.workItemId;
        const tenantId = workItem.tenantId;
        ...
        (async () => {
            try {
                ...
                if (isRetryable && freshItem.attempt < workItem.maxAttempts) {
                    // Retry
                    await this._store.fail(workItemId, tenantId, this._config.workerId, leaseId, fencingToken, failure, workItem.maxAttempts, true);
                    // Exponential backoff + bounded jitter calculation
                    const attempt = freshItem.attempt + 1;
                    const delay = Math.min(this._config.baseDelayMs * Math.pow(2, attempt - 1), this._config.maxDelayMs);
                    const jitter = Math.random() * 100; // Cap jitter at +100ms
                    const totalDelay = Math.max(0, delay + jitter);
                    await this._queue.release(workItemId, tenantId, this._config.workerId, leaseId, fencingToken, totalDelay);
                    ...
                    await this._store.fail(workItemId, tenantId, this._config.workerId, leaseId, fencingToken, failure, workItem.maxAttempts, false);
                    await this._store.deadLetter(workItemId, tenantId);
                    await this._queue.deadLetter(workItemId, tenantId, this._config.workerId, leaseId, fencingToken);
            ...
            try {
                ...
                if (fresh.status === "CANCELLING" || fresh.status === "CANCELLED") {
                    ...
                    if (fresh.status === "CANCELLING") {
                        fresh.confirmCancellationByWorker(this._config.workerId, workItem.leaseId, workItem.fencingToken);
                ...
                // Fencing check
                if (fresh.workerId !== this._config.workerId ||
                ...
                await this._store.heartbeat(workItem.workItemId, workItem.tenantId, this._config.workerId, workItem.leaseId, workItem.fencingToken, this._config.leaseDurationMs);
    ...
    stopHeartbeatAndAbort(workItemId) {
    ...
    emitObservability(event, workItemId, payload) {
    ...
    buildMatchSignalsFingerprint(signals) {
