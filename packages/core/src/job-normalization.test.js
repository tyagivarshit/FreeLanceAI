...
import { JobSourceReference, CanonicalBudget, CanonicalLocation, CanonicalJob, JobNormalizedFingerprint, JobNormalizationSnapshot, JobNormalization, JOB_NORMALIZATION_CREATED, JOB_NORMALIZED, JOB_NORMALIZATION_ARCHIVED, } from "./job-normalization.js";
describe("Chapter 8B — Job Normalization Domain Tests", () => {
    ...
    describe("1. Source Reference", () => {
        ...
        test("tenant isolation validation in logical references", () => {
            ...
            const normTenantA = JobNormalization.create("norm-a", "tenant-A", "owner-1", sourceRef, "v1", defaultCanonicalJob, defaultFingerprint);
            const normTenantB = JobNormalization.create("norm-b", "tenant-B", "owner-1", sourceRef, "v1", defaultCanonicalJob, defaultFingerprint);
        ...
        test("JobSourceReference immutability", () => {
            ...
            assert.throws(() => {
                ref.jobImportId = "mutated";
            }, TypeError);
            assert.throws(() => {
                ref.externalJobId = "mutated";
            }, TypeError);
        });
    });
    // ==========================================
    ...
    describe("2. Normalization Version", () => {
        test("v1 accepted and version is immutable", () => {
            const agg = JobNormalization.create("norm-1", "tenant-1", "owner-1", defaultSourceRef, "v1", defaultCanonicalJob, defaultFingerprint);
            assert.strictEqual(agg.normalizationVersion, "v1");
            assert.throws(() => {
                agg.normalizationVersion = "v2";
            }, TypeError);
        });
        test("invalid version rejected", () => {
            // Empty version
            assert.throws(() => {
                JobNormalization.create("norm-1", "tenant-1", "owner-1", defaultSourceRef, "", defaultCanonicalJob, defaultFingerprint);
            }, /Normalization Version is required/);
            // Non-v[number] format
            assert.throws(() => {
                JobNormalization.create("norm-1", "tenant-1", "owner-1", defaultSourceRef, "version1", defaultCanonicalJob, defaultFingerprint);
            }, /Invalid normalization version format/);
            assert.throws(() => {
                JobNormalization.create("norm-1", "tenant-1", "owner-1", defaultSourceRef, "v1.2", defaultCanonicalJob, defaultFingerprint);
            }, /Invalid normalization version format/);
        });
        test("version participates in reprocessing and identity boundary", () => {
            ...
            const version1 = "v1";
            const version2 = "v2";
            const normV1 = JobNormalization.create("norm-v1-id", "tenant-1", "owner-1", defaultSourceRef, version1, defaultCanonicalJob, defaultFingerprint);
            const normV2 = JobNormalization.create("norm-v2-id", "tenant-1", "owner-1", defaultSourceRef, version2, defaultCanonicalJob, new JobNormalizedFingerprint("normalized-fingerprint-v2"));
    ...
    describe("3. CanonicalJob", () => {
        ...
        test("required field validation", () => {
            assert.throws(() => new CanonicalJob({ title: "", description: "test", skills: [] }), /title is required/);
            assert.throws(() => new CanonicalJob({ title: "test", description: "", skills: [] }), /description is required/);
            assert.throws(() => new CanonicalJob({
                title: "test",
                description: "test",
                skills: null,
        ...
        test("CanonicalJob and nested immutability", () => {
            ...
            assert.throws(() => {
                testJob.budget = defaultBudget;
            }, TypeError);
            if (job.budget) {
                assert.throws(() => {
                    job.budget.minimum = 5000;
                }, TypeError);
            }
            if (job.location) {
                assert.throws(() => {
    ...
    describe("7. Budget Normalization", () => {
        ...
        test("invalid budget formats rejected", () => {
            assert.throws(() => new CanonicalBudget({ type: "invalid" }), /Unsupported budget type/);
            assert.throws(() => new CanonicalBudget({ type: "fixed", minimum: -5 }), /Budget minimum cannot be negative/);
            assert.throws(() => new CanonicalBudget({ type: "fixed", minimum: 1000, maximum: 500 }), /Budget minimum cannot be greater than maximum/);
        });
    ...
    describe("14. Aggregate Creation", () => {
        test("creates aggregate in CREATED status with snapshot version 1 and domain event", () => {
            const agg = JobNormalization.create("norm-123", "tenant-abc", "owner-789", defaultSourceRef, "v1", defaultCanonicalJob, defaultFingerprint);
    ...
    describe("15 & 16. Lifecycle", () => {
        ...
        test("illegal lifecycle transitions rejected, archived is terminal", () => {
            ...
            assert.throws(() => agg.markNormalized("owner-1"), /Invalid lifecycle transition from ARCHIVED to NORMALIZED/);
    ...
    describe("20. Domain Events", () => {
        test("exact names, payloads, snapshot versions, no infrastructure metadata, and frozen", () => {
            ...
            assert.throws(() => {
                evCreated.normalizationId = "mutated";
            }, TypeError);
    ...
    describe("21. Persistence Contracts", () => {
        test("interfaces compile and remain technology-neutral", () => {
            const dummyStore = {
                async save(_normalization) { },
                async findById(_id, _tenantId) {
                    return null;
                },
                async findBySource(_tenantId, _jobImportId, _normalizationVersion) {
                    return null;
                },
            };
            const dummyContract = {
