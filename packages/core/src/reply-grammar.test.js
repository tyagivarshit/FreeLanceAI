...
import { GrammarProfile, GrammarRequest, GrammarResult, ReplyGrammarCorrection, REPLY_GRAMMAR_DRAFTED, REPLY_GRAMMAR_REQUESTED, REPLY_GRAMMAR_CORRECTED, REPLY_GRAMMAR_ARCHIVED, } from "./reply-grammar.js";
describe("Chapter 7D — Reply Studio / Grammar Correction Domain Tests", () => {
    ...
    describe("Ownership validation", () => {
        test("ownership checks on command operations", () => {
            const aggregate = ReplyGrammarCorrection.create("grammar-1", "reply.1", "owner-1", 1, defaultRequest);
            // Unauthorized checks
            assert.throws(() => {
                aggregate.requestGrammarCorrection("owner-other");
            }, /Ownership validation failed: unauthorized owner context\./);
            assert.throws(() => {
                aggregate.completeGrammarCorrection("owner-other", new GrammarResult({ correctedText: "test", correctedAt: new Date() }));
            }, /Ownership validation failed: unauthorized owner context\./);
            assert.throws(() => {
    ...
    describe("Lifecycle Transitions", () => {
        ...
        test("Draft -> Archived, Requested -> Archived, Corrected -> Archived", () => {
            const agg1 = ReplyGrammarCorrection.create("grammar-1", "reply.1", "owner-1", 1, defaultRequest);
            agg1.archive("owner-1");
            assert.strictEqual(agg1.status, "Archived");
            const agg2 = ReplyGrammarCorrection.create("grammar-2", "reply.1", "owner-1", 1, defaultRequest);
            agg2.requestGrammarCorrection("owner-1");
            agg2.archive("owner-1");
            assert.strictEqual(agg2.status, "Archived");
            const agg3 = ReplyGrammarCorrection.create("grammar-3", "reply.1", "owner-1", 1, defaultRequest);
        ...
        test("Archived is terminal, transitions out are blocked", () => {
            const agg = ReplyGrammarCorrection.create("grammar-1", "reply.1", "owner-1", 1, defaultRequest);
            agg.archive("owner-1");
            assert.throws(() => {
    ...
    describe("Grammar completion rules", () => {
        test("requires valid result and stores it", () => {
            ...
            agg.requestGrammarCorrection("owner-1");
            assert.throws(() => {
                agg.completeGrammarCorrection("owner-1", null);
    ...
    describe("Persistence Interfaces Compile Checks", () => {
        test("signatures matching check", () => {
            ...
            const dummyContract = {
                async checkUniqueReference(_ownerId, _reference, _excludeCorrectionId) {
                    return true;
                },
            };
