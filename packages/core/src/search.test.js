...
describe("Phase 11D-1: Search Architecture & Domain Contract Tests", () => {
    ...
    test("2. whitespace-only and empty query rejected with INVALID_QUERY domain error", () => {
        assert.throws(() => new SearchQuery({ query: "" }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_QUERY");
            assert.strictEqual(err.statusCode, 400);
            return true;
        });
        assert.throws(() => new SearchQuery({ query: "   \t\n  " }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_QUERY");
            return true;
        });
        assert.throws(() => new SearchQuery({ query: null }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_QUERY");
            return true;
        });
    ...
    test("3. query length limit enforces maximum 500 characters", () => {
        ...
        assert.throws(() => new SearchQuery({ query: excessiveQueryText }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_QUERY");
    ...
    test("4. invalid page values (0, negative, float, NaN, non-number) rejected", () => {
        const invalidPages = [0, -1, -99, 1.5, NaN, Number.POSITIVE_INFINITY, "1"];
        for (const page of invalidPages) {
            assert.throws(() => new SearchQuery({ query: "valid", page }), (err) => {
                assert(err instanceof SearchDomainError);
    ...
    test("5. invalid pageSize values (0, negative, float, NaN, non-number) rejected", () => {
        ...
        for (const pageSize of invalidSizes) {
            assert.throws(() => new SearchQuery({ query: "valid", pageSize }), (err) => {
                assert(err instanceof SearchDomainError);
                assert.strictEqual(err.code, "INVALID_PAGINATION");
                return true;
            });
    ...
    test("6. pageSize maximum of 100 enforced", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "valid", pageSize: 101 }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_PAGINATION");
            assert.match(err.publicMessage, /cannot exceed maximum of 100/);
            return true;
        });
        assert.throws(() => new SearchQuery({ query: "valid", pageSize: 1000 }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_PAGINATION");
            return true;
        });
    ...
    test("7. unsupported result type rejected with UNSUPPORTED_RESULT_TYPE", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "valid", resultTypes: ["INVALID_TYPE"] }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "UNSUPPORTED_RESULT_TYPE");
            return true;
        });
        assert.throws(() => new SearchQuery({ query: "valid", resultTypes: ["USER"] }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "UNSUPPORTED_RESULT_TYPE");
            return true;
        });
    ...
    test("8. duplicate result types rejected with INVALID_SEARCH_REQUEST", () => {
        assert.throws(() => new SearchQuery({ query: "valid", resultTypes: ["CLIENT", "CLIENT"] }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            assert.match(err.publicMessage, /Duplicate result type/);
            return true;
        });
        assert.throws(() => new SearchQuery({ query: "valid", resultTypes: ["JOB", "MATCH", "JOB"] }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("9. unknown fields in SearchQuery payload are strictly rejected", () => {
        ...
        assert.throws(() => SearchQuery.fromRaw(forgedInput), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            assert.match(err.publicMessage, /Unknown search query parameter: ownerId/);
            return true;
        });
        assert.throws(() => SearchQuery.fromRaw({ query: "Acme", tenantId: "foreign-tenant" }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            assert.match(err.publicMessage, /Unknown search query parameter: tenantId/);
            return true;
        });
        assert.throws(() => SearchQuery.fromRaw({ query: "Acme", sqlInjection: "'; DROP TABLE clients; --" }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("11. invalid SearchResult properties rejected", () => {
        ...
        assert.throws(() => new SearchResult({
            resultType: "CLIENT",
            entityId: "",
        ...
        assert.throws(() => new SearchResult({
            ...
            entityId: "cli-1",
            display: { title: "Valid Title" },
            rawDatabaseRow: { id: "cli-1", secret: "foo" },
        }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
    ...
    test("12. SearchResultSet computes pagination and bounded boundaries correctly", () => {
        ...
        assert.throws(() => new SearchResultSet({
            results: [item1, item2],
            total: 2,
            page: 1,
    ...
    test("15. missing or invalid authorization scope rejected with UNAUTHORIZED_SCOPE", () => {
        assert.throws(() => new AuthorizedSearchScope({
            tenantId: "",
            ownerId: "owner-1",
        }), (err) => {
            assert(err instanceof SearchDomainError);
    ...
    test("16. tenant isolation contract prevents caller tenant tampering and scope leakage", () => {
        ...
        assert.throws(() => SearchQuery.fromRaw(rawQueryWithTenant), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("17. owner isolation contract prevents foreign owner tampering", () => {
        ...
        assert.throws(() => SearchQuery.fromRaw(rawQueryWithOwner), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("18. JSON-safe serialization generates valid DTOs and rejects non-serializable objects", () => {
        ...
        // Rejects functions
        assert.throws(() => assertSearchJsonSafe({ func: () => "evil" }, "testPayload"), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
        // Rejects symbols
        assert.throws(() => assertSearchJsonSafe({ sym: Symbol("secret") }, "testPayload"), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
        // Rejects BigInt
        assert.throws(() => assertSearchJsonSafe({ count: BigInt(123) }, "testPayload"), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("19. secret exclusion boundary rejects secret keys and tokens in search payloads", () => {
        ...
        for (const secretKey of secretKeys) {
            assert.throws(() => assertSearchJsonSafe({ [secretKey]: "super-confidential-value" }, "SecretCheck"), (err) => {
                assert(err instanceof SearchDomainError);
                assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
    ...
    test("20. provider-independent interface can be implemented and executed in pure domain context", async () => {
        ...
        class MockSearchProvider {
            ...
            async search(query, scope) {
                ...
                const scopedRecords = this._records.filter((r) => scope.matchesTenant(r.tenantId) && scope.matchesOwner(r.ownerId));
                ...
                const matched = typeFiltered.filter((r) => r.title.toLowerCase().includes(q) ||
                    r.snippet.toLowerCase().includes(q) ||
                    r.keywords.some((k) => k.includes(q)));
                // Map to canonical SearchResult items
                const results = matched.map((r) => new SearchResult({
        ...
        assert.strictEqual(res1.results.some((r) => r.entityId === "client-secret"), false);
