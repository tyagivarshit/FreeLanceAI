...
import { InsightContent, InsightClassification, InsightConfidence, InsightSourceReference, InsightMetadata, InsightFingerprint, InsightReference, ClientInsightSnapshot, ClientInsight, CLIENT_INSIGHT_IDENTIFIED, CLIENT_INSIGHT_VALIDATED, CLIENT_INSIGHT_PUBLISHED, CLIENT_INSIGHT_ARCHIVED, } from "./client-insight.js";
describe("Client Insights Domain Model & Invariants Tests", () => {
    const createDefaultContent = () => new InsightContent({
    ...
    const createAlternativeContent = () => new InsightContent({
    ...
    const createDefaultMetadata = () => new InsightMetadata({
    ...
    const createAlternativeMetadata = () => new InsightMetadata({
    ...
    test("Aggregate creation: status Draft, snapshot version 1, properties matched", () => {
        ...
        const insight = ClientInsight.create("insight-123", defaultReference, "client-456", "owner-789", content, defaultClassification, defaultConfidence, defaultSourceReference, metadata, defaultFingerprint);
    ...
    test("Snapshot history append-only and immutability", () => {
        const insight = ClientInsight.create("insight-123", defaultReference, "client-456", "owner-789", createDefaultContent(), defaultClassification, defaultConfidence, defaultSourceReference, createDefaultMetadata(), defaultFingerprint);
        const altContent = createAlternativeContent();
        const altMeta = createAlternativeMetadata();
        insight.update("owner-789", altContent, alternativeClassification, alternativeConfidence, alternativeSourceReference, altMeta, alternativeFingerprint);
    ...
    test("Lifecycle transitions: invalid path validation", () => {
        ...
        insight.publish("owner-789");
        assert.throws(() => {
            insight.update("owner-789", createAlternativeContent(), alternativeClassification, alternativeConfidence, alternativeSourceReference, createAlternativeMetadata(), alternativeFingerprint);
        }, /Cannot update/);
    });
    ...
    test("Mock interfaces contract compliance check", async () => {
        ...
        const insightObj = ClientInsight.create("insight-123", defaultReference, "client-456", "owner-789", createDefaultContent(), defaultClassification, defaultConfidence, defaultSourceReference, createDefaultMetadata(), defaultFingerprint);
        await mockStore.save(insightObj);
        const mockPersist = {
            checkUniqueReference: async (ownerId, reference, excludeInsightId) => {
                assert.ok(ownerId);
                assert.ok(reference);
                if (excludeInsightId) {
    ...
    test("ClientInsight properties immutability checks", () => {
        ...
        assert.throws(() => {
            insight.id = "mutated-id";
        }, TypeError);
        assert.throws(() => {
            insight.insightReference = alternativeReference;
        }, TypeError);
        assert.throws(() => {
            insight.clientId = "client-mutated";
        }, TypeError);
        assert.throws(() => {
            insight.ownerId = "owner-mutated";
        }, TypeError);
        assert.throws(() => {
            insight.content = createAlternativeContent();
        }, TypeError);
        assert.throws(() => {
            insight.classification = alternativeClassification;
        }, TypeError);
        assert.throws(() => {
            insight.confidence = alternativeConfidence;
        }, TypeError);
        assert.throws(() => {
            insight.sourceReference = alternativeSourceReference;
        }, TypeError);
        assert.throws(() => {
            insight.metadata = createAlternativeMetadata();
        }, TypeError);
        assert.throws(() => {
            insight.fingerprint = alternativeFingerprint;
        }, TypeError);
    });
});
