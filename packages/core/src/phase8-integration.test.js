...
test("Phase 8: Master Matching Orchestrator Certification Suite", async (t) => {
    ...
    const orchestrator = new JobMatchingOrchestratorService(normalizationEngine, embeddingEngine, matchingEngine, cacheService, redisUrl);
    ...
    await t.test("2. SIMULATE BACKOFF: Exponential Retry Algorithm on Transient Failures", async (ctx) => {
        ...
        const retries = dispatchSpy.mock.calls.filter((c) => c.arguments[0].attempt > 1);
    ...
    await t.test("3. EXCEPTION BOUNDARY: Cross-Tenant Asset Matching Spoofing is Blocked", async () => {
        ...
        try {
            ...
            assert.match(err.message, /Fatal: Orchestration envelope missing strict B2B isolation boundaries/, "Immediate Tenant Isolation Exception must fire.");
        }
    });
