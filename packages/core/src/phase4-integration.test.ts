...
import { SummaryMapReduceEngine, SummaryRepository } from "./client-summary.js";
import { ImportStreamEngine } from "./services/import-stream-engine.js";
import { InsightEngineService, InsightRepository } from "./services/insight-engine.js";
  ...
  describe("4C: Insights Engine", () => {
    it("should throttle concurrent insight extraction tasks to prevent HTTP 429 API rate limits", async () => {
      ...
      const savedInsights = await db
        ...
        .from(clientInsights)
        .where(and(
          eq(clientInsights.tenantId, TEST_TENANT_ID),
          eq(clientInsights.insightKey, "TEST_THROTTLED_KEY")
        ));
