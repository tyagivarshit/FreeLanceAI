...
import { QueryEmbedding, SearchScope, SearchFilter, SimilarityMetric, SimilarityScore, SearchLimit, SearchCandidate, VectorSearchResult, VectorSearchRequest, } from "./vector-search.js";
describe("Vector Search Domain and Boundary Tests", () => {
    ...
    test("Search candidate immutability", () => {
        ...
        assert.throws(() => {
            candidate.embeddingReference = defaultEmbeddingRef;
        }, TypeError);
    });
    ...
    test("Mock contracts compliance verification", async () => {
        ...
        const mockProvider = {
            searchCandidates: async (req) => {
                assert.ok(req);
                return new VectorSearchResult([]);
            },
        };
        const mockRepo = {
            querySimilarity: async (req) => {
                assert.ok(req);
                return new VectorSearchResult([]);
            },
        };
