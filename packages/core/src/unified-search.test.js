...
import { SearchQuery, AuthorizedSearchScope, SearchResult, SearchResultSet, SearchDomainError, assertSearchJsonSafe, UnifiedSearchEngine, ClientSearchEngine, InMemoryClientSearchRepository, JobSearchEngine, InMemoryJobSearchRepository, MatchSearchEngine, InMemoryMatchSearchRepository, TimelineSearchEngine, InMemoryTimelineSearchRepository, } from "./index.js";
describe("Phase 11D-7: Unified Search Orchestration Tests", () => {
    ...
    test("6. subset filtering skips excluded engines", async () => {
        ...
        await engine.search(new SearchQuery({ query: "test", resultTypes: ["CLIENT", "TIMELINE"] }), scopeA);
        assert.strictEqual(clientCalls, 1);
        assert.strictEqual(timelineCalls, 1);
    ...
    test("10. global page slicing slices merged results correctly across pages", async () => {
        const createItem = (id, score, type) => new SearchResult({
            resultType: type,
            entityId: id,
        ...
        const engine = new UnifiedSearchEngine({
            clientEngine: {
                search: async () => new SearchResultSet({
        ...
        const res1 = await engine.search(new SearchQuery({ query: "test", page: 1, pageSize: 2 }), scopeA);
        ...
        const res2 = await engine.search(new SearchQuery({ query: "test", page: 2, pageSize: 2 }), scopeA);
    ...
    test("11. pageSize is never exceeded", async () => {
        const createItems = (count, type) => Array.from({ length: count }, (_, i) => new SearchResult({
            resultType: type,
            entityId: `id-${i}`,
    ...
    test("14. CLIENT -> JOB -> MATCH -> TIMELINE tie-break ordering applies on identical scores", async () => {
        const createItem = (id, type) => new SearchResult({
            resultType: type,
            entityId: id,
            display: { title: id, subtitle: "sub" },
    ...
    test("15. entityId deterministic tie-break orders descending lexicographically", async () => {
        const createItem = (id) => new SearchResult({
            resultType: "CLIENT",
            entityId: id,
            display: { title: id, subtitle: "sub" },
            relevance: { score: 1.0, matchedFields: ["name"] },
    ...
    test("16. duplicate same type/entityId is removed during aggregation", async () => {
        ...
        const engine = new UnifiedSearchEngine({
            clientEngine: {
                search: async () => new SearchResultSet({ results: [item1, item2], total: 2, page: 1, pageSize: 20 }),
            },
            jobEngine: {
                search: async () => new SearchResultSet({ results: [], total: 0, page: 1, pageSize: 20 }),
            },
    ...
    test("17. same ID across different types is preserved and not merged", async () => {
        ...
        const engine = new UnifiedSearchEngine({
            clientEngine: {
                search: async () => new SearchResultSet({ results: [clientItem], total: 1, page: 1, pageSize: 20 }),
            },
            jobEngine: {
                search: async () => new SearchResultSet({ results: [jobItem], total: 1, page: 1, pageSize: 20 }),
            },
        });
        const res = await engine.search(new SearchQuery({ query: "test" }), scopeA);
    ...
    test("19. AuthorizedSearchScope is passed unmodified to every active engine", async () => {
        let capturedScopeClient = null;
        let capturedScopeJob = null;
        const engine = new UnifiedSearchEngine({
            clientEngine: {
                search: async (_q, s) => {
    ...
    test("20. forged ownerId and tenantId in search query payload are rejected", () => {
        assert.throws(() => SearchQuery.fromRaw({ query: "test", ownerId: ownerB }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
        assert.throws(() => SearchQuery.fromRaw({ query: "test", tenantId: tenantB }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("21. unauthorized scope fails fast", async () => {
        ...
        await assert.rejects(async () => unifiedEngine.search(query, null), (err) => {
            assert(err instanceof SearchDomainError);
    ...
    test("22. validation errors fail fast", async () => {
        const { unifiedEngine } = createTestHarness();
        await assert.rejects(async () => unifiedEngine.search(null, scopeA), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_SEARCH_REQUEST");
            return true;
        });
    ...
    test("23. provider error in an active engine fails unified search instead of empty success", async () => {
        ...
        await assert.rejects(async () => engine.search(new SearchQuery({ query: "test" }), scopeA), (err) => {
            assert(err instanceof SearchDomainError);
    ...
    test("27. secret-containing result data fails JSON safety assertion if tainted", () => {
        ...
        assert.throws(() => assertSearchJsonSafe(taintedResult, "TaintedResult"), (err) => {
            assert(err instanceof SearchDomainError);
            return true;
        });
    });
    ...
    test("29. deterministic results across repeated identical searches", async () => {
        ...
        assert.deepStrictEqual(res1.results.map((r) => `${r.resultType}:${r.entityId}`), res2.results.map((r) => `${r.resultType}:${r.entityId}`));
    });
    // --------------------------------------------------------------------------
