...
import { SearchQuery, AuthorizedSearchScope, SearchDomainError, assertSearchJsonSafe, MatchSearchEngine, InMemoryMatchSearchRepository, mapMatchToSearchResult, } from "./index.js";
describe("Phase 11D-4: Match Search Domain & Provider Tests", () => {
    ...
    test("4. case-insensitive matching produces identical results", async () => {
        ...
        assert.deepStrictEqual(resLower.results.map((r) => r.entityId), resUpper.results.map((r) => r.entityId));
    });
    // --------------------------------------------------------------------------
    // 5. No-Result Search
    // --------------------------------------------------------------------------
    ...
    test("8. pageSize maximum 100 enforced", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "react", pageSize: 101 }), (err) => {
    ...
    test("18. malformed search queries fail safely with domain errors", () => {
        ...
        assert.throws(() => new SearchQuery({ query: "react", page: 0 }), (err) => {
            assert(err instanceof SearchDomainError);
            assert.strictEqual(err.code, "INVALID_PAGINATION");
            return true;
        });
    ...
    test("20. repository interface enforces single bounded query without N+1 hydration", async () => {
        ...
        const singleCallRepo = {
            searchMatches: async (_q, _scope, page, pageSize) => {
                callCount++;
                return {
                    items: [
                        {
