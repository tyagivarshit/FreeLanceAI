...
const queueConnections = new WeakMap();
...
function createRedisConnection() {
    ...
    connection.on("error", (err) => {
        console.error("[Queue Redis Error] Unexpected socket error:", err instanceof Error ? err.message : err);
    });
    return connection;
}
/**
 ...
 */
export function registerQueue(name, options) {
...
export function registerWorker(name, processor, options) {
    ...
    const bullmqProcessor = async (job) => {
